...
---
# <a name="fruity-falls-game-details"></a>Fruity 便在 2007 年遊戲詳細資料
...
Fruity 落擴充所介紹的概念[BouncingGame 指南](~/graphics-games/cocossharp/first-game/index.md)中加入下列：
...
雖然 BouncingGame 著重於介紹核心 CocosSharp 概念，Fruity 落會示範如何一起放到遊戲成品。 因為本指南會參考 BouncingGame，讀取器應該先瞭解自行向[BouncingGame 指南](~/graphics-games/cocossharp/first-game/index.md)再閱讀本指南。

本指南涵蓋的實作和 Fruity 便在 2008 年的設計，提供可以協助您建立自己的遊戲的見解。 它涵蓋下列主題：


- [GameController 類別](#GameController_Class)
- [遊戲的實體](#Game_Entities)
- [水果圖形](#Fruit_Graphics)
- [物理](#Physics)
- [遊戲的內容](#Game_Content)
...
Fruity 便會使用`Velocity`和`Acceleration`值以控制其實體，類似於移動[BouncingGame](~/graphics-games/cocossharp/first-game/index.md)。 實體在名為的方法中實作其移動邏輯`Activity`，這會呼叫一次每個畫面格。 例如，我們可以看到的實作中移動`Fruit`類別`Activity`方法：
...
如稍早在本指南中所述，GameScene 負責所有遊戲物件具現化、 定位和間的物件 （例如衝突） 的邏輯。 所有物件都會都新增至其中的四個`CCLayer`執行個體：
