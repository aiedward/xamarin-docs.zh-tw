...
- 延遲的視圖配置傳遞已經過優化和擴充。
- 拖放 API 現在包含新的 `NSFilePromiseProvider` 和 `NSFilePromiseReceiver` 類別，以支援拖曳 flocking。
- 現有的控制項中已加入數個便利的函式：
...
- 已將新的新增 `NSGridView` 至將子視圖集合自動設定到具有可變大小之資料列和資料行的方格中，可以動態地隱藏或顯示。
...
- 介面已在 `SecKey` 所有平臺（iOS、tvOS、watchOS 和 macOS）中現代化並整合。
