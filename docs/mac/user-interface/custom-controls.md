...
在這些情況下，您可以輕鬆地從 `NSControl` 繼承，並建立自訂工具，透過程式C#代碼或 Xcode 的 Interface Builder，將其新增至應用程式的 UI。 藉由從 `NSControl` 繼承，您的自訂控制項就會自動擁有內建使用者介面控制項所擁有的所有標準功能（例如 `NSButton`）。

如果您的自訂使用者介面控制項只顯示資訊（例如自訂圖表和圖形工具），您可能會想要從 `NSView` 繼承而不是 `NSControl`。
...
在 Visual Studio for Mac 中，開啟您想要為其建立自訂使用者介面控制項的 Xamarin. Mac 專案（或建立新的）。 新增類別，並將其呼叫 `NSFlipSwitch`：
...
關於自訂類別的第一件事，就是從 `NSControl` 繼承，然後使用**Register**命令將此類別公開給目標-C 和 Xcode 的 Interface Builder：
...
因為我們的自訂控制項是參數，所以我們需要一種方式來追蹤交換器的開啟/關閉狀態。 我們會使用 `NSFlipSwitch`中的下列程式碼來處理這種情況：
...
當交換器的狀態變更時，我們需要一種方法來更新 UI。 這麼做的方法是強制控制項使用 `NeedsDisplay = true`重繪其 UI。
...
當控制項的狀態變更時（例如從**On**到**Off**），我們將調整其視覺表示。 當狀態變更時，我們可以使用 `NeedsDisplay = true` 命令來強制控制項重新繪製新的狀態。
...
在上述程式碼中，我們呼叫 `FlipSwitchState` 方法（定義于上方），以在 `MouseDown` 方法中翻轉參數的開啟/關閉狀態。 這也會強制重新繪製控制項，以反映目前的狀態。
...
接下來，編輯 `FlipSwitchState` 方法，使其看起來如下所示：
...
首先，我們會提供 `ValueChanged` 事件，讓我們可以在程式碼中C#將處理常式新增至，讓我們可以在使用者變更切換狀態時執行動作。

第二，由於我們的自訂控制項會繼承 `NSControl`，因此它會自動具有可在 Xcode 的 Interface Builder 中指派的**動作**。 若要在狀態變更時呼叫此**動作**，我們會使用下列程式碼：
...
若要使用 Interface Builder 加入控制項，請先執行 Xamarin. Mac 專案的全新組建，然後按兩下 `Main.storyboard` 檔案，在 Interface Builder 進行編輯 中開啟該檔案：
...
切換至 [**助理編輯器**]，並建立自訂控制項的 [**輸出**] （請務必將它系結到 `ViewController.h` 檔案，而不是 `.m` 檔案）：

[![](custom-controls-images/custom05.png "Configuring a new Outlet")](custom-controls-images/custom05.png#lightbox)

儲存您的變更，返回 Visual Studio for Mac 並允許變更同步。編輯 `ViewController.cs` 檔案，使 `ViewDidLoad` 方法看起來如下所示：
...
在這裡，我們會回應我們在 `NSFlipSwitch` 類別上定義的 `ValueChanged` 事件，並在使用者按一下控制項時寫出目前的**值**。
