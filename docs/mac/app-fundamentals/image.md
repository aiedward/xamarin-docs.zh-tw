...
- **影像資料格**-當做以資料表為基礎的控制項（`NSTableView` 或 `NSOutlineView`）的一部分，影像可以用於影像資料格（`NSImageCell`）。
- **工具列專案**-影像可以加入工具列（`NSToolbar`）做為影像工具列專案（`NSToolbarItem`）。
- **來源清單圖示**-做為來源清單的一部分（特殊格式的 `NSOutlineView`）。
- **應用程式圖示**-一系列的影像可以群組在一起成為 `.icns` 組，並作為應用程式的圖示使用。 如需詳細資訊，請參閱我們的[應用程式圖示](~/mac/deploy-test/app-icon.md)檔。
...
- **自訂資料夾或子資料夾 [取代]** -開發人員可以將自訂資料夾加入至專案來源樹狀結構，並在該處儲存影像。 新增檔案的位置可以嵌套在子資料夾中，以進一步協助組織專案。 例如，如果開發人員將 [`Card`] 資料夾新增至專案，並將 `Hearts` 的子資料夾加入該資料夾，則會在 [`Hearts`] 資料夾中**儲存圖像，** `NSImage.ImageNamed("Card/Hearts/Jack.png")` 會在執行時間載入影像。
- **資產目錄映射集 [慣用]** -新增在 OS X El Capitan 中，**資產目錄影像集**包含支援應用程式的各種裝置和調整因素所需之映射的所有版本或標記法。 而不是依賴影像資產檔案名（ **@1x** ， **@2x** ）。
...
已新增至**影像集**的特殊**向量**類別，可讓我們在 casset 中包含_PDF_格式的向量影像，而不會在不同的解析度上包括個別的點陣圖檔案。 使用此方法時，您會提供 **@1x** 解析的單一向量檔案（格式化為向量 PDF 檔案），而且檔案的 **@2x** 和 **@3x** 版本將會在編譯時期產生，並包含在應用程式套件組合中.

[![映射集編輯器介面](image-images/imageset05.png "映射集編輯器介面")](image-images/imageset05-large.png#lightbox)

例如，如果您將 `MonkeyIcon.pdf` 檔案作為資產目錄的向量，並將其解析為 150px x 150px，則下列點陣圖資產會在編譯時包含在最終的應用程式套件組合中：
...
1. 以滑鼠右鍵按一下  **Solution Pad**中的專案，然後選取 **加入** > **新增檔案 ...**
...
2. 從 [**新增**檔案] 對話方塊中，選取要新增至專案的影像檔案，針對 [覆**寫組建] 動作**選取 [`BundleResource`]，然後按一下 [**開啟**] 按鈕：
...
> [!IMPORTANT]
> 如果您要將影像新增至 [**資源**] 資料夾，您可以將 [覆**寫組建] 動作**設定為 [**預設**]。 此資料夾的預設組建動作為 `BundleResource`。
...
當您在 Mac 上執行 Xamarin. Mac 應用程式時， _ImageName_ **。** _檔案名-延伸_格式影像將會用於標準解析度顯示， **ImageName@2x.filename-extension** 的影像會在 Retina 顯示基底 mac 上自動選取。
...
1. 使用 `BundleResource` 的 **建立 動作**，將影像新增至 **資源** 資料夾： 
...
上述步驟適用于可在**屬性偵測器**中設定其影像屬性的任何 UI 元素。 同樣地，如果您已包含影像檔案的 **@2x** 版本，它就會自動用於以 Retina 顯示為基礎的 mac。

> [!IMPORTANT]
> 如果 [**映射名稱**] 下拉式清單中的影像無法使用，請在 Xcode 中關閉您的分鏡腳本專案，並從 Visual Studio for Mac 將它重新開啟。 如果影像仍然無法使用，請確定其**組建動作**已 `BundleResource`，而且已將映射新增至**Resources**資料夾。
...
上述程式碼會使用 `NSImage` 類別的靜態 `ImageNamed("...")` 方法，從**Resources**資料夾將指定的影像載入記憶體中，如果找不到影像，`null` 將會傳回。 就像在 Interface Builder 中指派的映射一樣，如果您已包含影像檔案的 **@2x** 版本，它將會自動用於以 Retina 顯示為基礎的 mac。
...
> [!IMPORTANT]
> 特別是在 macOS Mojave 中呈現深色模式時，請務必在 reating 自訂轉譯 `NSImage` 物件時，避免 `LockFocus` API。 這類影像會變成靜態，而且不會自動更新以考慮外觀或顯示密度變更。
>
> 藉由採用上述以處理常式為基礎的機制，在裝載 `NSImage` （例如 `NSImageView`）時，會自動重新呈現動態條件。
...
這裡有幾行需要注意。 首先，對於我們要包含影像的資料行，我們會建立所需大小和位置的新 `NSImageView`，我們也會建立新的 `NSTextField`，並根據我們是否使用影像來放置其預設位置。:
...
其次，我們需要在父 `NSTableCellView` 中包含新的影像視圖和文字欄位：
