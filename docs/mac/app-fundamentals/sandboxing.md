...
2. 從 [**新增專案**] 對話方塊中，選取 [ **Mac**  > **應用程式** > **Cocoa 應用程式**]：
...
2. 選取**憑證、識別碼 & 設定檔**：
    ...
    [![選取憑證、識別碼 & 設定檔](sandboxing-images/sign02.png "選取憑證、識別碼 & 設定檔")](sandboxing-images/sign02-large.png#lightbox)
...
2. 請確認套件組合**識別碼**符合我們在上面建立的應用程式 ID （範例： `com.appracatappra.MacSandbox`）：
...
1. 在搜尋工具中，檢查 [`~/Library/Containers/`] 資料夾的內容-如果應用程式已沙箱化，則會有一個名為的資料夾，如同您應用程式的套件組合識別碼（例如： `com.appracatappra.MacSandbox`）：

    [![開啟應用程式的配套](sandboxing-images/sample09.png "開啟應用程式的配套")](sandboxing-images/sample09-large.png#lightbox)
2. 系統會在活動監視器中將應用程式視為沙箱：
    - 啟動 [活動監視器] （在 [`/Applications/Utilities`] 底下）。
    - 選擇 [ **View**  > **Columns** ]，並確定已核取 [**沙箱**] 功能表項目。
    - 請確定 [沙箱] 資料行讀取您應用程式的 `Yes`：

    [![檢查活動監視器中的應用程式](sandboxing-images/sample10.png "檢查活動監視器中的應用程式")](sandboxing-images/sample10-large.png#lightbox)
3. 檢查應用程式二進位檔是否已沙箱化：
    - 啟動終端機應用程式。
    - 流覽至 `bin` 目錄的應用程式。
    - 發出此命令： `codesign -dvvv --entitlements :- executable_path` （其中 `executable_path` 是您應用程式的路徑）：
...
針對上述範例應用程式，您可以看到內核因為應用程式沙箱而封鎖了 `network-outbound` 流量，因為我們並未要求該許可權。
...
- **相關專案**-具有適當的權利，您的應用程式可以存取具有相同名稱但副檔名不同的檔案。 例如，同時儲存為 `.txt` 檔案和 `.pdf` 的檔。
...
- 它位於使用者主目錄中的隱藏位置（通常是 `~Library/Containers`），可以使用應用程式內的 `NSHomeDirectory` 函式（請參閱下文）來存取。 因為它是在主目錄中，所以每個使用者都會取得自己的應用程式容器。
- 應用程式對容器目錄及其所有子目錄和檔案，具有不受限制的讀取/寫入存取權。
- 大部分的 macOS 路徑尋找 Api 都是相對於應用程式的容器。 例如，容器會有自己的連結**庫**（可透過 `NSLibraryDirectory`）、**應用程式支援**和**喜好**設定子目錄。
...
> [!IMPORTANT]
> 可惜的是，Xamarin 尚未有 100% API 涵蓋範圍（與 Xamarin 不同），因此 `NSHomeDirectory` API 尚未在目前的 Xamarin 版本中對應。
...
_Powerbox_是 macOS 的安全性技術，會與使用者互動以擴充您的沙箱化 Xamarin. Mac 應用程式的檔案存取權限。 Powerbox 沒有 API，但會在應用程式呼叫 `NSOpenPanel` 或 `NSSavePanel` 時，以透明的方式啟動。 Powerbox 存取是透過您為 Xamarin. Mac 應用程式所設定的權利來啟用。
...
- 讀取下列全球可讀取位置的檔案：
  ...
  - `/usr/share`
  - `/System`
- 在 `NSTemporaryDirectory` 所建立的目錄中讀取和寫入檔案。
...
應用程式沙箱可讓應用程式存取具有相同檔案名但副檔名不同的相關專案。 此功能有兩個部分： a）應用程式的 `Info.plst` 檔 b）中的相關延伸模組清單，告訴沙箱應用程式將使用這些檔案執行的動作。

有兩種情況是合理的：

1. 應用程式必須能夠儲存不同版本的檔案（使用新的副檔名）。 例如，將 `.txt` 檔案匯出至 `.pdf` 檔案。 若要處理這種情況，您必須使用 `NSFileCoordinator` 來存取檔案。 您將先呼叫 `WillMove(fromURL, toURL)` 方法，將檔案移至新的擴充功能，然後呼叫 `ItemMoved(fromURL, toURL)`。
2. 應用程式必須開啟具有一個副檔名的主要檔案，以及數個具有不同副檔名的支援檔案。 例如，電影和副標題檔案。 使用 `NSFilePresenter` 來取得次要檔案的存取權。 提供 `PrimaryPresentedItemURL` 屬性的主要檔案，以及 `PresentedItemURL` 屬性的次要檔案。 開啟主要檔案時，請呼叫 `NSFileCoordinator` 類別的 `AddFilePresenter` 方法，以註冊次要檔案。

在這兩種情況下，應用程式的**plist**檔案都必須宣告應用程式可以開啟的檔案類型。 針對任何檔案類型，將 `NSIsRelatedItemType` （值為 `YES`）新增至 `CFBundleDocumentTypes` 陣列中的專案。

#### <a name="open-and-save-dialog-behavior-with-sandboxed-apps"></a>使用沙箱應用程式開啟並儲存對話行為

下列限制會放在 `NSOpenPanel` 上，並在從沙箱化的 Xamarin. Mac 應用程式中呼叫它們時 `NSSavePanel`：

- 您無法以程式設計方式叫用 [**確定]** 按鈕。
- 您無法以程式設計方式改變 `NSOpenSavePanelDelegate` 中的使用者選取專案。
...
當使用者將資源檔匯入至專案時（透過 `NSOpenPanel`），應用程式會為儲存在專案中的專案建立檔範圍的書簽，讓應用程式一律可存取該檔案。
...
2. **建立安全性範圍的書簽**-您會針對使用者已提供存取權的任何檔案或資料夾（例如透過 `NSOpenPanel`）執行此動作，應用程式將需要持續存取。 使用 `NSUrl` 類別的 `public virtual NSData CreateBookmarkData (NSUrlBookmarkCreationOptions options, string[] resourceValues, NSUrl relativeUrl, out NSError error)` 方法來建立書簽。
3. **解析安全性範圍的書簽**-當應用程式需要再次存取資源時（例如重新開機後），必須將書簽解析為安全性範圍的 URL。 使用 `NSUrl` 類別的 `public static NSUrl FromBookmarkData (NSData data, NSUrlBookmarkResolutionOptions options, NSUrl relativeToUrl, out bool isStale, out NSError error)` 方法來解析書簽。
4. **明確通知系統您想要從安全性範圍的 url 存取**檔案-此步驟必須在取得上述安全性範圍的 url 之後立即完成，或者，當您稍後想要重新存取資源之後，釋放您對它的存取權。 呼叫 `NSUrl` 類別的 `StartAccessingSecurityScopedResource ()` 方法，以開始存取安全性範圍的 URL。
5. **明確通知系統您已從安全性範圍的 URL 完成存取**檔案-儘快，您應該在應用程式不再需要存取檔案時通知系統（例如，使用者關閉它時）。 呼叫 `NSUrl` 類別的 `StopAccessingSecurityScopedResource ()` 方法，以停止存取安全性範圍的 URL。
...
1. 開啟 [終端機] 應用程式（`/Applications/Utilities`）。
2. 開啟 [搜尋工具] 視窗至 Apple 已簽署的 Xamarin. Mac 應用程式版本。
3. 在終端機視窗中輸入 `asctl container acl add -file`。
...
- **將分散式通知中的使用者資訊字典傳送至其他**工作-使用應用程式沙箱，張貼至 `NSDistributedNotificationCenter` 物件以進行其他工作的訊息時，您就不能包含 `userInfo` 字典。
...
- **開啟、儲存和追蹤檔**-如果您要使用 `NSDocument` 以外的任何技術來管理檔，您應該切換到它，因為應用程式沙箱的內建支援。 `NSDocument` 會自動使用 PowerBox，並提供支援，讓您可以在使用者于搜尋工具中移動檔時，將其保留在您的沙箱中。
- **保留對檔案系統資源的存取**-如果 Xamarin 應用程式相依于其容器外的資源持續存取，請使用安全性範圍的書簽來維持存取權。
- **建立應用程式的登入專案**-使用應用程式沙箱，您無法使用 `LSSharedFileList` 建立登入專案，也不能使用 `LSRegisterURL` 來操作啟動服務的狀態。 依照[使用服務管理架構的「新增登入專案](https://developer.apple.com/library/prerelease/mac/documentation/MacOSX/Conceptual/BPSystemStartup/Chapters/CreatingLoginItems.html#//apple_ref/doc/uid/10000172i-SW5-SW1)」檔中所述，使用 `SMLoginItemSetEnabled` 函數。
- **存取使用者資料**-如果您使用 POSIX 函數（例如 `getpwuid` 從目錄服務取得使用者的主目錄），請考慮使用 Cocoa 或核心的符號，例如 `NSHomeDirectory`。
