...
### <a name="em0002-option-x-is-not-supported"></a>不支援 EM0002 `X` ：選項
...
此工具不支援此平臺`X`。 另一個版本的工具可能會支援它，或`X`平臺不適用於此環境。

<a name="EM0004" />

### <a name="em0004-the-target-x-is-not-valid"></a>EM0004：目標`X`無效。

此工具不支援目標`X`。 另一個版本的工具可能會支援它，而目標則`X`不會套用到此環境中。
...
此工具無法使用`xcode-select -p`命令找出目前選取的 Xcode 位置。 請確認您可以順利執行此命令，並傳回正確的 Xcode 位置。

<a name="EM0007" />

### <a name="em0007-could-not-get-the-sdk-version-for-sdk"></a>EM0007：無法取得 ' {sdk} ' 的 sdk 版本。

此工具無法使用`xcrun --show-sdk-version --sdk {sdk}`命令取得 SDK 版本。 請確認您可以順利執行此命令，並傳回 SDK 版本。
...
錯誤訊息中的架構對目標平臺無效。 請確認已針對`--abi`選項傳遞有效的架構。
...
工具無法剖析為命令列選項`A`提供的語法。 請參閱檔，以取得正確的語法。

<a name="EM0099" />

### <a name="em0099-internal-error--please-file-a-bug-report-with-a-test-case-httpsgithubcommonoembeddinator-4000issues"></a>EM0099：內部錯誤 *。 請以測試案例（https://github.com/mono/Embeddinator-4000/issues)）提出 bug 報告。
...
這是將**warning**忽略類型`T` （也就是不會產生任何內容）的警告，因為它使用`X`了不支援的功能。
...
這是將**warning**忽略類型`T` （也就是不會產生任何專案）的警告，因為它會從 .net framework 公開需要額外封送處理的內容。

注意：在未來版本的工具中，這是可能會受到支援的專案，但有一些限制。

<a name="EM1020" />

### <a name="em1020-constructor-c-is-not-generated-because-of-parameter-type-t-is-not-supported"></a>EM1020：因為`C`不支援參數類型`T` ，所以不會產生此函式。
...
### <a name="em1021-constructor-c-has-default-values-for-which-no-wrapper-is-generated"></a>EM1021：此`C`函式具有不會產生包裝函式的預設值。

這是一則**警告**，表示函式的`C`預設參數不會產生任何額外的程式碼。 最常見的原因是現有的方法已經具有相同的簽章。 例如，在 .NET 中，可能會有：
...
在這種情況下， `init`只會建立兩個產生的選取器，同時呼叫 Mono，但不會有後者的包裝函式。

<a name="EM1030" />

### <a name="em1030-method-m-is-not-generated-because-return-type-t-is-not-supported"></a>EM1030：不`M`會產生方法，因為不`T`支援傳回類型。

這是**警告**，會忽略方法`M` （也就是不會產生任何內容），因為不支援其`T`傳回型別。
...
這是**警告**，方法`M`的預設參數不會產生任何額外的程式碼。 最常見的原因是現有的方法已經具有相同的簽章。 例如，在 .NET 中，可能會有：
...
在這種情況下， `Increment`只會建立兩個產生的選取器，同時呼叫 Mono，但不會有後者的包裝函式。

<a name="EM1033" />

### <a name="em1033-method-m-is-not-generated-because-another-method-exposes-the-operator-with-a-friendly-name"></a>EM1033：方法`M`不會產生，因為另一個方法會以易記名稱公開運算子。

這是不**warning**會產生方法`M`的警告，因為另一個方法會使用易記名稱來公開運算子。 (https://msdn.microsoft.com/library/ms229032(v=vs.110).aspx)

<a name="EM1034" />

### <a name="em1034-extension-method-m-is-not-generated-inside-a-category-because-they-cannot-be-created-on-primitive-type-t-a-normal-static-method-was-generated"></a>EM1034：因為無法`M`在基本類型`T`上建立擴充方法，所以不會在類別內產生。 已產生一般的靜態方法。

這是一則**警告**，指出已找到 primivite 類型（例如`System.Int32`）上的擴充方法。 在 [目標-C] 中，您無法在基本型別的類型上建立類別目錄。 相反地，產生器會產生一般的靜態方法。
...
### <a name="em1050-field-f-is-not-generated-because-of-field-type-t-is-not-supported"></a>EM1050：未`F`產生欄位，因為不支援欄位`T`類型。
...
### <a name="em1051-element-e-is-generated-instead-as-f-because-its-name-conflicts-with-an-important-objective-c-selector"></a>EM1051：會`E`改為`F`產生元素，因為它的名稱與重要的目標-c 選取器衝突。

這是將**warning**產生元素`E`的警告， `F`因為它的名稱與重要的目標-c 選取器衝突。
...
### <a name="em1052-element-e-is-not-generated-its-name-conflicts-with-other-elements-on-the-same-class"></a>EM1052：未`E`產生元素的名稱與相同類別上的其他元素衝突。

這是**警告**，當元素`E`的名稱與相同類別上的其他專案衝突時，不會產生此專案。

<a name="EM1053" />

### <a name="em1053-target-e-is-not-supported-for-xamarinios-and-xamarinmac-only-the-framework-option-is-considered-supported-and-should-be-used"></a>EM1053：不`E`支援將目標用於 Xamarin. IOS 和 Xamarin. Mac。 只會`framework`將選項視為受支援，應加以使用。
