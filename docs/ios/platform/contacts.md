...
[![](contacts-images/add01.png "IOS 裝置上的範例連絡人工作表")](contacts-images/add01.png#lightbox)
...
`CNContact`類別提供安全線程的唯讀存取權給連絡人的屬性，例如名稱、位址或電話號碼。 `CNContact`之類的`NSDictionary`函式包含多個唯讀的屬性集合（例如位址或電話號碼）：

[![](contacts-images/contactobjects.png "Contact 物件總覽")](contacts-images/contactobjects.png#lightbox)

對於可以有多個值的任何屬性（例如電子郵件地址或電話號碼），它們會以物件的`NSLabeledValue`陣列表示。 `NSLabeledValue`是由一組唯讀的標籤和值組成的安全線程元組，其中標籤會定義使用者的值（例如，首頁或公司電子郵件）。 Contacts 架構提供預先定義的標籤（ `CNLabelKey`透過和`CNLabelPhoneNumberKey`靜態類別）供您在應用程式中使用，或者您可以選擇為您的需求定義自訂標籤。

針對需要調整現有連絡人的值（或建立新的）的任何 Xamarin 應用程式，請使用`NSMutableContact`類別的版本及其子類別（ `CNMutablePostalAddress`例如）。
...
[![](contacts-images/add01.png "新增至使用者集合的新連絡人")](contacts-images/add01.png#lightbox)
...
因為終端使用者可以根據每個應用程式來授與或拒絕其連絡人資訊的存取權，所以當您第一次呼叫`CNContactStore`時，將會顯示一個對話方塊，要求他們允許應用程式的存取權。
...
您可以使用`IsKeyAvailable` `CNContact`實例的或`AreKeysAvailable`方法，輕鬆檢查指定的連絡人是否具有所需的屬性。 例如：
...
[![](contacts-images/unified01.png "整合連絡人總覽")](contacts-images/unified01.png#lightbox)
...
如上述的「[連絡人物件](#Contact_Objects)」一節所示，您`CNContactStore`可以使用和的實例`CNMutableContact`來建立新的連絡人，然後使用`CNSaveRequest`下列方式，將它們寫入至使用者的連絡人資料庫：
...
也可以用來將多個連絡人和群組變更快取成單一作業，然後將這些修改批`CNContactStore`次到。 `CNSaveRequest`
...
每次修改連絡人時，contact Store 都會張貼`CNContactStoreDidChangeNotification`到預設的通知中心。 如果您已經快取或目前正在顯示任何連絡人，就必須從連絡人存放區（`CNContactStore`）重新整理這些物件。
...
[![](contacts-images/containers01.png "容器和群組總覽")](contacts-images/containers01.png#lightbox)
...
[![](contacts-images/containers02.png "容器和群組內的重迭")](contacts-images/containers02.png#lightbox)
...
Contact view Controller （`CNContactViewController`）類別提供一個控制器，向終端使用者呈現標準連絡人的觀點。 連絡人視圖可以顯示新的新的、未知或現有的連絡人，而且必須先指定類型，才能藉由呼叫正確的靜態函式`FromNewContact`（ `FromUnknownContact`， `FromContact`，）來顯示視圖。 例如：
