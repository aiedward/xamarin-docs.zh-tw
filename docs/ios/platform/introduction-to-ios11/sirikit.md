...
每個方法都有特定的意圖類型，其中包含從使用者的要求（也可能在和方法中更新）中 Siri 的所有資訊 `Resolve*` `Confirm*` 。
...
- `Ready`–在確認階段（即來自 `Confirm*` 方法，但不是從 `Handle*` 方法）傳回。
...
3. 呼叫 `INPreferences.RequestSiriAuthorization` 應用程式中的方法，以提示使用者允許 Siri 互動。
...
> [!NOTE]
>
> - 在 .NET 中，介面的慣例是以大寫開頭 `I` ，而 Xamarin 會在從 IOS SDK 系結通訊協定時遵守。
> - Xamarin 也會保留 iOS 的類型名稱，而 Apple 會使用類型名稱中的前兩個字元來反映類型所屬的架構。
> - 針對 `Intents` 架構，類型前面會加上 `IN*` （例如 `INExtension`），但這些_不_是介面。
> - 它也會遵循這些通訊協定（在 c # 中變成介面），最後會有兩個 `I` ，例如 `IINAddTasksIntentHandling` 。

#### <a name="handling-intents"></a>處理意圖

每個意圖（新增工作、設定工作屬性等等）都會在類似下面所示的單一方法中執行。 方法應該執行三個主要功能：

1. **處理意圖**： Siri 所剖析的資料可在 `intent` 特定于意圖類型的物件中使用。 您的應用程式可能已使用選擇性方法驗證該資料 `Resolve*` 。
...
只要您的 iOS 應用程式透過 plist 中的金鑰來支援它，您就可以設定自訂活動類型 `NSUserActivityTypes` 。 **Info.plist** 接著，您可以在應用程式開啟時處理這種情況，並執行特定作業（例如開啟相關的視圖控制器並從 Siri 作業載入資料）。
