...
[![使用 SiriKit 圖擴充應用程式](implementing-sirikit-images/elements01.png)](implementing-sirikit-images/elements01.png#lightbox)
...
2. 以滑鼠右鍵按一下**Solution Pad**中的方案名稱，然後選取 [**加入**  >  **新專案**]： 
    ...
    [![加入新的專案](implementing-sirikit-images/prep01.png)](implementing-sirikit-images/prep01.png#lightbox)
...
4. 輸入 `MonkeyChatCommon` 作為**名稱**，然後按一下 [**建立**] 按鈕： 
    ...
    [![在 [名稱] 中輸入 MonkeyChatCommon](implementing-sirikit-images/prep03.png)](implementing-sirikit-images/prep03.png#lightbox)
...
接下來，回到主要應用程式，並在 `using` 應用程式使用其中一個已移動類別的任何位置，為原生程式庫的命名空間新增語句：
...
2. 切換至 [來源]**** 索引標籤。
3. 新增 `com.apple.developer.siri` **屬性**，並將**類型**設定為 `Boolean` ，並將**值**設為 `Yes` ： 

    [![新增 siri 屬性。](implementing-sirikit-images/setup01.png)](implementing-sirikit-images/setup01.png#lightbox)
4. 將變更儲存至檔案。
5. 按兩下 [**方案總管**中的**專案**檔，將其開啟進行編輯。
6. 選取 [IOS 套件組合**簽署**]，並確認 `Entitlements.plist` 已在 [**自訂權利**] 欄位中選取檔案： 
...
1. 按兩下 `Entitlements.plist` [**方案總管**中的檔案以開啟它進行編輯。
2. 新增 `com.apple.developer.siri` **屬性**，並將**類型**設定為 `Boolean` ，並將**值**設為 `Yes` ： 

    [![新增 siri 屬性。](implementing-sirikit-images/setup01w.png)](implementing-sirikit-images/setup01w.png#lightbox)
3. 將變更儲存至檔案。
4. 按兩下 [**方案總管**中的**專案**檔，將其開啟進行編輯。
5. 選取 [IOS 套件組合**簽署**]，並確認 `Entitlements.plist` 已在 [**自訂權利**] 欄位中選取檔案。

-----

完成後，應用程式的檔案 `Entitlements.plist` 應該如下所示（在外部編輯器中開啟）：
...
編輯應用程式的檔案 `Info.plist` ，切換至**來源**視圖並加入 `NSSiriUsageDescription` 具有字串值的索引鍵，以描述應用程式將如何使用 Siri，以及將傳送哪些類型的資料。 例如，MonkeyChat 應用程式可能會說「MonkeyChat 連絡人將會傳送至 Siri」：
...
編輯應用程式的檔案 `Info.plist` ，並 `NSSiriUsageDescription` 以字串值新增金鑰，其中描述應用程式將如何使用 Siri，以及將傳送哪些類型的資料。 例如，MonkeyChat 應用程式可能會說「MonkeyChat 連絡人將會傳送至 Siri」：
...
`RequestSiriAuthorization` `INPreferences` 第一次啟動應用程式時，呼叫類別的方法。 編輯 `AppDelegate.cs` 類別，讓 `FinishedLaunching` 方法看起來如下所示：
...
第一次呼叫此方法時，會顯示警示，提示使用者允許應用程式存取 Siri。 開發人員新增到上述的訊息 `NSSiriUsageDescription` 將會顯示在此警示中。 如果使用者一開始拒絕存取，他們就可以使用 [**設定**] 應用程式來授與應用程式的存取權。

應用程式隨時都可以藉由呼叫類別的方法，檢查應用程式存取 Siri 的能力 `SiriAuthorizationStatus` `INPreferences` 。

### <a name="localization-and-siri"></a>當地語系化和 Siri

在 iOS 裝置上，使用者可以選取與系統預設值不同的 Siri 語言。 使用當地語系化資料時，應用程式必須使用類別的方法， `SiriLanguageCode` `INPreferences` 從 Siri 取得語言代碼。 例如：
...
應用程式會藉由呼叫 `SetVocabularyStrings` 類別的方法 `INVocabulary` ，並 `NSOrderedSet` 從主要應用程式傳入集合，來提供使用者特定的詞彙。 應用程式應該一律先呼叫 `RemoveAllVocabularyStrings` 方法，以移除任何現有的詞彙，然後再加入新的字詞。 例如：
...
- `IntentPhrases`**選擇性**-包含使用中定義之自訂詞彙的範例片語 `ParameterVocabularies` 。

中的每個專案 `ParameterVocabularies` 都必須指定識別碼字串、詞彙和詞彙適用的意圖。 此外，單一詞彙可能適用于多個意圖。
...
1. 在 [**方案總管**中的專案名稱上按一下滑鼠右鍵，**然後選取 [**  >  **新增檔案 ...**  >  ]。**iOS**：
    ...
    [![新增屬性清單](implementing-sirikit-images/plist01.png)](implementing-sirikit-images/plist01.png#lightbox)
2. 按兩下 `AppIntentVocabulary.plist` [**方案總管**中的檔案以開啟它進行編輯。
3. 按一下 **+** 以加入索引鍵，將**名稱**設為， `ParameterVocabularies` 並將**類型**設定為 `Array` ：

    [![將 [名稱] 設為 ParameterVocabularies，並將類型設定為 [陣列]](implementing-sirikit-images/plist02.png)](implementing-sirikit-images/plist02.png#lightbox)
4. 展開 `ParameterVocabularies` 並按一下 [] **+** 按鈕，並將**類型**設定為 `Dictionary` ：

    [![將 [類型] 設定為 [字典]](implementing-sirikit-images/plist03.png)](implementing-sirikit-images/plist03.png#lightbox)
5. 按一下 **+** 以加入新的索引鍵，將**名稱**設為， `ParameterNames` 並將**類型**設定為 `Array` ：

    [![將 [名稱] 設為 ParameterNames，並將類型設定為 [陣列]](implementing-sirikit-images/plist04.png)](implementing-sirikit-images/plist04.png#lightbox)
6. 按一下 **+** 以加入具有**類型**的新機碼 `String` ，並使用值做為其中一個可用的參數名稱。 例如，`INStartWorkoutIntent.workoutName`：
...
7. 將 `ParameterVocabulary` 金鑰新增至 `ParameterVocabularies` **類型**為的索引鍵 `Array` ：
...
8. 新增**型別為的新**機碼 `Dictionary` ：

    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist07.png)](implementing-sirikit-images/plist07.png#lightbox)
9. 新增 `VocabularyItemIdentifier` 具有**類型**的索引鍵 `String` ，並指定詞彙的唯一識別碼：
...
10. 新增 `VocabularyItemSynonyms` **類型**為的索引鍵 `Array` ：
...
11. 新增**型別為的新**機碼 `Dictionary` ：

    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist10.png)](implementing-sirikit-images/plist10.png#lightbox)
12. 新增 `VocabularyItemPhrase` 具有**類型**的索引鍵 `String` ，以及應用程式所定義的詞彙：

    [![新增具有字串類型的 VocabularyItemPhrase 索引鍵，以及應用程式所定義的詞彙](implementing-sirikit-images/plist11.png)](implementing-sirikit-images/plist11.png#lightbox)
13. 新增 `VocabularyItemPronunciation` 具有**類型**的索引鍵 `String` ，以及詞彙的語音發音：

    [![新增具有字串類型的 VocabularyItemPronunciation 索引鍵，以及詞彙的語音發音](implementing-sirikit-images/plist12.png)](implementing-sirikit-images/plist12.png#lightbox)
14. 新增 `VocabularyItemExamples` **類型**為的索引鍵 `Array` ：
...
15. 新增一些索引 `String` 鍵，其中包含下列詞彙的範例用法：
...
18. 新增 `IntentPhrases` **類型**為的索引鍵 `Array` ：

    [![新增具有陣列類型的 IntentPhrases 索引鍵](implementing-sirikit-images/plist15.png)](implementing-sirikit-images/plist15.png#lightbox)
19. 新增**型別為的新**機碼 `Dictionary` ：

    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist16.png)](implementing-sirikit-images/plist16.png#lightbox)
20. 在範例中新增 `IntentName` 具有和意圖**類型**的索引鍵 `String` ：

    [![在範例中新增具有字串類型和意圖的 IntentName 索引鍵](implementing-sirikit-images/plist17.png)](implementing-sirikit-images/plist17.png#lightbox)
21. 新增 `IntentExamples` **類型**為的索引鍵 `Array` ：

    [![新增具有陣列類型的 IntentExamples 索引鍵](implementing-sirikit-images/plist18.png)](implementing-sirikit-images/plist18.png#lightbox)
22. 新增一些索引 `String` 鍵，其中包含下列詞彙的範例用法：
...
1. 以滑鼠右鍵按一下 [**方案總管**中的專案名稱，然後選取 [**新增 > 新專案] > Apple > 屬性清單 > Info. plist**：

    [![加入新的資訊。 plist](implementing-sirikit-images/plist01.w157-sml.png)](implementing-sirikit-images/plist01.w157.png#lightbox)

2. 按兩下 `AppIntentVocabulary.plist` [**方案總管**中的檔案以開啟它進行編輯。
3. 按一下 **+** 以加入索引鍵，將**名稱**設為， `ParameterVocabularies` 並將**類型**設定為 `Array` ：

    [![將 [名稱] 設為 ParameterVocabularies，並將類型設定為 [陣列]](implementing-sirikit-images/plist02w.png)](implementing-sirikit-images/plist02w.png#lightbox)
4. 展開 `ParameterVocabularies` 並按一下 [] **+** 按鈕，並將**類型**設定為 `Dictionary` ：

    [![將 [類型] 設定為 [字典]](implementing-sirikit-images/plist03w.png)](implementing-sirikit-images/plist03w.png#lightbox)
5. 按一下 **+** 以加入新的索引鍵，將**名稱**設為， `ParameterNames` 並將**類型**設定為 `Array` ：

    [![將 [名稱] 設為 ParameterNames，並將類型設定為 [陣列]](implementing-sirikit-images/plist04w.png)](implementing-sirikit-images/plist04w.png#lightbox)
6. 按一下 **+** 以加入具有**類型**的新機碼 `String` ，並使用值做為其中一個可用的參數名稱。 例如，`INStartWorkoutIntent.workoutName`：
...
8. 新增**型別為的新**機碼 `Dictionary` ：
    ...
    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist07w.png)](implementing-sirikit-images/plist07w.png#lightbox)
9. 新增 `VocabularyItemIdentifier` 具有**類型**的索引鍵 `String` ，並指定詞彙的唯一識別碼：

    [![新增具有字串類型的 VocabularyItemIdentifier 索引鍵，並指定詞彙的唯一識別碼](implementing-sirikit-images/plist08w.png)](implementing-sirikit-images/plist08w.png#lightbox)
10. 新增 `VocabularyItemSynonyms` **類型**為的索引鍵 `Array` ：
...
11. 新增**型別為的新**機碼 `Dictionary` ：
    ...
    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist10w.png)](implementing-sirikit-images/plist10w.png#lightbox)
12. 新增 `VocabularyItemPhrase` 具有**類型**的索引鍵 `String` ，以及應用程式所定義的詞彙：

    [![新增具有字串類型的 VocabularyItemPhrase 索引鍵，以及應用程式所定義的詞彙](implementing-sirikit-images/plist11w.png)](implementing-sirikit-images/plist11w.png#lightbox)
13. 新增 `VocabularyItemPronunciation` 具有**類型**的索引鍵 `String` ，以及詞彙的語音發音：

    [![新增具有字串類型的 VocabularyItemPronunciation 索引鍵，以及詞彙的語音發音](implementing-sirikit-images/plist12w.png)](implementing-sirikit-images/plist12w.png#lightbox)
14. 新增 `VocabularyItemExamples` **類型**為的索引鍵 `Array` ：
...
19. 新增**型別為的新**機碼 `Dictionary` ：
    ...
    [![加入具有字典類型的新機碼](implementing-sirikit-images/plist16w.png)](implementing-sirikit-images/plist16w.png#lightbox)
20. 在範例中新增 `IntentName` 具有和意圖**類型**的索引鍵 `String` ：

    [![在範例中新增具有字串類型和意圖的 IntentName 索引鍵](implementing-sirikit-images/plist17w.png)](implementing-sirikit-images/plist17w.png#lightbox)
21. 新增 `IntentExamples` **類型**為的索引鍵 `Array` ：
...
> [!IMPORTANT]
> 在 `AppIntentVocabulary.plist` 開發期間，將會在測試裝置上向 Siri 註冊，而且可能需要一些時間，Siri 才能納入自訂詞彙。 因此，測試人員必須等待數分鐘，才能嘗試測試應用程式特定詞彙（當它已更新）。
...
4. 最後，按一下 [**建立**] 按鈕，將意圖延伸新增至應用程式解決方案： 
    ...
    [![將意圖延伸新增至應用程式解決方案](implementing-sirikit-images/intents07.png)](implementing-sirikit-images/intents07.png#lightbox)
...
2. 從對話方塊中，選取 [ **Visual c #] > IOS 擴充功能 > 意圖延伸**]，然後按 [**下一步]** 按鈕：
...
針對已新增至應用程式解決方案的每個意圖延伸模組，必須在檔案中設定， `Info.plist` 才能與應用程式搭配使用。

就像任何一般的應用程式擴充功能一樣，應用程式也會有和的現有金鑰 `NSExtension` `NSExtensionAttributes` 。 意圖擴充有兩個必須設定的新屬性：
...
若要設定意圖延伸的檔案 `Info.plist` ，請在 [**方案總管**中按兩下該檔案以開啟它進行編輯。 接下來，切換至**來源**視圖，然後展開 `NSExtension` 編輯器中的和索引 `NSExtensionAttributes` 鍵：

# <a name="visual-studio-for-mac"></a>[Visual Studio for Mac](#tab/macos)

[![編輯器中的 NSExtension 和 NSExtensionAttributes 鍵](implementing-sirikit-images/intents02.png)](implementing-sirikit-images/intents02.png#lightbox)

# <a name="visual-studio"></a>[Visual Studio](#tab/windows)

[![編輯器中的 NSExtension 和 NSExtensionAttributes 鍵](implementing-sirikit-images/intents02w.png)](implementing-sirikit-images/intents02w.png#lightbox)
...
[![新增的 INSendMessageIntent 鍵](implementing-sirikit-images/intents10.png)](implementing-sirikit-images/intents10.png#lightbox)

# <a name="visual-studio"></a>[Visual Studio](#tab/windows)

[![新增的 INSendMessageIntent 鍵](implementing-sirikit-images/intents10w.png)](implementing-sirikit-images/intents10w.png#lightbox)
...
接下來，開發人員必須將主要類別設定為 Siri 中意圖延伸的主要進入點。 它必須是符合委派的子類別 `INExtension` `IINIntentHandler` 。 例如：
...
應用程式必須在意圖延伸主要類別（方法）上執行的一個單獨方法 `GetHandler` 。 這個方法會透過 SiriKit 傳遞意圖，應用程式必須傳回符合給定意圖類型的**意圖處理常式**。

由於範例 MonkeyChat 應用程式只會處理一個意圖，因此它會在方法中傳回本身 `GetHandler` 。 如果延伸模組處理了一個以上的意圖，開發人員就會為每個意圖類型加入一個類別，並根據 `Intent` 傳遞給方法的來傳回這裡的實例。
...
1. 以滑鼠右鍵按一下**Solution Pad**中的**方案名稱**，**然後選取 [新增]**[  >  **新增專案**]。
2. 從對話方塊中，選取 [ **iOS**擴充功能] [  >  **Extensions**  >  **意圖 UI 延伸**模組]，然後按 [**下一步]** 按鈕 
...
4. 最後，按一下 [**建立**] 按鈕，將意圖延伸新增至應用程式解決方案： 
    ...
    [![將意圖延伸新增至應用程式解決方案](implementing-sirikit-images/intents13.png)](implementing-sirikit-images/intents13.png#lightbox)
5. 在 [**方案總管**中，以滑鼠右鍵按一下新建立之意圖延伸模組的 [**參考**] 資料夾。 檢查通用共用程式碼程式庫專案（如上所建立的應用程式）的名稱，然後按一下 [**確定]** 按鈕： 
...
1. 以滑鼠右鍵按一下**方案總管**中的**方案名稱**，**然後選取 [新增]**[  >  **新增專案**]。
2. 從對話方塊中選取 [ **iOS**擴充功能] [  >  **Extensions**  >  **意圖 UI 延伸**模組]，然後按 [**下一步]** 按鈕。
3. 接下來，輸入意圖延伸的**名稱**，然後按一下 [**確定]** 按鈕。
...
就像任何一般的應用程式擴充功能一樣，應用程式也會有和的現有金鑰 `NSExtension` `NSExtensionAttributes` 。 針對意圖延伸模組，有一個必須設定的新屬性：
...
若要設定意圖 UI 延伸模組的檔案 `Info.plist` ，請在**方案總管**中按兩下該檔案以開啟它進行編輯。 接下來，切換至**來源**視圖，然後展開 `NSExtension` 編輯器中的和索引 `NSExtensionAttributes` 鍵：

[![編輯器中的 NSExtension 和 NSExtensionAttributes 鍵](implementing-sirikit-images/intents04.png)](implementing-sirikit-images/intents04.png#lightbox)

# <a name="visual-studio"></a>[Visual Studio](#tab/windows)

若要設定意圖 UI 延伸模組的檔案 `Info.plist` ，請在**方案總管**中按兩下該檔案以開啟它進行編輯。 展開 `NSExtension` 編輯器中的和索引 `NSExtensionAttributes` 鍵：

[![在編輯器中 NSExtension 和 NSExtensionAttributes 索引鍵](implementing-sirikit-images/intents04w.png)](implementing-sirikit-images/intents04w.png#lightbox)
...
將作為意圖 UI 延伸模組主要進入點的主要類別設定為 Siri。 它必須是符合介面的子類別 `UIViewController` `IINUIHostedViewController` 。 例如：
...
Siri 會將 `INInteraction` 類別實例傳遞至 `Configure` `UIViewController` 意圖 UI 延伸模組內實例的方法。

`INInteraction`物件提供三個重要的資訊片段給延伸模組：

1. 正在處理的意圖物件。
2. 來自 `Confirm` 意圖延伸之和方法的意圖回應物件 `Handle` 。
3. 定義應用程式與 Siri 之間互動狀態的互動狀態。

`UIViewController`實例是與 Siri 互動的準則類別，因為它繼承自 `UIViewController` ，所以可以存取 UIKit 的所有功能。

當 Siri 呼叫它的方法時， `Configure` `UIViewController` 它會傳入 view 內容，指出 view 控制器將會裝載于 Siri Snippit 或 Maps 介面卡中。

Siri 也會傳入完成處理常式，應用程式必須在應用程式完成設定後，傳回所需的視圖大小。

### <a name="design-the-ui-in-ios-designer"></a>在 iOS 設計工具中設計 UI

在 iOS 設計工具中版面配置意圖 UI 延伸模組的使用者介面。 按兩下 [方案總管中的延伸模組檔案 `MainInterface.storyboard` ， **Solution Explorer**將它開啟以供編輯。 拖曳所有必要的 UI 元素，以建立使用者介面並儲存變更。
...
使用在 iOS 設計工具中建立的意圖 UI 延伸模組的使用者介面，編輯子 `UIViewController` 類別並覆寫 `Configure` 方法，如下所示：
...
如果意圖 UI 延伸模組需要覆寫預設 Siri UI 的專案，子 `UIViewController` 類別就必須執行 `IINUIHostedViewSiriProviding` 介面，並加入宣告以顯示特定的介面元素。

將下列程式碼新增至子 `UIViewController` 類別，以告知 Siri 意圖 UI 延伸已經顯示訊息內容：
