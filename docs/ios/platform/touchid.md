...
從 iOS 8 開始，現在已有新的使用者目前狀態原則 `SecAccessControl`，其會由 iPhone 5 秒和更新版本上的 secure 記憶體保護區強制執行。 在下表中，我們可以看到裝置設定會如何影響原則評估：
...
- `EvaluatePolicy` –這會啟動驗證作業並顯示 UI，並傳回 `true` 或 `false` 的答案。
...
2. 按兩下 [`MainStoryboard.Storyboard`]，在 iOS 設計工具中開啟範例。 在此範例中，我們想要將新畫面新增至應用程式，以控制驗證。 這會在目前的 `MasterViewController`之前。
...
4. 將新的 View Controller 命名為 `AuthenticationViewController`。
5. 接下來，拖曳按鈕，並將它放在 `AuthenticationViewController`上。 呼叫此 `AuthenticateButton`，並為它提供文字 `Add a Chore`。
6. 在稱為 `AuthenticateMe`的 `AuthenticateButton` 上建立事件。
7. 按一下底部的黑色列，然後從列中按**Ctrl + 拖曳**至 `MasterViewController` 並選擇 [**推送**] （或 [**顯示**是否使用大小] 類別），以從 `AuthenticationViewController` 建立手動 segue：
...
8. 按一下新建立的 segue，並為其指定識別碼 `AuthenticationSegue`，如下所示：
...
首先，我們需要建立裝置是否能夠接受 Touch ID 輸入，方法是使用 `CanEvaluatePolicy` 並傳入原則 `DeviceOwnerAuthenticationWithBiometrics`。 如果這是 true，我們可以使用 `EvaluatePolicy`顯示 Touch ID UI。 有三項資訊必須傳入 `EvaluatePolicy` –原則本身、說明為何需要驗證的字串，以及回復處理常式。 回復處理常式會告訴應用程式，它應該在驗證成功或失敗時執行的動作。 讓我們更仔細看一下回複處理常式：

[![](touchid-images/image9.png "The reply handler")](touchid-images/image9.png#lightbox)

回復處理常式的指定類型為 `LAContextReplyHandler`，其接受參數 success – a `bool` 值，以及稱為 `error`的 `NSError`。 如果成功，就會實際執行我們想要驗證的任何內容，在此情況下會顯示畫面，讓我們加入新的工作。 請記住，本機驗證的其中一個警告是必須在前景上執行，因此請務必使用 `InvokeOnMainThread`：
