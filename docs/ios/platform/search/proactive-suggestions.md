...
已新增 iOS 9，讓您能夠在網站中包含結構化資料標記，以來擴充使用者在焦點和 Safari 搜尋結果中看到的內容（請參閱[使用 Web 標記搜尋](~/ios/platform/search/web-markup.md)）。 iOS 10 新增了包含以位置為基礎之標記的功能（例如[Schema.org](https://schema.org/)所定義的[PostalAddress](https://schema.org/PostalAddress) ），以進一步加強使用者的體驗。 例如，如果使用者在網站上查看標示為頁面的位置，則系統可以在開啟對應時，建議相同的位置。
