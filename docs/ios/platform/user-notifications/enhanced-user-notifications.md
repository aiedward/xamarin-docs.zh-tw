...
在 iOS 10 中，Apple 引進了新的使用者通知架構，它會取代上面所述的現有 `UIApplication` 方法。
...
只要應用程式啟動，就應該要求通知許可權，方法是將下列程式碼新增至 `AppDelegate` 的 `FinishedLaunching` 方法，並設定所需的通知類型（`UNAuthorizationOptions`）：
...
> 缺少推播通知權利-您的應用程式包含適用于 Apple 推播通知服務的 API，但應用程式的簽章中遺漏了 `aps-environment` 權利。

若要提供必要的許可權，請執行下列動作：

# <a name="visual-studio-for-mactabmacos"></a>[Visual Studio for Mac](#tab/macos)

1. 按兩下**Solution Pad**中的 `Entitlements.plist` 檔案，將它開啟以供編輯。
2. 切換至 [**來源**] 視圖： 

    [![](enhanced-user-notifications-images/setup01.png "The Source view")](enhanced-user-notifications-images/setup01.png#lightbox)
3. 按一下 [ **+** ] 按鈕以新增新的金鑰。
4. 輸入**屬性**的 `aps-environment`，將**類型**保留為 `String`，然後針對**值**輸入 `development` 或 `production`： 
...
2. 按一下 [ **+** ] 按鈕以新增新的金鑰。
3. 輸入**屬性**的 `aps-environment`，將**類型**保留為 `String`，然後針對**值**輸入 `development` 或 `production`： 
...
如果應用程式將會傳送和接收遠端通知，它仍然需要使用現有的 `UIApplication` API 來_註冊權杖_。 此註冊會要求裝置必須具有即時網路連線存取 APNs，以產生將會傳送至應用程式的必要權杖。 應用程式必須將此權杖轉送至開發人員的伺服器端應用程式，以註冊遠端通知：
...
當本機通知準備就緒時，應用程式必須呼叫 `UNUserNotificationCenter` 物件的 `Add` 方法，將其顯示排程給使用者。 對於遠端通知，伺服器端應用程式會將通知承載傳送至 APNs，然後將封包傳送到使用者的裝置。
...
IOS 10 的新功能，應用程式可以在處於前景時以不同的方式處理通知，並觸發通知。 藉由提供 `UNUserNotificationCenterDelegate` 並實作為 `WillPresentNotification` 方法，應用程式可能會接管顯示通知的責任。 例如:
...
這段程式碼只會將 `UNNotification` 的內容寫出至應用程式輸出，並要求系統顯示通知的標準警示。 

如果應用程式想要在前景時顯示通知本身，而不是使用系統預設值，請將 `None` 傳遞至完成處理常式。 範例：

```csharp
completionHandler (UNNotificationPresentationOptions.None);
```

將此程式碼備妥之後，請開啟 `AppDelegate.cs` 檔案進行編輯，並變更 `FinishedLaunching` 的方法，如下所示：
...
此程式碼會將上述的自訂 `UNUserNotificationCenterDelegate` 附加到目前的 `UNUserNotificationCenter`，讓應用程式可以在使用中和前景時處理通知。

## <a name="notification-management"></a>通知管理

對於 iOS 10 的新功能，通知管理可讓您存取擱置和傳遞的通知，並新增移除、更新或升級這些通知的能力。

通知管理的一個重要部分是在建立和排程系統時指派給通知的_要求識別碼_。 對於遠端通知，這是透過 HTTP 要求標頭中的 [新增 `apps-collapse-id`] 欄位所指派。
...
建立新的 `UNNotificationAction`時，系統會為其指派唯一的識別碼，以及顯示在按鈕上的標題。 根據預設，動作將會建立為背景動作，不過可以提供選項來調整動作的行為（例如，將其設定為前景動作）。

所建立的每個動作都必須與一個類別相關聯。 建立新的 `UNNotificationCategory`時，會指派唯一的識別碼、可執行檔動作清單、意圖識別碼清單，以提供有關類別中動作意圖的詳細資訊，以及一些控制分類行為的選項。
...
針對遠端通知，請在遠端通知承載中設定符合上述其中一個類別的 `category`。 例如:
...
針對本機通知，請設定 `UNMutableNotificationContent` 物件的 `CategoryIdentifier` 屬性。 例如:
...
當使用者與上述所建立的自訂動作和類別互動時，應用程式必須完成要求的工作。 這是藉由提供 `UNUserNotificationCenterDelegate` 並執行 `UserNotificationCenter` 方法來完成。 例如:
...
傳入的 `UNNotificationResponse` 類別具有 `ActionIdentifier` 屬性，可以是預設動作或關閉動作。 使用 `response.Notification.Request.Identifier` 來測試是否有任何自訂動作。

`UserText` 屬性會保留任何使用者文字輸入的值。 `Notification` 屬性會保留原始通知，其中包含含有觸發程式和通知內容的要求。 應用程式可以決定它是以觸發程式類型為基礎的本機或遠端通知。
...
3. 選取 [ **iOS** > **擴充**功能 > **通知服務延伸**]，然後按 [**下一步]** 按鈕： 
...
> [!IMPORTANT]
> 服務延伸模組的套件組合識別碼應符合主要應用程式的套件組合識別碼，並將 `.appnameserviceextension` 附加至結尾。 例如，如果主要應用程式具有 `com.xamarin.monkeynotify`的套件組合識別碼，則服務延伸模組應具有 `com.xamarin.monkeynotify.monkeynotifyserviceextension`的套件組合識別碼。 這應該會在擴充功能新增至解決方案時自動設定。 
...
第一個方法（`DidReceiveNotificationRequest`）將透過 `request` 物件傳遞通知識別碼和通知內容。 必須呼叫傳入的 `contentHandler`，才能向使用者呈現通知。

第二個方法（`TimeWillExpire`）只會在時間即將用完以供服務延伸來處理要求之前呼叫。 如果服務延伸無法在配置的時間內呼叫 `contentHandler`，就會向使用者顯示原始內容。
...
此程式碼會從 `encrypted-content` 金鑰解密加密內容、建立新的 `UNMutableNotificationContent`、將 `Body` 屬性設定為解密的內容，並使用 `contentHandler` 向使用者呈現通知。
