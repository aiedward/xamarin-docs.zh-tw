...
description: '本檔說明「Xamarin 類型註冊機構」，這可讓 c # 類別可供目標-C 執行時間使用。'
ms.prod: xamarin
...
例如，請考慮 `Main` Xamarin iOS 應用程式中常見的 managed 方法：

```csharp
UIApplication.Main (args, null, "AppDelegate");
```

此程式碼會指示目標 C 執行時間使用稱為的類型 `AppDelegate` 做為應用程式的委派類別。 若要讓目標 C 執行時間能夠建立 c # 類別的實例，則 `AppDelegate` 必須註冊該類別。
...
從 Xamarin. iOS 8.10 開始，您可以使用 c # 語法來建立目標 C 類別。
...
所有 managed 擴充方法都必須是靜態的，但您可以使用擴充方法的標準 c # 語法來建立目標-C 實例方法：
...
這個範例會將原生 `toUpper` 實例方法新增至 `NSString` 類別。 您可以從目標-C 呼叫這個方法：
...
從 Xamarin. iOS 8.10 開始，具有屬性的介面 `[Protocol]` 將會匯出至目標-C 作為通訊協定：
...
此程式碼 `IMyProtocol` 會匯出至目標-C 作為名為的通訊協定 `MyProtocol` ，以及會執行此 `MyClass` 通訊協定的類別。
...
- 短期的缺點也是，如果使用帳戶架構，就必須使用 Clang （這是因為 Apple 的**帳戶 .h**標頭只能由 Clang 編譯）。 `--compiler:clang`如果您使用 Xcode 4.6 或更早版本，請將新增至其他 mtouch 引數以使用 Clang （Xamarin. iOS 會自動選取 Xcode 5.0 或更新版本中的 Clang）。

- 如果使用 Xcode 4.6 （或更早版本），則當匯出的類型名稱包含非 ASCII 字元時，必須選取 GCC/G + + （這是因為隨附于 Xcode 4.6 的 Clang 版本不支援在目標-C 程式碼中的識別碼內有非 ASCII 字元）。 將新增 `--compiler:gcc` 至其他 mtouch 引數，以使用 GCC。
...
- 協力廠商原生程式庫中的目標 C 類別和方法沒有（原生）靜態參考，這表示我們無法要求原生連結器移除未實際使用的協力廠商原生程式碼（因為所有專案都會被移除）。 這是 `-force_load libNative.a` 每個協力廠商系結必須執行（或屬性中的對等 `ForceLoad=true` ）的原因 `[LinkWith]` 。
...
Variadic 函數必須使用 `IsVariadic` 屬性的引數 `[Export]` ：
