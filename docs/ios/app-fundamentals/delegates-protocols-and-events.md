...
UIControlEvent C#旗標的事件與個別旗標具有一對一的對應。 當您想要讓同一段程式碼處理兩個以上的事件時，請使用 `UIControl.AddTarget` 方法：
...
Apple 在 iOS 中使用通訊協定來定義要採用的類別合約，同時從呼叫端抽象化實作為類別，因此操作方式就C#像介面一樣。 通訊協定會同時用於非委派案例（例如，與下圖所示的 `MKAnnotation` 範例），並使用委派（如本檔稍後的委派一節中所提供）。
...
如此一來，就會使用 `MKAnnotation` 通訊協定來提供注釋隨附的相關資料。 批註本身的實際視圖是從採用 `MKAnnotation` 通訊協定的物件資料建立的。 例如，當使用者按下注釋時出現的注標文字（如下列螢幕擷取畫面所示），來自于執行通訊協定的類別中的 `Title` 屬性：
...
如下一節所述，「[通訊協定深入探討](#protocols-deep-dive)」-「Xamarin」會將通訊協定系結至抽象類別。 針對 `MKAnnotation` 通訊協定，系結C#的類別會命名為 `MKAnnotation` 以模擬通訊協定的名稱，而且它是 `NSObject` 的子類別，也就是 CocoaTouch 的根基類。 通訊協定需要針對座標實作為 getter 和 setter;不過，標題和副標題是選擇性的。 因此，在 `MKAnnotation` 類別中，`Coordinate` 屬性是*抽象*的，需要實作為，而且 `Title` 和 `Subtitle` 屬性會標示為*虛擬*，使其成為選擇性，如下所示：
...
任何類別都可以藉由直接從 `MKAnnotation` 衍生來提供注釋資料，前提是至少會執行 `Coordinate` 屬性。 例如，下列範例類別會接受在此函式中的座標，並傳回標題的字串：
...
透過它所系結的通訊協定，子類別 `MKAnnotation` 的任何類別都可以提供相關資料，以供對應在建立批註的視圖時使用。 若要將批註加入至對應，只要呼叫 `MKMapView` 實例的 `AddAnnotation` 方法即可，如下列程式碼所示：
...
例如，以下是在 Xamarin 中系結 `UITableViewDataSource` 通訊協定的一部分：
...
不過，不同于目標 C 通訊協定（或C#介面）， C#類別不支援多重繼承。 這會影響使用通訊C#協定的程式碼設計，而且通常會導致嵌套的類別。 如需此問題的詳細資訊，請在本文稍後的委派一節中討論。
 ...
 `GetCell(…)` 是一種抽象方法，系結至目標-C*選取器*，`tableView:cellForRowAtIndexPath:`，這是 `UITableViewDataSource` 通訊協定的必要方法。 選取器是方法名稱的目標-C 詞彙。 為了在必要時強制執行方法，Xamarin 會將它宣告為抽象。 另一個方法（`NumberOfSections(…)`）會系結至 `numberOfSectionsInTableview:`。 這個方法在通訊協定中是選擇性的，因此 Xamarin 會將它宣告為虛擬，讓它可以選擇性C#地在中覆寫。

Xamarin 會為您處理所有 iOS 系結。 不過，如果您需要以手動方式系結通訊協定，可以藉由使用 `ExportAttribute` 裝飾類別來執行此動作。 這與 Xamarin 本身所使用的方法相同。
...
委派可讓您擴充類別的行為，而不需要建立子類別。 IOS 中的應用程式通常會在發生重要動作之後，當一個類別回呼另一個類別時，使用委派。 例如，當使用者在地圖上按下注釋時，`MKMapView` 類別會回呼其委派，讓委派類別的作者有機會在應用程式內回應。 您可以在本文稍後的範例中，逐步解說這種類型的委派使用方式，例如搭配使用委派與 Xamarin。
...
備妥委派類別之後，您只需要在控制器中具現化委派的實例，並將它指派給 `MKMapView` 的 `Delegate` 屬性，如下所示：
...
若要使用弱式委派來完成相同的工作，您必須在衍生自 `NSObject` 的任何類別中系結方法，並將它指派給 `MKMapView` 的 `WeakDelegate` 屬性。 由於 `UIViewController` 類別最終是衍生自 `NSObject` （例如 CocoaTouch 中的每個目標-C 類別），因此我們可以直接在控制器中執行系結至 `mapView:didSelectAnnotationView:` 的方法，並將控制器指派給 `MKMapView` 的 `WeakDelegate`，以避免需要額外的nested 類別。 下列程式碼示範此方法：
...
執行此程式碼時，應用程式的行為會與執行強型別委派版本時完全相同。 這段程式碼的優點是，弱式委派不需要建立在我們使用強型別委派時所建立的額外類別。 不過，這就是安全類型的代價。 如果您在傳遞給 `ExportAttribute` 的選擇器中犯了錯誤，您就不會發現執行時間。
