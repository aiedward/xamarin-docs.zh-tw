...
1. 建立新的專案。 在 Visual Studio 中，選取**檔案 > 新增 > 專案**，然後選擇 **單一檢視應用程式 (iPhone)**:
    
    ![新的專案，使用單一檢視應用程式 (iPhone) 選取視窗](launch-screens-images/launch01-vs.png)

    - 為專案的名稱，選擇的位置，然後選取**確定**。
...
3. 建立要做為啟動螢幕的分鏡腳本。 在**方案總管] 中**，以滑鼠右鍵按一下專案，然後選擇 [**新增 > 新的項目...**後面**空分鏡腳本**。 將這個分鏡腳本**LaunchScreen.storyboard**按一下**新增**:
    ...
    ![加入新項目] 視窗，請選取 [空白的分鏡腳本](launch-screens-images/launch03-vs.png)
...
5. 使其可用於啟動畫面，請將影像加入專案中的資產目錄：
    ...
    ![加入新項目 視窗，請選取的資產目錄](launch-screens-images/launch05-vs.png)
...
> [!NOTE]
> **請注意**： 做為啟動螢幕的分鏡腳本_必須_包含只有簡單、 內建的 UI 項目和**無法**執行任何計算，或衍生自的自訂類別。
