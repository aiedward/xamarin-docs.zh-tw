...
|Directory|描述|
|---|---|
|[ApplicationName].app/|**在 iOS 7 及更早版本中**這是`ApplicationBundle`儲存您的應用程式可執行檔的目錄。 您在您的應用程式中建立的目錄結構存在於此目錄 （例如，映像和其他已標示為資源在您 Mac 專案的 Visual Studio 中的檔案類型）。<br /><br />如果您需要存取您的應用程式套件組合中的內容檔案，此目錄的路徑是可透過使用`NSBundle.MainBundle.BundlePath`屬性。|
...
```
> [!IMPORTANT]
> 如果群組路徑傳回為`null`，檢查權利和佈建設定檔的組態，並確定它們是否正確。
