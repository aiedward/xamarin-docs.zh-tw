...
### <a name="mt0000-unexpected-error---please-fill-a-bug-report-at-httpbugzillaxamarincom"></a>MT0000： 未預期的錯誤-請填寫 錯誤報告在 http://bugzilla.xamarin.com
...
### <a name="mt0001--devname-was-provided-without-any-device-specific-action"></a>MT0001: '-devname' 並無任何裝置的特定動作提供
...
### <a name="mt0004-new-refcounting-logic-requires-sgen-to-be-enabled-too"></a>MT0004： 新 refcounting 邏輯需要 SGen 太啟用。
...
### <a name="mt0013-profiling-support-requires-sgen-to-be-enabled-too"></a>MT0013： 設定檔作業支援需要 sgen 太啟用。
...
### <a name="mt0019-only-one---loginstallkilllaunchdev-or---launchdebugsim-option-can-be-used"></a>MT0019: 只有一個-[記錄檔 | 安裝 | kill | 啟動] 開發人員或--[啟動 | 偵錯] sim 選項才能使用。
...
### <a name="mt0020-the-valid-options-for--are-"></a>MT0020： 的有效選項 '\*'是'\*'。

<a name="MT0021" />

### <a name="mt0021-cannot-compile-using-gccg---use-gcc-when-using-the-static-registrar-this-is-the-default-when-compiling-for-device-either-remove-the---use-gcc-flag-or-use-the-dynamic-registrar---registrardynamic"></a>MT0021： 無法編譯使用 gcc / g + + (-使用 gcc) 時使用靜態的註冊機構 （的裝置進行編譯時，這是預設值）。 請移除--使用 gcc 旗標，或使用動態註冊機構 (-註冊機構： 動態)。

<a name="MT0022" />

### <a name="mt0022-the-options---unsupported--enable-generics-in-registrar-and---registrar-are-not-compatible"></a>MT0022: 選項 '-不支援-啟用-泛型-中的註冊機構' 和 '-註冊機構 ' 不相容。
...
### <a name="mt0025-no-sdk-version-was-provided-please-add---sdkxy-to-specify-which-ios-sdk-should-be-used-to-build-your-application"></a>MT0025： 未不提供任何 SDK 版本。 請新增`--sdk=X.Y`來指定哪些 iOS SDK 應該用來建置應用程式。
...
### <a name="mt0029-repl---enable-repl-is-only-supported-in-the-simulator---sim"></a>MT0029: REPL (-啟用 repl) 僅適用於在模擬器 (-sim)。

如果您要建置適用於模擬器僅支援複寫。 這表示，如果您傳遞`--enable-repl`至 mtouch，您還必須傳遞`--sim`。

<a name="MT0030" />

### <a name="mt0030-the-executable-name--and-the-app-name--are-different-this-may-prevent-crash-logs-from-getting-symbolicated-properly"></a>MT0030： 檔名 (\*) 和應用程式名稱 (\*) 都不同，這可能導致損毀記錄檔無法取得正確 symbolicated。
...
### <a name="mt0033-a-xamarinios-project-must-reference-either-monotouchdll-or-xamariniosdll"></a>MT0033: Xamarin.iOS 專案必須參考 monotouch.dll 或 Xamarin.iOS.dll
...
### <a name="mt0037-monotouchdll-is-not-64-bit-compatible-either-reference-xamariniosdll-or-do-not-build-for-a-64-bit-architecture-arm64-andor-x8664"></a>MT0037: monotouch.dll 與 64 位元不相容。 請參考 Xamarin.iOS.dll，或針對 64 位元的架構 （ARM64 和/或 x86_64） 建立。

<a name="MT0038" />

### <a name="mt0038-the-old-registrars---registraroldstaticolddynamic-are-not-supported-when-referencing-xamariniosdll"></a>MT0038: 舊註冊機構 (-註冊機構： oldstatic | olddynamic) 不支援參考 Xamarin.iOS.dll 時。

<a name="MT0039" />

### <a name="mt0039-applications-targeting-armv6-cannot-reference-xamariniosdll"></a>無法為目標 ARMv6 應用程式參考 Xamarin.iOS.dll MT0039:。
...
### <a name="mt0042-no-reference-to-either-monotouchdll-or-xamariniosdll-was-found-a-reference-to-monotouchdll-will-be-added"></a>MT0042: Monotouch.dll 或 Xamarin.iOS.dll 沒有參考找不到。 將加入 monotouch.dll 的參考。
...
### <a name="mt0044---listsim-is-only-supported-with-xcode-60-or-later"></a>MT0044: Xcode 6.0 或更新版本才支援-listsim。

安裝 Xcode 較新版本。

<a name="MT0045" />

### <a name="mt0045---extension-is-only-supported-when-using-the-ios-80-or-later-sdk"></a>MT0045:-擴充功能，才支援使用 iOS 8.0 （含） 以後的 SDK。
...
### <a name="mt0052-no-command-specified"></a>MT0052： 未指定的命令。
...
### <a name="mt0056-cannot-find-xcode-in-the-default-location-applicationsxcodeapp-please-install-xcode-or-pass-a-custom-path-using---sdkroot-path"></a>MT0056： 在預設位置找不到 Xcode (/ Applications/Xcode.app)。 請安裝 Xcode，或將自訂路徑使用-sdkroot 傳遞<path>。
...
### <a name="mt0064-xamarinios-only-supports-embedded-frameworks-with-unified-projects"></a>MT0064: Xamarin.iOS 只支援內嵌的架構來使用整合專案。
...
### <a name="mt0076-watchos-projects-must-be-extensions"></a>MT0076: WatchOS 專案必須是擴充功能。
...
### <a name="mt0080-disabling-newrefcount---new-refcountfalse-is-deprecated"></a>MT0080： 停用 NewRefCount、--新-refcount:false，已被取代。
...
### <a name="mt0081-the-command-line-argument---download-crash-report-also-requires---download-crash-report-to"></a>MT0081： 命令列引數--下載當機報告也需要-下載損毀-報表-對。
...
### <a name="mt0083-asm-only-bitcode-is-not-supported-on-watchos-use-either---bitcodemarker-or---bitcodefull"></a>MT0083: WatchOS 不支援僅限 Asm bitcode。 使用任一-bitcode： 標記或-bitcode： 完整。

<a name="MT0084" />

### <a name="mt0084-bitcode-is-not-supported-in-the-simulator-do-not-pass---bitcode-when-building-for-the-simulator"></a>MT0084: Bitcode 不支援在模擬器中。 建置適用於模擬器時，請勿傳送-bitcode。
...
### <a name="mt0086-a-target-framework---target-framework-must-be-specified-when-building-for-tvos-or-watchos"></a>MT0086: 的目標 framework (-目標架構) 為 TVOS 或 WatchOS 建置時，必須指定。
...
### <a name="mt0088-the-gc-must-be-in-cooperative-mode-for-watchos-apps-please-remove-the---coopfalse-argument-to-mtouch"></a>MT0088： 合作式模式 watchOS 應用程式必須是 GC。 請移除 mtouch--coop: false 引數。

<a name="MT0089" />

### <a name="mt0089-the-option--cannot-take-the-value--when-cooperative-mode-is-enabled-for-the-gc"></a>MT0089： 選項 '\*'無法採用的值'\*' GC 啟用合作式模式時。
...
### <a name="mt0095-aot-files-could-not-be-copied-to-the-destination-directory-dest-error"></a>MT0095: Aot 檔案無法複製到目的地目錄 {d}: {error}

<a name="MT0096" />

### <a name="mt0096-no-reference-to-xamariniosdll-was-found"></a>MT0096: Xamarin.iOS.dll 沒有參考找不到。
...
### <a name="mt0103-the-static-object--contains-more-than-one-assembly--but-each-static-object-must-correspond-with-exactly-one-assembly"></a>MT0103： 靜態物件 '\*' 包含多個組件 ('\*')，但每個靜態物件必須對應於一個組件。
...
### <a name="mt0107-the-assemblies--have-different-custom-llvm-optimizations--which-is-not-allowed-when-they-are-all-compiled-to-a-single-binary"></a>MT0107： 組件\*' 有不同的自訂 LLVM 最佳化 (\*)，這不允許在所有編譯時以單一的二進位檔。
...
### <a name="mt0110-incremental-builds-have-been-disabled-because-this-version-of-xamarinios-does-not-support-incremental-builds-in-projects-that-include-third-party-binding-libraries-and-that-compiles-to-bitcode"></a>MT0110： 累加建置已停用，因為在專案中，包含第三方繫結的程式庫和可編譯成 bitcode Xamarin.iOS 這個版本不支援累加建置。
...
### <a name="mt0111-bitcode-has-been-enabled-because-this-version-of-xamarinios-does-not-support-building-watchos-projects-using-llvm-without-enabling-bitcode"></a>MT0111: Bitcode 已啟用因為 Xamarin.iOS 這個版本不支援建置 watchOS 專案使用 LLVM 不啟用 bitcode。
...
### <a name="mt0115-it-is-recommended-to-reference-dynamic-symbols-using-code---dynamic-symbol-modecode-when-bitcode-is-enabled"></a>MT0115： 建議您參考動態使用程式碼的符號 (-動態符號模式 = 程式碼) 時，bitcode 已啟用。
...
### <a name="mt0116-invalid-architecture-arch-32-bit-architectures-are-not-supported-when-deployment-target-is-11-or-later-make-sure-the-project-does-not-build-for-a-32-bit-architecture"></a>MT0116： 無效的架構: {arch}。 當部署目標是 11 或更新版本不支援 32 位元架構。 請確定專案未建置的 32 位元架構。

iOS 11 不包含支援 32 位元應用程式，所以它不支援部署目標是 iOS 11 或更新版本時，建置為 32 位元應用程式。

變更專案的 iOS 組建選項中的目標架構 arm64，或在專案的 Info.plist 中部署目標變更為較早的 iOS 版本。
...
### <a name="mt0118-aot-files-could-not-be-found-at-the-expected-directory-msymdir"></a>MT0118： 在 '{msymdir}' 的預期目錄找不到 Aot 檔案。
...
這通常發生在使用任何項目從平台組件; 可執行檔專案中沒有程式碼執行個體的空白 Main 方法 （和其他程式碼） 會顯示這個錯誤：
...
### <a name="mt0124-could-not-set-the-current-language-to-lang-according-to-langlang-exception"></a>MT0124： 無法將目前的語言設定為 '{lang}' （根據 LANG = {LANG}）: {例外狀況。

這是警告，表示目前的語言無法設定中的錯誤訊息的語言。

目前的語言會預設為系統語言。

<a name="MT0125" />

### <a name="mt0125-the---assembly-build-target-command-line-argument-is-ignored-in-the-simulator"></a>MT0125:--組件的建置目標在模擬器中的命令列引數會被忽略。
...
### <a name="mt0127-incremental-builds-have-been-disabled-because-this-version-of-xamarinios-does-not-support-incremental-builds-in-projects-that-include-more-than-one-third-party-binding-libraries"></a>MT0127： 累加建置已停用，因為這個版本的 Xamarin.iOS 不支援累加建置包含多個的其中一個協力廠商的繫結的程式庫的專案中。
...
最有可能可以忽略此警告。發生任何問題提報 bug (https://bugzilla.xamarin.com] (https://bugzilla.xamarin.com/enter_bug.cgi?product=iOS)) ，將會加以調查。
...
### <a name="mt1001-could-not-find-an-application-at-the-specified-directory"></a>在指定的目錄中 MT1001： 找不到應用程式
...
### <a name="mt1004-could-not-get-the-list-of-installed-applications"></a>MT1004： 無法取得已安裝的應用程式的清單。
...
### <a name="mt1013-dependency-tracking-error-no-files-to-compare-please-file-a-bug-report-at-httpbugzillaxamarincom-with-a-test-case"></a>MT1013： 相依性追蹤時發生錯誤： 沒有要比較的檔案。 請檔案錯誤報告在http://bugzilla.xamarin.com與測試案例。
...
### <a name="mt1021-could-not-copy-the-file--to--2"></a>MT1021： 無法將檔案複製 '\*'to'\*': {2}

無法複製檔案。 從複製作業的錯誤訊息有錯誤的詳細資訊。

<a name="MT1022" />

### <a name="mt1022-could-not-copy-the-directory--to--2"></a>MT1022： 無法複製目錄 '\*'to'\*': {2}
...
### <a name="mt1030-launching-applications-on-device-using-a-bundle-identifier-is-deprecated-please-pass-the-full-path-to-the-bundle-to-launch"></a>MT1030： 啟動應用程式使用的套件組合識別碼的裝置上已被取代。 請將完整路徑傳遞至要啟動的組合。

建議您將路徑傳遞至應用程式，而不是套件組合識別碼的裝置上啟動。

<a name="MT1031" />

### <a name="mt1031-could-not-launch-the-app--on-the-device--because-the-device-is-locked-please-unlock-the-device-and-try-again"></a>MT1031： 無法啟動應用程式 '\*'上的裝置 」\*' 因為裝置已被鎖定。 請解除鎖定裝置，然後再試一次。
...
### <a name="mt1036-framework-name-included-from-path-related-to-previous-error"></a>MT1036: 包含從 Framework '{name}': {path} （與之前錯誤相關）
...
### <a name="mt1103-could-not-detach"></a>MT1103： 無法中斷連接
...
### <a name="mt1106-could-not-get-list-of-applications-on-the-device-request-timed-out"></a>MT1106： 無法取得裝置上的應用程式清單： 要求已逾時。
...
### <a name="mt1110-application-failed-to-launch-because-of-ios-security-restrictions-please-ensure-the-developer-is-trusted"></a>MT1110： 應用程式無法啟動，因為 iOS 安全性限制。 請確定開發人員是受信任。

如果您要部署企業應用程式，或使用可用的佈建設定檔，您可能會對信任的開發人員 (說明<a href="http://stackoverflow.com/a/30726375/183422">這裡</a>)。

<a name="MT1111" />

### <a name="mt1111-application-launched-successfully-but-its-not-possible-to-wait-for-the-app-to-exit-as-requested-because-its-not-possible-to-detect-app-termination-when-launching-using-gdbserver"></a>MT1111： 應用程式啟動成功，但不可能在等候結束，因為無法偵測應用程式終止時啟動使用 gdbserver 要求應用程式。
...
### <a name="mt1210-invalid-simulator-specification--unknown-key-"></a>MT1210： 無效的模擬器規格： \*，未知的機碼 '\*'
...
### <a name="mt1301-native-library---was-ignored-since-it-does-not-match-the-current-build-architectures-"></a>MT1301： 原生程式庫`*`(\*) 已略過，因為它不符合目前組建 architecture(s) (\*)
...
### <a name="mt1303-could-not-decompress-the-native-framework--from--please-review-the-build-log-for-more-information-from-the-native-zip-command"></a>MT1303： 無法解壓縮原生架構 '\*'from'\*'。 請檢閱組建記錄檔，如需詳細資訊，請從原生 'zip' 命令。
...
### <a name="mt2001-could-not-link-assemblies"></a>MT2001： 無法連結的組件
...
### <a name="mt2015-invalid-httpmessagehandler--for-watchos-the-only-valid-value-is-nsurlsessionhandler"></a>MT2015： 無效的 HttpMessageHandler `*` watchOS 的。 唯一有效的值是 NSUrlSessionHandler。
...
### <a name="mt2030-remove-user-resources-failed-processing-"></a>MT2030： 移除使用者資源無法處理`...`。
...
### <a name="mt2070-metadata-reducer-failed-processing-"></a>MT2070： 無法處理之中繼資料減壓器`...`。
...
### <a name="mt2090-inliner-failed-processing-"></a>MT2090： 內嵌者的失敗處理`...`。

發生無法預期發生時，對內嵌程式碼嘗試從應用程式。 錯誤訊息中名為造成問題的組件。 若要修正此問題的組件將需要在中提供[bug 報表](https://bugzilla.xamarin.com)以及完成的組建記錄檔，具有已啟用的詳細資訊 (也就是`-v -v -v -v`中**其他 mtouch 引數**)。
...
### <a name="mt2100-smart-enum-conversion-preserver-failed-processing-"></a>MT2100： 智慧列舉轉換守護者無法處理`...`。

當嘗試將轉換方法，從應用程式的智慧列舉標記時發生發生無法預期。 錯誤訊息中名為造成問題的組件。 若要修正此問題的組件將需要在中提供[bug 報表](https://bugzilla.xamarin.com)以及完成的組建記錄檔，具有已啟用的詳細資訊 (也就是`-v -v -v -v`中**其他 mtouch 引數**)。
...
錯誤訊息中名為造成問題的組件。 若要修正此問題的組件將需要在中提供[bug 報表](https://bugzilla.xamarin.com)以及完成的組建記錄檔，具有已啟用的詳細資訊 (也就是`-v -v -v -v`中**其他 mtouch 引數**)。
...
### <a name="mt4002-failed-to-compile-the-generated-code-for-pinvoke-methods-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4002： 無法編譯 P/Invoke 方法產生的程式碼。 請檔案在錯誤報告 http://bugzilla.xamarin.com
...
### <a name="mt4102-the-registrar-found-an-invalid-type--in-signature-for-method--use--instead"></a>MT4102: 註冊機構找到無效的型別`*`中方法的簽章`*`。 請改用 `*`。

這目前只會發生一種類型： System.DateTime。 請改用 OBJECTIVE-C 對等項目 (NSDate)。

<a name="MT4103" />

### <a name="mt4103-the-registrar-found-an-invalid-type--in-signature-for-method--the-type-implements-inativeobject-but-does-not-have-a-constructor-that-takes-two-intptr-bool-arguments"></a>MT4103: 註冊機構找到無效的型別`*`中方法的簽章`*`： 型別實作 INativeObject，但是並沒有會採用兩個建構函式 (IntPtr，bool) 引數
...
### <a name="mt4104-the-registrar-cannot-marshal-the-return-value-for-type--in-signature-for-method-"></a>MT4104: 註冊機構無法封送處理類型的傳回值`*`中方法的簽章`*`。
...
### <a name="mt4105-the-registrar-cannot-marshal-the-parameter-of-type--in-signature-for-method-"></a>MT4105: 註冊機構無法封送處理類型的參數`*`中方法的簽章`*`。

如果您認為 Xamarin.iOS 應該支援的型別有問題，請檔案在的增強功能要求[ http://bugzilla.xamarin.com ](http://bugzilla.xamarin.com)。

<a name="MT4106" />

### <a name="mt4106-the-registrar-cannot-marshal-the-return-value-for-structure--in-signature-for-method-"></a>MT4106: 註冊機構無法封送處理結構的傳回值`*`中方法的簽章`*`。
...
### <a name="mt4107-the-registrar-cannot-marshal-the-parameter-of-type--in-signature-for-method-"></a>MT4107: 註冊機構無法封送處理類型的參數`*`中方法的簽章`+`。
...
### <a name="mt4109-failed-to-compile-the-generated-registrar-code-please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4109： 無法編譯產生的註冊機構的程式碼。 請檔案在錯誤報告 http://bugzilla.xamarin.com
...
### <a name="mt4125-the-registrar-found-an-invalid-type--in-signature-for-method--the-interface-must-have-a-protocol-attribute-specifying-its-wrapper-type"></a>MT4125: 註冊機構找到無效的型別 '\*'中方法的簽章'\*': 介面必須要有通訊協定屬性，指定其包裝函式型別。
...
### <a name="mt4127-cannot-register-more-than-one-interface-method-for-the-method--which-is-implementing-"></a>MT4127： 無法註冊之方法的多個介面方法 '\*' (這實作 '\*')。

<a name="MT4128" />

### <a name="mt4128-the-registrar-found-an-invalid-generic-parameter-type--in-the-method--the-generic-parameter-must-have-an-nsobject-constraint"></a>MT4128: 註冊機構找到無效的泛型參數型別 '\*'中方法'\*'。 泛型參數必須要有 'NSObject' 條件約束。

<a name="MT4129" />

### <a name="mt4129-the-registrar-found-an-invalid-generic-return-type--in-the-method--the-generic-return-type-must-have-an-nsobject-constraint"></a>MT4129: 註冊機構找到無效的泛型傳回類型 '\*'中方法'\*'。 泛型傳回類型必須有 'NSObject' 條件約束。
...
### <a name="mt4131-the-registrar-cannot-export-static-properties-in-generic-classes-"></a>MT4131: 註冊機構無法匯出泛型類別中的靜態屬性 ('\*。\*')。

<a name="MT4132" />

### <a name="mt4132-the-registrar-found-an-invalid-generic-return-type--in-the-property--the-return-type-must-have-an-nsobject-constraint"></a>MT4132: 註冊機構找到無效的泛型傳回類型 '\*'中屬性'\*'。 傳回的類型必須有 'NSObject' 條件約束。
...
### <a name="mt4146-the-name-parameter-of-the-registrar-attribute-on-the-class---contains-an-invalid-character--"></a>MT4146: Name 參數的註冊機構上的屬性類別\*'('\*') 包含無效的字元:'\*' (\*)。
...
### <a name="mt4147-detected-a-protocol-inheriting-from-the-jsexport-protocol-while-using-the-dynamic-registrar-it-is-not-possible-to-export-protocols-to-javascriptcore-dynamically-the-static-registrar-must-be-used-add---registrarstatic-to-the-additional-mtouch-arguments-in-the-projects-ios-build-options-to-select-the-static-registrar"></a>MT4147： 偵測到使用動態註冊機構時繼承 JSExport 通訊協定的通訊協定。 不可能將通訊協定動態; 匯出至 JavaScriptCore必須使用靜態的註冊機構 (加入 '-註冊機構： 靜態中專案的 iOS 組建選項，以選取靜態註冊機構的其他 mtouch 引數)。
...
### <a name="mt4149-cannot-register-the-method--because-the-type-of-the-first-parameter--does-not-match-the-category-type-"></a>MT4149： 無法登錄的方法 '\*。\*' 因為第一個參數的型別 ('\*') 不符合類別類型 ('\*')。
...
### <a name="mt4154-cannot-register-the-method--as-a-category-method-because-its-generic"></a>MT4154： 無法登錄的方法 '\*' 做為類別方法因為它是泛型。
...
### <a name="mt4157-cannot-register-the-category-method--because-at-least-one-parameter-is-required-and-its-type-must-match-the-category-type-"></a>MT4157： 無法註冊類別方法 '\*' 需要至少一個參數，因此 (和其類型必須符合類別類型 '\*')
...
### <a name="mt4160-invalid-character---found-in-selector--for-"></a>MT4160： 無效的字元 '\*' (\*) 位於選取器 '\*'for'\*'。
...
### <a name="mt4165-the-registrar-couldnt-find-the-type-systemvoid-in-any-of-the-referenced-assemblies"></a>MT4165: 註冊機構找不到類型 'System.Void' 在任何參考的組件中。

這項錯誤最可能指出 Xamarin.iOS 中的 bug。 請檔案錯誤報告在[ http://bugzilla.xamarin.com ](https://bugzilla.xamarin.com/enter_bug.cgi?product=iOS)。

<a name="MT4166" />

### <a name="mt4166-cannot-register-the-method--because-the-signature-contains-a-type--that-isnt-a-reference-type"></a>MT4166： 無法登錄的方法 '\*' 因為簽章包含的類型 (\*)，這並不是參考型別。
...
### <a name="mt4168-cannot-register-the-type-managedname-because-its-objective-c-name-exportedname-is-an-objective-c-keyword-please-use-a-different-name"></a>MT4168： 無法註冊類型 '{管理\_name}' 因為其 Objective C 的名稱' {匯出\_名稱}' 是 Objective C 關鍵字。 請使用不同的名稱。
...
### <a name="mt4170-the-registrar-cant-convert-from-managed-type-to-native-type-for-the-return-value-in-the-method-method"></a>MT4170: 註冊機構無法從轉換 '{managed 型別}' '{原生型別}' 中方法 {方法} 傳回值。

請參閱錯誤的描述<a href="#MT4172">MT4172</a>。

<a name="MT4171" />

### <a name="mt4171-the-bindas-attribute-on-the-member-member-is-invalid-the-bindas-type-type-is-different-from-the-property-type-type"></a>MT4171: BindAs 上的屬性成員 {member} 是無效： BindAs 類型 {type} 為 {type} 屬性型別不同。

請確定 BindAs 屬性中的型別符合它附加到成員的型別。

<a name="MT4172" />

### <a name="mt4172-the-registrar-cant-convert-from-native-type-to-managed-type-for-the-parameter-parameter-name-in-the-method-method"></a>MT4172: 註冊機構無法從轉換 '{原生類型}' 到 '{managed type}' 中方法 {方法} 的參數 '{parameter name}'。

在註冊機構不支援所述的類型之間轉換。

如果有問題的 API Xamarin.iOS; 所提供，這會是 Xamarin.iOS 中的 bug請將 bug 歸檔在[ http://bugzilla.xamarin.com ] [ 1]。

如果您遇到此開發原生程式庫的繫結專案時，我們開啟新增新類型組合的支援。 如果這種情況，請檔案的增強功能要求 ([http://bugzilla.xamarin.com][2]) 與測試案例，我們會評估它。
...
### <a name="mt5202-native-linking-failed-please-review-the-build-log"></a>MT5202： 原生連結失敗。 請檢閱組建記錄檔。
...
### <a name="mt5211-native-linking-failed-undefined-objective-c-class--the-symbol--could-not-be-found-in-any-of-the-libraries-or-frameworks-linked-with-your-application"></a>MT5211： 原生連結失敗，未定義的 Objective C 類別： \*。 符號 '\*' 找不到的任何程式庫或連結到您的應用程式的架構。
...
### <a name="mt5218-cant-ignore-the-dynamic-symbol-symbol---ignore-dynamic-symbolsymbol-because-it-was-not-detected-as-a-dynamic-symbol"></a>MT5218： 無法略過動態符號 {symbol} (-忽略動態符號 = {符號}) 因為它未偵測到做為動態的符號。
...
### <a name="mt5303-failed-to-generate-the-debug-symbols-dsym-directory-please-review-the-build-log"></a>MT5303： 無法產生偵錯符號 （dSYM 目錄）。 請檢閱組建記錄檔。

若要建立偵錯符號的最終.app 目錄上執行 dsymutil 時，就會發生錯誤。 請檢閱組建記錄檔以查看 dsymutil 的輸出，以及查看如何修正它。

<a name="MT5304" />

### <a name="mt5304-failed-to-strip-the-final-binary-please-review-the-build-log"></a>MT5304： 無法刪除最終的二進位檔。 請檢閱組建記錄檔。
...
### <a name="mt5306-failed-to-create-the-a-fat-library-please-review-the-build-log"></a>MT5306： 無法建立 fat 文件庫。 請檢閱組建記錄檔。

執行 'lipo' 工具時，就會發生錯誤。 請檢閱組建記錄檔以查看 'lipo' 回報的錯誤。

<a name="MT5307" />

### <a name="mt5307-failed-to-sign-the-executable-please-review-the-build-log"></a>MT5307： 無法註冊可執行檔。 請檢閱組建記錄檔。
...
### <a name="mt6001-running-version-of-cecil-doesnt-support-assembly-stripping"></a>MT6001: Cecil 執行版本不支援移除組件

<a name="MT6002" />

### <a name="mt6002-could-not-strip-assembly-"></a>MT6002： 可能不會刪除組件`*`。
...
### <a name="mt7007-the-app-extension--has-a-cfbundleidentifier--that-ends-with-the-illegal-suffix-key"></a>MT7007: 應用程式擴充功能 '\*' 具有 CFBundleIdentifier (\*) 的不合法的後置詞".key 」 以結束。
...
### <a name="mt7027-the-watch-extension--has-a-cfbundleidentifier--that-ends-with-the-illegal-suffix-key"></a>MT7027: Watch 擴充功能 '\*' 具有 CFBundleIdentifier (\*) 的不合法的後置詞".key 」 以結束。
...
### <a name="mt7035-the-watch-app--does-not-have-a-valid-uidevicefamily-value-expected--but-found--"></a>MT7035： 監看式應用程式 '\*' 沒有有效的 UIDeviceFamily 值。 必須是 '\*'但卻發現'\* (\*)'。
...
### <a name="mt7037-the-watchkit-extension-extensionname-has-an-invalid-wkappbundleidentifier-value--it-does-not-match-the-watch-apps-cfbundleidentifier-"></a>MT7037: WatchKit 擴充功能 '{extensionName}' 有無效的 WKAppBundleIdentifier 值 ('\*')，它不符合監看式應用程式的 CFBundleIdentifier ('\*')。
...
### <a name="mt7058-merge-specified-entry-must-be-a-container"></a>MT7058: 合併: 指定項目必須是容器。
...
### <a name="mt8001-version-mismatch-between-the-native-xamarinios-runtime-and-monotouchdll-please-reinstall-xamarinios"></a>原生 Xamarin.iOS 執行階段之間 monotouch.dll MT8001： 版本不相符。 請重新安裝 Xamarin.iOS。

<a name="MT8002" />

### <a name="mt8002-could-not-find-the-method--in-the-type-"></a>MT8002： 找不到方法 '\*'in '的 type\*'。

<a name="MT8003" />

### <a name="mt8003-failed-to-find-the-closed-generic-method--on-the-type-"></a>MT8003： 無法找到封閉式泛型方法 '\*'on '的 type\*'。
...
### <a name="mt8006-failed-to-find-the-selector--on-the-type-"></a>MT8006: 若要尋找的選取器無法 '\*'on '的 type\*'

<a name="MT8007" />

### <a name="mt8007-cannot-get-the-method-descriptor-for-the-selector--on-the-type--because-the-selector-does-not-correspond-to-a-method"></a>MT8007： 無法取得選取器的方法描述元 '\*'on '的 type\*'，因為選取器沒有對應至方法
...
### <a name="mt8012-invalid-delegateproxyattribute-for-the-return-value-for-the-method--delegatetype-is-null-please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8012： 方法的傳回值的無效 DelegateProxyAttribute *。*： 其實是 null。 請將 bug 歸檔在http://bugzilla.xamarin.com。
...
### <a name="mt8013-invalid-delegateproxyattribute-for-the-return-value-for-the-method--delegatetype-2-specifies-a-type-without-a-handler-field-please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8013： 方法的傳回值的無效 DelegateProxyAttribute *。*： 其實 ({2}) 指定沒有 'Handler' 欄位的類型。 請將 bug 歸檔在http://bugzilla.xamarin.com。
