...
### <a name="mt0000-unexpected-error---please-fill-a-bug-report-at-httpbugzillaxamarincom"></a>MT0000： 未預期的錯誤-請填寫在錯誤報告 http://bugzilla.xamarin.com

發生未預期的錯誤狀況。 請[提出 bug 報告](https://bugzilla.xamarin.com/enter_bug.cgi?product=iOS)盡可能最多資訊，包括：
...
### <a name="mt0004-new-refcounting-logic-requires-sgen-to-be-enabled-too"></a>MT0004： 新 refcounting 邏輯需要太啟用 SGen。
...
### <a name="mt0013-profiling-support-requires-sgen-to-be-enabled-too"></a>MT0013： 分析支援需要太啟用 sgen。
...
### <a name="mt0052-no-command-specified"></a>MT0052： 沒有指定的命令。
...
### <a name="mt0057-cannot-determine-the-path-to-xcodeapp-from-the-sdk-root--please-specify-the-full-path-to-the-xcodeapp-bundle"></a>MT0057： 無法判定要 Xcode.app 路徑從 sdk 根目錄 ' *'。 請指定 Xcode.app 套件組合的完整路徑。
...
### <a name="mt0063-cannot-find-the-executable-in-the-extension--no-cfbundleexecutable-entry-in-its-infoplist"></a>MT0063： 延伸模組中找不到可執行檔 * （在其 Info.plist 中的沒有 CFBundleExecutable 項目）
...
### <a name="mt0068-invalid-value-for-target-framework-"></a>MT0068： 目標 framework 無效的值: *。
...
### <a name="mt0075-no-architecture-specified-using-the---abi-argument-an-architecture-is-required-for--projects"></a>MT0075： 未指定 （使用-abi 引數） 的架構。 架構是為了 * 專案。
...
### <a name="mt0077-incremental-builds-are-enabled-with-a-deployment-target--80-currently--this-is-not-supported-the-resulting-application-will-not-launch-on-ios-9-so-the-deployment-target-will-be-set-to-80"></a>MT0077： 累加建置會啟用與部署目標 < 8.0 (目前 *)。 這不支援 （產生的應用程式將不會啟動在 iOS 9），因此部署目標將設為 8.0。
...
### <a name="mt0078-the-recommended-xcode-version-for-xamarinios--is-xcode--or-later-the-current-xcode-version-found-in--is-"></a>MT0078： 建議使用的 Xcode 版本適用於 Xamarin.iOS * 是 Xcode * 或更新版本。 目前的 Xcode 版本 (位於 *) 是 *。
...
### <a name="mt0086-a-target-framework---target-framework-must-be-specified-when-building-for-tvos-or-watchos"></a>MT0086: 的目標 framework (-目標架構) 建置 TVOS 或 WatchOS 時必須指定。
...
### <a name="mt0105-no-assembly-build-target-was-specified-for-"></a>MT0105: 指定沒有組件建置 」 目標 ' *'。
...
### <a name="mt0110-incremental-builds-have-been-disabled-because-this-version-of-xamarinios-does-not-support-incremental-builds-in-projects-that-include-third-party-binding-libraries-and-that-compiles-to-bitcode"></a>MT0110： 累加建置已停用，因為此版本的 Xamarin.iOS 不支援累加建置在專案中，包含第三方繫結程式庫，且會編譯成 bitcode。
...
### <a name="mt0111-bitcode-has-been-enabled-because-this-version-of-xamarinios-does-not-support-building-watchos-projects-using-llvm-without-enabling-bitcode"></a>MT0111: Bitcode 具有已啟用，因為此版本的 Xamarin.iOS 不支援建置 watchOS 專案而不啟用 bitcode 使用 LLVM。
...
### <a name="mt0118-aot-files-could-not-be-found-at-the-expected-directory-msymdir"></a>MT0118： 在預期的目錄 '{msymdir}' 找不到 Aot 檔案。
...
### <a name="mt0124-could-not-set-the-current-language-to-lang-according-to-langlang-exception"></a>MT0124： 無法將目前的語言設定為 '{lang}' （根據 l a n G = {LANG}）: {exception}
...
### <a name="mt0125-the---assembly-build-target-command-line-argument-is-ignored-in-the-simulator"></a>MT0125:--組件-建置目標命令列引數會忽略在模擬器中。
...
### <a name="mt0127-incremental-builds-have-been-disabled-because-this-version-of-xamarinios-does-not-support-incremental-builds-in-projects-that-include-more-than-one-third-party-binding-libraries"></a>MT0127： 累加建置已停用，因為此版本的 Xamarin.iOS 不支援累加建置包含多個協力廠商的繫結程式庫的專案中。
...
### <a name="mt1012-failed-to-list-the-apps-on-the-device--"></a>MT1012: 列出在裝置上的應用程式無法 ' *': *
...
### <a name="mt1014-failed-to-re-use-cached-version-of--"></a>MT1014： 無法重複使用快取的版本的 ' *': *。
...
### <a name="mt1023-could-not-communicate-with-the-device-to-find-the-application---"></a>MT1023: 無法與找到的應用程式的裝置通訊 ' *': *
...
### <a name="mt1024-the-application-signature-could-not-be-verified-on-device--please-make-sure-that-the-provisioning-profile-is-installed-and-not-expired-error-0xe8008017"></a>MT1024： 應用程式簽章無法確認裝置上 ' *'。 請確定佈建設定檔已安裝且過期 (錯誤： 0xe8008017)。
...
### <a name="mt1029-could-not-find-an-application-at-the-specified-directory-"></a>MT1029： 在指定的目錄找不到應用程式: *
...
### <a name="mt1032-this-application-executable-might-be-too-large--mb-to-execute-on-device-if-bitcode-was-enabled-you-might-want-to-disable-it-for-development-it-is-only-required-to-submit-applications-to-apple"></a>MT1032： 這個應用程式可執行檔可能會太大 (* MB) 裝置上執行。 如果已啟用 bitcode 要停用它進行開發，只需要提交給 Apple 的應用程式。
...
### <a name="mt11xx-debug-service"></a>MT11xx： 服務進行偵錯
...
### <a name="mt1104-failed-to-send-packet-"></a>MT1104: 無法傳送的封包: *
...
### <a name="mt1106-could-not-get-list-of-applications-on-the-device-request-timed-out"></a>MT1106： 無法在裝置上取得的應用程式清單： 要求已逾時。
...
如果您要部署企業應用程式，或使用免費的佈建設定檔，您可能需要信任的開發人員 (這會說明<a href="http://stackoverflow.com/a/30726375/183422">此處</a>)。
...
### <a name="mt1111-application-launched-successfully-but-its-not-possible-to-wait-for-the-app-to-exit-as-requested-because-its-not-possible-to-detect-app-termination-when-launching-using-gdbserver"></a>MT1111： 應用程式啟動成功，但它不可以等候結束要求，因為它不能啟動使用 gdbserver 偵測應用程式終止的應用程式。
...
### <a name="mt1213-invalid-simulator-specification-for-xcode-4-"></a>MT1213： Xcode 4 無效的模擬器規格: *
...
### <a name="mt1215-invalid-sdk-specified-"></a>MT1215： 指定無效的 SDK: *
...
### <a name="mt1218-no-bundle-identifier-found-in-the-app-at-"></a>MT1218： 在應用程式中找到沒有套件組合識別碼 ' *'。
...
### <a name="mt1603-unknown-format-for-fat-entry-at-position--in-"></a>MT1603： 未知的格式為 fat 的項目位置 * 的 *。
...
### <a name="mt2001-could-not-link-assemblies"></a>MT2001： 無法將連結的組件
...
### <a name="mt3007-debug-info-files-mdb-will-not-be-loaded-when-llvm-is-enabled"></a>MT3007： 偵錯啟用 llvm 時，將不會載入檔案 (*.mdb) 的資訊。
...
### <a name="mt4101-the-registrar-cannot-build-a-signature-for-type-"></a>MT4101: 註冊機構都無法建立類型的簽章`*`。
...
### <a name="mt4102-the-registrar-found-an-invalid-type--in-signature-for-method--use--instead"></a>MT4102: 註冊機構找到無效的型別`*`方法的簽章中`*`。 請改用 `*`。

這目前僅發生一種類型： System.DateTime。 請改用 Objective C 對等項目 (NSDate)。

<a name="MT4103" />

### <a name="mt4103-the-registrar-found-an-invalid-type--in-signature-for-method--the-type-implements-inativeobject-but-does-not-have-a-constructor-that-takes-two-intptr-bool-arguments"></a>MT4103: 註冊機構找到無效的型別`*`方法的簽章中`*`： 型別會實作 INativeObject，但並沒有會採用兩個建構函式 (IntPtr，bool) 引數
...
### <a name="mt4104-the-registrar-cannot-marshal-the-return-value-for-type--in-signature-for-method-"></a>MT4104: 註冊機構無法封送處理類型的傳回值`*`方法的簽章中`*`。
...
### <a name="mt4105-the-registrar-cannot-marshal-the-parameter-of-type--in-signature-for-method-"></a>MT4105: 註冊機構無法封送處理的型別參數`*`方法的簽章中`*`。

如果您認為 Xamarin.iOS 應該支援的類型有問題，請提出的增強功能要求[ http://bugzilla.xamarin.com ](http://bugzilla.xamarin.com)。

<a name="MT4106" />

### <a name="mt4106-the-registrar-cannot-marshal-the-return-value-for-structure--in-signature-for-method-"></a>MT4106: 註冊機構無法封送處理結構的傳回值`*`方法的簽章中`*`。
...
### <a name="mt4107-the-registrar-cannot-marshal-the-parameter-of-type--in-signature-for-method-"></a>MT4107: 註冊機構無法封送處理的型別參數`*`方法的簽章中`+`。
...
### <a name="mt4108-the-registrar-cannot-get-the-objectivec-type-for-managed-type-"></a>MT4108: 註冊機構都無法取得 managed 類型的 ObjectiveC 類型`*`。
...
### <a name="mt4117-the-registrar-found-a-signature-mismatch-in-the-method----the-selector-indicates-the-method-takes--parameters-while-the-managed-method-has--parameters"></a>MT4117: 註冊機構位於簽章不相符的方法 '*。*'-選取器指出此方法會採用 * 參數，而受管理的方法有 * 參數。
...
### <a name="mt4120-the-registrar-found-an-unknown-field-type--in-field--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4120: 註冊機構找了未知的欄位類型 '\*'中欄位'\*。 *'。 請提出錯誤報告在 http://bugzilla.xamarin.com
...
### <a name="mt4123-the-type-of-the-variadic-parameter-in-the-variadic-function--must-be-systemintptr"></a>MT4123: 在 variadic 函式之 variadic 參數類型 ' *' 必須是 System.IntPtr。
...
### <a name="mt4125-the-registrar-found-an-invalid-type--in-signature-for-method--the-interface-must-have-a-protocol-attribute-specifying-its-wrapper-type"></a>MT4125: 註冊機構找到無效的類型 '\*'中方法簽章'\*': 介面必須要有通訊協定屬性，指定其包裝函式型別。
...
### <a name="mt4128-the-registrar-found-an-invalid-generic-parameter-type--in-the-method--the-generic-parameter-must-have-an-nsobject-constraint"></a>MT4128: 註冊機構找到無效的泛型參數類型 '\*'中方法'\*'。 泛型參數都必須有 'NSObject' 條件約束。
...
### <a name="mt4130-the-registrar-cannot-export-static-methods-in-generic-classes-"></a>MT4130: 註冊機構無法匯出中的泛用類別的靜態方法 ('* ')。
...
### <a name="mt4136-the-registrar-cannot-marshal-the-parameter-type--of-the-parameter--in-the-method-"></a>MT4136: 註冊機構無法封送處理參數類型 '\*'的參數'\*'中方法'\*。 *'

<!-- MT4137 is unused -->

<a name="MT4138" />

### <a name="mt4138-the-registrar-cannot-marshal-the-property-type--of-the-property-"></a>MT4138: 註冊機構無法封送處理屬性類型 '\*'的屬性'\*。 *'。

<a name="MT4139" />

### <a name="mt4139-the-registrar-cannot-marshal-the-property-type--of-the-property--properties-with-the-connect-attribute-must-have-a-property-type-of-nsobject-or-a-subclass-of-nsobject"></a>MT4139: 註冊機構無法封送處理屬性類型 '\*'的屬性'\*。 *'。 NSObject 的屬性類型 （或 NSObject 的子類別），必須使用 [連線] 屬性的屬性。

<a name="MT4140" />

### <a name="mt4140-the-registrar-found-a-signature-mismatch-in-the-method----the-selector-indicates-the-variadic-method-takes--parameters-while-the-managed-method-has--parameters"></a>MT4140: 註冊機構位於簽章不相符的方法 '*。*'-選取器可讓您表示 variadic 方法會採用 * 參數，而受管理的方法有 * 參數。
...
### <a name="mt4146-the-name-parameter-of-the-registrar-attribute-on-the-class---contains-an-invalid-character--"></a>MT4146: 註冊機構上屬性的類別的 Name 參數\*'('\*') 包含無效的字元:'\*' (\*)。
...
### <a name="mt4148-the-registrar-found-a-generic-protocol--exporting-generic-protocols-is-not-supported"></a>MT4148: 註冊機構找泛用的通訊協定: ' *'。 不支援匯出一般通訊協定。
...
### <a name="mt4163-internal-error-in-the-registrar--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT4163： 發生內部錯誤的註冊機構 （*）。 請提出錯誤報告在 http://bugzilla.xamarin.com
...
### <a name="mt4165-the-registrar-couldnt-find-the-type-systemvoid-in-any-of-the-referenced-assemblies"></a>MT4165: 註冊機構找不到類型 'System.Void' 任何參考的組件中。
...
### <a name="mt4170-the-registrar-cant-convert-from-managed-type-to-native-type-for-the-return-value-in-the-method-method"></a>MT4170: 註冊機構的無法轉換從 '{managed type}' '{原生型別}' {方法} 方法中傳回的值。

請參閱錯誤的描述<a href="#MT4172">MT4172</a>。

<a name="MT4171" />

### <a name="mt4171-the-bindas-attribute-on-the-member-member-is-invalid-the-bindas-type-type-is-different-from-the-property-type-type"></a>MT4171： 成員 {成員} BindAs 屬性無效： BindAs 型別 {type} 是 {type} 屬性類型不同。

請確定 BindAs 屬性中的型別符合它附加到成員的型別。

<a name="MT4172" />

### <a name="mt4172-the-registrar-cant-convert-from-native-type-to-managed-type-for-the-parameter-parameter-name-in-the-method-method"></a>MT4172: 註冊機構無法轉換從 '{原生 type}' 為 '{managed type}' 中方法 {方法} 的參數 '{parameter name}'。
...
### <a name="mt5103-failed-to-compile-the-file--please-file-a-bug-report-at-httpbugzillaxamarincom"></a>MT5103: 無法將檔案編譯 ' *'。 請提出錯誤報告在 http://bugzilla.xamarin.com
...
### <a name="mt5213-duplicate-symbol-in--location-related-to-previous-error"></a>MT5213： 重複的符號: * （與之前錯誤相關的位置）
...
### <a name="mt5215-references-to--might-require-additional--frameworkxxx-or--lxxx-instructions-to-the-native-linker"></a>MT5215： 參考至 ' *' 可能會需要額外-framework = XXX 或-lXXX 指示原生連結器
...
### <a name="mt7058-merge-specified-entry-must-be-a-container"></a>MT7058: 合併: 指定項目必須是一個容器。
...
### <a name="mt8001-version-mismatch-between-the-native-xamarinios-runtime-and-monotouchdll-please-reinstall-xamarinios"></a>之間的原生的 Xamarin.iOS 執行階段和 monotouch.dll MT8001： 版本不相符。 請重新安裝 Xamarin.iOS。
...
### <a name="mt8003-failed-to-find-the-closed-generic-method--on-the-type-"></a>MT8003： 若要尋找封閉式泛型方法失敗 '\*'在類型'\*'。
...
### <a name="mt8006-failed-to-find-the-selector--on-the-type-"></a>MT8006: 若要尋找的選取器失敗，'\*'在類型'\*'

<a name="MT8007" />

### <a name="mt8007-cannot-get-the-method-descriptor-for-the-selector--on-the-type--because-the-selector-does-not-correspond-to-a-method"></a>MT8007： 無法取得選取器的方法描述元 '\*'在類型'\*'，因為選取器未對應至方法

<a name="MT8008" />

### <a name="mt8008-the-loaded-version-of-xamariniosdll-was-compiled-for--bits-while-the-process-is--bits-please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8008: Xamarin.iOS.dll 載入的版本所編譯 * 位元，而處理程序是 * 位元。 請將 bug 歸檔在 http://bugzilla.xamarin.com。
...
### <a name="mt8010-native-type-size-mismatch-between-xamariniosmacdll-and-the-executing-architecture-xamariniosmacdll-was-built-for--bit-while-the-current-process-is--bit"></a>MT8010: Xamarin 之間的原生類型的大小不相符。[iOS |Mac].dll 和執行的架構。 Xamarin。[iOS |專為 Mac].dll *-位元，而目前的處理程序 * 位元。
...
### <a name="mt8012-invalid-delegateproxyattribute-for-the-return-value-for-the-method--delegatetype-is-null-please-file-a-bug-at-httpbugzillaxamarincom"></a>MT8012： 方法的傳回值的無效 DelegateProxyAttribute *。*： 委派為 null。 請將 bug 歸檔在 http://bugzilla.xamarin.com。
...
### <a name="mt8020-could-not-find-the-module-with-metadatatoken--in-the-assembly-"></a>MT8020： 找不到模組 MetadataToken * 組件中 *。
...
### <a name="mt8024-could-not-find-a-valid-extension-type-for-the-smart-enum-smarttype-please-file-a-bug-at-httpsbugzillaxamarincom"></a>MT8024： 找不到有效的延伸模組型別智慧列舉 '{smart_type}'。 請將 bug 歸檔在 https://bugzilla.xamarin.com。
