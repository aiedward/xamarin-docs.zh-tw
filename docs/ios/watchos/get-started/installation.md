...
- **監看式延伸模組專案**-這包含監看式應用程式 （例如，控制器類別） 的程式碼。
...
下載並執行[WatchKitCatalog](https://developer.xamarin.com/samples/monotouch/WatchKit/WatchKitCatalog/)範例著手。
...
**注意：** 請記住，[監看式] 畫面時，沒有自動出現的 iOS 模擬器中使用 Xcode 6。
