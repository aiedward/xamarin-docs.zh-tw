...
下列 c # 範本包含可執行檔最小必要方法 `CLKComplicationDataSource` 。
...
所有複雜的功能都實作為單一類別，並覆寫 `CLKComplicationDataSource` 抽象類別的方法（這會實作為 `ICLKComplicationDataSource` 介面）。
...
- `GetSupportedTimeTravelDirections`-傳回的選項 `CLKComplicationTimeTravelDirections` ，例如 `None` 、 `Forward` 、 `Backward` 或 `Forward | Backward` 。
...
如果這個方法傳回，則 `HideOnLockScreen` 當監看式鎖定時，複雜的會顯示圖示或應用程式名稱（而不是任何資料）。
...
時間移動支援是選擇性的，而且是由 `GetSupportedTimeTravelDirections` 方法所控制。 如果它傳回 `Forward` 、 `Backward` 或，則 `Forward | Backward` 您必須執行下列方法
...
這些範本類別名稱的前面都會加上 `CLKComplicationTemplateCircularSmall` ：
...
這些範本類別名稱的前面都會加上 `CLKComplicationTemplateModularSmall` ：
...
這些範本類別名稱的前面都會加上 `CLKComplicationTemplateModularLarge` ：
...
這些範本類別名稱的前面都會加上 `CLKComplicationTemplateUtilitarianSmall` ：
