...
1. 選取您的小組，並按一下 [管理憑證…]：[![選取小組](ad-hoc-distribution-images/selectteam.png)](ad-hoc-distribution-images/selectteam.png#lightbox)

2. 接著，按一下 [加號 (+)] 按鈕，然後選取 [iOS App Store]：[![選取 iOS App Store](ad-hoc-distribution-images/selectcert.png)](ad-hoc-distribution-images/selectcert.png#lightbox)
...
1. 返回 Apple Provisioning Portal 並選取 [佈建] > [散發]：[![選取布建 > 散發](ad-hoc-distribution-images/distribute01.png)](ad-hoc-distribution-images/distribute01.png#lightbox)
...
2. 從 [組態] 下拉式清單中選取 [iOS Bundle Signing] (iOS 套件組合簽署) 和組建的類型：
    ...
    ![從 [設定] 下拉式清單中選取組建的類型](ad-hoc-distribution-images/releasexs01.png)
3. 在大多數情況下，可以保留 [簽署識別] 和 [佈建設定檔] 的預設值為 [自動]，Visual Studio for Mac 會根據 Info.plist 中的應用程式套件組合識別碼選擇正確的設定檔：

    ![簽署身分識別和布建設定檔設定為預設值 [自動]](ad-hoc-distribution-images/releasexs02.png)
4. 視需要，可在下拉式清單中選取 [簽署識別] 和 [散發設定檔] (上面所建立的)：

    ![選取簽署身分識別和散發設定檔](ad-hoc-distribution-images/releasexs03.png)
5. 按一下 [確定] 按鈕以儲存所做的變更。
...
2. 從 [組態] 下拉式清單中選取 [iOS Bundle Signing] (iOS 套件組合簽署) 和組建的類型：
    ...
    ![從 [設定] 下拉式清單中選取組建的類型](ad-hoc-distribution-images/releasevs01.png)
3. 在大多數情況下，可以保留 [簽署識別] 和 [佈建設定檔] 的預設值為 [自動]，Visual Studio 會根據 Info.plist 中的應用程式套件組合識別碼選擇正確的設定檔：

    ![簽署身分識別和布建設定檔設定為預設值 [自動]](ad-hoc-distribution-images/releasevs02.png)
4. 視需要，可在下拉式清單中選取 [簽署識別] 和 [散發設定檔] (上面所建立的)：

    ![選取簽署身分識別和散發設定檔](ad-hoc-distribution-images/releasevs03.png)
5. 將您的變更儲存至專案屬性。
