...
---
# <a name="app-group-capabilities"></a>應用程式群組功能
...
應用程式群組會有識別碼，這通常是含有 group. 前置詞的「套件組合識別碼」。 例如，「套件組合識別碼」`com.xamarin.WatchSettings` 的應用程式群組會是 `group.com.xamarin.WatchSettings`。
