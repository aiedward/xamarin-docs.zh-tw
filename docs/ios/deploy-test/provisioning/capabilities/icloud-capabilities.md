...
3. 輸入 iCloud 容器的 [Description] \(描述\) 和唯一 [Identifier] \(識別碼\)： 
    ...
    ![螢幕擷取畫面顯示註冊「我的雲端」容器。](icloud-capabilities-images/image24.png)

4. 按 [Continue] \(繼續\)，確定資訊正確，然後按 [註冊] 以建立 iCloud Container (iCloud 容器)：  
    
    ![螢幕擷取畫面顯示確認我的雲端容器。](icloud-capabilities-images/image25.png)
...
此應用程式識別碼現在可用來產生（或重新產生）新的布建設定檔，如 [使用功能](~/ios/deploy-test/provisioning/capabilities/index.md) 指南中所述。 
