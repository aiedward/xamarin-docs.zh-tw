...
[![最終的應用程式將會有第二個畫面顯示通話記錄，如以下螢幕擷取畫面所示](hello-ios-multiscreen-quickstart-images/00.png)](hello-ios-multiscreen-quickstart-images/00.png#lightbox)
...
1. 在 Visual Studio for Mac 中開啟 **Phoneword** 應用程式。 如有必要，可從[這裡](https://docs.microsoft.com/samples/xamarin/ios-samples/hello-ios)下載[Hello，iOS 逐步](~/ios/get-started/hello-ios/index.md)解說指南的完整 Phoneword 應用程式。

2. 從 **Solution Pad** 開啟 **Main.storyboard** 檔案：

    ![iOS 設計工具中的 Main.storyboard](hello-ios-multiscreen-quickstart-images/02new.png)

3. 將 [**流覽控制器**] 從 [**工具箱**] 拖曳至設計介面（您可能需要縮小，才能在設計介面上調整這些大小！）：

    ![將 [流覽控制器] 從 [工具箱] 拖曳至設計介面](hello-ios-multiscreen-quickstart-images/03new.png)

4. 將 [ **segue] segue** （也就是 [單一視圖控制器] 左邊的灰色箭號）拖曳至 [**流覽控制器**]，以變更應用程式的起點：

    ![將 [Segue] Segue 拖曳至 [流覽控制器]，以變更應用程式的起點](hello-ios-multiscreen-quickstart-images/04new.png)

5. 按一下底部列來選取現有的 [**根視圖控制器**]，然後按 [**刪除**] 將它從設計介面中移除。
然後，移至 [**流覽控制器**] 旁的**Phoneword**場景：

    ![移動 [流覽控制器] 旁的 Phoneword 場景](hello-ios-multiscreen-quickstart-images/05new.png)

6. 將**ViewController**設定為流覽控制器的**根視圖控制器**。 按下**Ctrl**鍵，然後按一下**流覽控制器**內部。 應該會出現藍色線條。 然後，按住**Ctrl**鍵，從 [**流覽控制器**] 拖曳至**Phoneword**場景，然後放開。 這稱為「Ctrl 拖曳」__：

    ![從 [流覽控制器] 拖曳至 Phoneword 場景，然後放開](hello-ios-multiscreen-quickstart-images/06.png)
...
11. 建立 [通話記錄]**** 畫面。 從 [**工具箱**] 中，將 [**資料表視圖控制器**] 拖曳至設計介面：

    ![將 [資料表視圖控制器] 拖曳至設計介面](hello-ios-multiscreen-quickstart-images/12new.png)

12. 接下來，按一下場景底部的黑色列來選取 [**資料表視圖控制器**]。 在 [ **Properties Pad**中，將 [**資料表視圖控制器] 的**類別變更為 `CallHistoryController` ，然後按**enter**鍵：

    ![將資料表視圖控制器類別變更為為 callhistorycontroller](hello-ios-multiscreen-quickstart-images/13new.png)
...
14. 在**Phoneword**場景與**通話記錄**場景之間建立_segue_ （轉換）。
  在**Phoneword 場景**中，選取 [**通話記錄] 按鈕**，然後從**按鈕**Ctrl 拖曳至**通話記錄**場景：

    ![從按鈕 Ctrl 拖曳至通話記錄場景](hello-ios-multiscreen-quickstart-images/15.png)
...
15. 選取場景底部的黑色列，然後將 [ **view controller] 標題**變更為 [ **Properties Pad**中的 [**記錄**]，以將**標題**新增至**資料表視圖控制器**：
...
16. 執行應用程式時，**通話記錄按鈕**將會開啟 [通話記錄]**** 畫面，但資料表檢視會是空白，這是因為沒有可追蹤和顯示電話號碼的程式碼。
    ...
    `using` `System.Collections.Generic` 在**ViewController**頂端新增的指示詞：
...
18. 按下 [**啟動**] 按鈕，在**iOS**模擬器內啟動應用程式：

    ![按下 [啟動] 按鈕，在 iOS 模擬器內啟動應用程式](hello-ios-multiscreen-quickstart-images/19.png)
...
4. 將 [ **segue] Segue** （這是**Phoneword**場景左邊的灰色箭號）從 [ **Phoneword** ] 場景拖曳至 [**流覽控制器**]，以變更應用程式的起點：

    ![將 [Segue] Segue 拖曳至 [流覽控制器]，以變更應用程式的起點](hello-ios-multiscreen-quickstart-images/image3.png)

5. 按一下黑色列來選取 [**根視圖控制器**]，然後按 [**刪除**]，將它從設計介面中移除。
  然後，移至 [**流覽控制器**] 旁的**Phoneword**場景：

    ![移動 [流覽控制器] 旁的 Phoneword 場景](hello-ios-multiscreen-quickstart-images/image4.png)

6. 將**ViewController**設定為流覽控制器的根視圖控制器。 按下 **Ctrl** 鍵，並在 [瀏覽控制器]**** 內部按一下。 應該會出現藍色線條。 然後，按住**Ctrl**鍵，從 [**流覽控制器**] 拖曳至**Phoneword**場景，然後放開。 這稱為「Ctrl 拖曳」__：

    ![從 [流覽控制器] 拖曳至 Phoneword 場景，然後放開](hello-ios-multiscreen-quickstart-images/image5.png)
...
12. 按一下場景底部的黑色列，以選取 [**資料表視圖控制器**]。 在 [**屬性] Explorer**中，將 [**資料表視圖控制器] 的**類別變更為 `CallHistoryController` ，然後按**enter**鍵：

    ![將資料表視圖控制器類別變更為為 callhistorycontroller](hello-ios-multiscreen-quickstart-images/image11.png)
...
14. 在**Phoneword**場景與**通話記錄**場景之間建立_segue_ （轉換）。
  在**Phoneword 場景**中，選取 [**通話記錄] 按鈕**，然後從**按鈕** **Ctrl 拖曳**至**通話記錄**場景：

    ![從按鈕 Ctrl 拖曳至通話記錄場景](hello-ios-multiscreen-quickstart-images/image13.png)
...
15. 選取場景底部的黑色列，然後在 [**屬性] Explorer**中，將 [ **view Controller] > 標題**變更為**通話記錄**，以將**標題**新增至**資料表視圖控制器**：
