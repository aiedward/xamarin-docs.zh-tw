...
ms.locfileid: "73021907"
---
# <a name="editing-tables-with-xamarinios"></a>使用 Xamarin 編輯資料表

資料表編輯功能是藉由覆寫 `UITableViewSource` 子類別中的方法來啟用。 最簡單的編輯行為是可以使用單一方法覆寫來實作為滑動的刪除手勢。
...
- **CommitEditingStyle** –資料表來源會偵測是否已覆寫這個方法，並自動啟用輕刷刪除手勢。 方法的實作為應該呼叫 `UITableView` 上的 `DeleteRows`，讓儲存格消失，同時也從模型中移除基礎資料（例如，陣列、字典或資料庫）。 
...
`UITableViewSource` 上有許多不同的方法會影響資料表的編輯模式行為：

- **CanEditRow** –是否可以編輯每個資料列。 傳回 false，以避免在編輯模式中進行輕刷刪除和刪除。 
- **CanMoveRow** –傳回 true 以啟用移動 ' handle ' 或 false 以防止移動。 
- **EditingStyleForRow** –當資料表處於編輯模式時，這個方法的傳回值會決定資料格是否顯示紅色刪除圖示或綠色的新增圖示。 如果資料列不應該是可編輯的，則傳回 `UITableViewCellEditingStyle.None`。 
...
`MoveRow` 的實做比較複雜，因為它需要改變基礎資料結構，以符合新的順序。 由於資料會實作為 `List`，因此下列程式碼會刪除舊位置的資料項目，並將它插入新的位置。 如果資料是儲存在具有 ' order ' 資料行的 SQLite 資料庫資料表中（例如），這個方法就必須執行一些 SQL 作業來重新排列該資料行中的數位。
...
`UITableViewSource` 上有許多不同的方法會影響資料表的編輯模式行為。 在範例程式碼中，這些方法已實作為下列方式：

- **EditingStyleForRow** –傳回包含資料之資料列的 `UITableViewCellEditingStyle.Delete`，並傳回最後一列的 `UITableViewCellEditingStyle.Insert` （將特別新增為 [插入] 按鈕的行為）。 
...
- **WillBeginTableEditing** –觸及 [**編輯**] 按鈕時，會呼叫 `SetEditing` 將資料表置於編輯模式。 這會觸發 WillBeginTableEditing 方法，在資料表的結尾顯示 **（加入新的）** 資料列，以作為「插入按鈕」。 
- **DidFinishTableEditing** –觸及 [完成] 按鈕時 `SetEditing` 再次呼叫以關閉編輯模式。 當不再需要編輯時，範例程式碼會從資料表中移除 **（加入新的）** 資料列。 
...
不常使用此資料列插入 UI 模式，不過您也可以使用 `UITableView.BeginUpdates` 和 `EndUpdates` 方法，以動畫顯示在任何資料表中插入或移除儲存格。 使用這些方法的規則是，在 `BeginUpdates` 和 `EndUpdates` 呼叫之間 `RowsInSection` 所傳回的值差異，必須符合使用 `InsertRows` 和 `DeleteRows` 方法新增/刪除的資料格數目。 如果基礎資料來源未變更為符合資料表視圖上的插入/刪除，則會發生錯誤。
