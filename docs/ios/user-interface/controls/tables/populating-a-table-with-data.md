...
產生的資料表看起來像這樣：
 ...
 [![執行中的範例資料表](populating-a-table-with-data-images/image3.png)](populating-a-table-with-data-images/image3.png#lightbox)

大部分的資料表都可讓使用者觸及資料列來選取它，並執行其他動作（例如播放歌曲、呼叫連絡人，或顯示另一個畫面）。 為了達成此目的，我們需要做幾件事。 首先，讓我們建立一個 AlertController，在使用者按一下資料列時顯示訊息，方法是將下列內容新增至 `RowSelected` 方法：
...
輸出如下所示：
 ...
 [![範例標頭和頁尾](populating-a-table-with-data-images/image6.png)](populating-a-table-with-data-images/image6.png#lightbox)
