...
`UITableView`具有原型內容的通常是用來顯示清單中每個專案的原型資料格（如您可以定義一個以上的儲存格）。 資料格不需要具現化，而是在方法中藉 `GetView` 由呼叫 `DequeueReusableCell` 其的方法取得 `UITableViewSource` 。
...
在 Visual Studio 中建立新的方案，方法是使用 **（建立） [新增專案 ...] > 單一視圖應用程式（c #）**，並呼叫它_StoryboardTables_。

 [![[建立新專案] 對話方塊](creating-tables-in-a-storyboard-images/npd.png)](creating-tables-in-a-storyboard-images/npd.png#lightbox)

方案將會開啟，其中包含一些 c # 檔案和 `Main.storyboard` 已建立的檔案。 按兩下檔案， `Main.storyboard` 在 IOS 設計工具中開啟檔案。
...
8. 新的 UITableViewController 必須設定其類別名稱和識別碼。 選取 [視圖控制器]，然後在 [ **Properties Pad**中輸入**類別**的_TaskDetailViewController_ –這會 `TaskDetailViewController.cs` 在 Solution Pad 中建立新的檔案。 輸入**StoryboardID**做為_詳細資料_，如下列範例所示。 稍後將用來以 c # 程式碼載入此視圖：  
...
- 在 [ **Properties Pad**的 [列]**按鈕專案**下，選取 [**識別碼：新增**] （使其成為 *+* 加號按鈕）。 
...
其餘工作會在 Mac 或 Windows 上以 c # Visual Studio 完成。 請注意，程式碼中使用的屬性名稱會反映上述逐步解說中的設定。
...
接下來，建立 `RootTableSource` 繼承自的類別 `UITableViewSource` 。 

這個範例和非分鏡腳本資料表的觀點，在於 `GetView` 方法不需要具現化任何資料格– `theDequeueReusableCell` 方法一律會傳回原型資料格的實例（具有相符的識別碼）。
...
若要使用 `RootTableSource` 類別，請在的函式中建立新集合 `ItemViewController` ：
...
在中，將 `ViewWillAppear` 集合傳遞至來源並指派給資料表視圖：
...
若要在 segue 中「傳送參數」，請覆寫 `PrepareForSegue` 方法並在上設定屬性 `DestinationViewController` （ `TaskDetailViewController` 在此範例中為）。 目的地視圖控制器類別會具現化，但尚未向使用者顯示–這表示您可以設定類別的屬性，但不能修改任何 UI 控制項：
...
在方法中，會將 `TaskDetailViewController` `SetTask` 其參數指派給屬性，以便在 viewwillappear: 每當中參考它們。 無法在中修改控制項屬性， `SetTask` 因為當呼叫時，可能不存在 `PrepareForSegue` ：
...
接下來，您必須將按鈕的 `TouchUpInside` 事件處理常式新增至 `ViewDidLoad` **TaskDetailViewController.cs**的方法。 `Delegate`已明確建立的屬性參考， `ItemViewController` 因此我們可以呼叫 `SaveTask` 和，這會在其作業中 `DeleteTask` 關閉此視圖：
...
最後一項要建立的功能就是建立新的工作。 在**ItemViewController.cs**中，新增建立新工作的方法，並開啟詳細資料檢視。 若要從分鏡腳本具現化視圖，請使用方法搭配該 `InstantiateViewController` 視圖的， `Identifier` 在此範例中會是「詳細資料」：
...
最後，在**ItemViewController.cs**的方法的導覽列中，連上按鈕 `ViewDidLoad` 來呼叫它：
...
- 如何建立 segue 並覆寫方法， `PrepareForSegue` 以通知目標視圖其所需的任何參數。 
- 使用方法直接載入分鏡腳本視圖 `Storyboard.InstantiateViewController` 。
