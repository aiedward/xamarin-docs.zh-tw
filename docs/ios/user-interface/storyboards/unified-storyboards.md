...
雖然應用程式會從程式碼完全建立其 UI，而不是使用 Xcode 的 Interface Builder 來建立整合的分鏡腳本，但同樣的技巧也適用。 
...
- 向後部署至 iOS 6 和 iOS 7。
- 預覽不同裝置、方向和作業系統版本的版面配置。

<a name="enabling-size-classes"></a>

### <a name="enabling-size-classes"></a>啟用大小類別

根據預設，任何新的 Xamarin 專案都會使用大小類別。 若要在腳本中使用來自較舊專案的大小類別和自我調整 Segue，必須先將它轉換成 Xcode 6 統一分鏡腳本格式，然後在您的分鏡腳本的 Xcode 檔案偵測器內選取 [使用大小類別] 核取方塊。
...
在新的 iPhone 6 和 iPhone 6 Plus 裝置中， (以及即將推出的 Apple Watch) 與所有現有的 iPhone 和 iPad 裝置相結合，這代表了一大系列不同大小、方向以及 `Default.png` 必須建立和維護之啟動畫面影像資產的解析度。 此外，這些檔案可能很大，而且會「膨脹」交付項應用程式套件組合，以增加從 iTunes App Store 下載應用程式所需的時間， (可能使其無法透過行動電話通訊網路進行傳遞) 並增加終端使用者裝置上所需的儲存空間量。
...
2. 在 [ **Solution Explorer**] 中，以滑鼠右鍵按一下檔案 `MainStoryboard.storyboard` ，然後選取 [**開啟方式**  >  **Xcode Interface Builder**：

    [![使用 Xcode Interface Builder 開啟](unified-storyboards-images/dls01.png)](unified-storyboards-images/dls01.png#lightbox)
...
12. 等候 Visual Studio for Mac 完成與 Xcode 的同步變更。
13. 在 [ **Solution Explorer**] 中，以滑鼠右鍵按一下 [**資源**] 資料夾，然後選取 [**新增**  >  **新增檔案 ...**：
...
停止 Visual Studio for Mac 中的應用程式，然後選取 iPad iOS 8 裝置。 執行應用程式，此裝置和方向的啟動畫面將會正確格式化：
...
最後，本文涵蓋了建立單一動態啟動畫面的基本功能，其會在每個 iOS 8 裝置上顯示為啟動畫面。
