...
只要按鈕的 `Enabled` 屬性為 `true` ，而且它不是由另一個控制項或視圖所涵蓋，就可以使用 Siri 遠端，將它設為焦點專案。 如果使用者選取按鈕並按一下觸控介面，則 `ButtonPressed` 會執行上述定義的動作。

> [!IMPORTANT]
> 雖然 `TouchUpInside` `UIButton` 在建立**事件處理常式**時，可以在 iOS 設計工具中將動作（例如）指派給，但絕不會呼叫它，因為 Apple TV 沒有觸控式螢幕或支援觸控事件。 建立 tvOS 使用者介面元素的**動作**時，您應該一律使用預設**動作類型**。
...
或者， `UIButton` 可以在 c # 程式碼中建立，並將其新增至 tvOS 應用程式的 view。 例如：
...
當您在程式碼中建立新的時 `UIButton` ，您會將其指定 `UIButtonType` 為下列其中一項：
...
然後，設定按鈕的標題。 `UIButtons`與大部分 `UIKit` 的控制項不同，因為它們具有狀態，所以您不能只是變更標題，而必須針對指定的進行變更 `UIControlState` 。 例如：
...
> [!IMPORTANT]
> 雖然可以將動作（例如）指派給 `TouchUpInside` ，但 `UIButton` 永遠不會呼叫它，因為 Apple TV 沒有觸控式螢幕或支援觸控事件。 您應該一律使用**AllEvents**或**PrimaryActionTriggered**之類的事件。
...
如先前所見，與 `UIButtons` 大部分的控制項不同，因為 `UIKit` 它們具有狀態，所以您不能只是變更標題，而必須針對指定的進行變更 `UIControlState` 。 例如：
...
而且您可以使用來調整標題的陰影 `SetTitleShadowColor` 。 例如：
...
將 `AdjustsImageWhenHiglighted` 屬性設定為 `true` ，以在反白顯示按鈕時，將影像繪製為較淡（這是預設值）。 將 `AdjustsImageWhenDisabled` 屬性設定為 `true` ，以在停用按鈕時將影像繪製為較暗（同樣地，這是預設值）。
...
您 `TintColor` 可以使用屬性來設定同時套用至標題和按鈕影像的色彩色調。 對於類型的按鈕 `Custom` ，此屬性沒有任何作用，您必須 `TintColor` 自行執行行為。

<a name="Summary"></a>

## <a name="summary"></a>總結

本文涵蓋設計和使用 tvOS 應用程式內的按鈕。 它示範如何使用 iOS 設計工具中的按鈕，以及如何在 c # 程式碼中建立按鈕。 最後，它會示範如何修改按鈕的標題，以及變更其樣式和外觀。
