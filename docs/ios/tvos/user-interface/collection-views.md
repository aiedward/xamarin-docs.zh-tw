...
在 tvOS 中使用集合視圖時，您的應用程式會負責使用資料來源（）提供與集合相關聯的資料 `UICollectionViewDataSource` 。 您可以選擇性地組織集合視圖資料，並將其呈現在不同的群組中（區段）。

[集合] 視圖會使用資料格（）來呈現螢幕上的個別專案 `UICollectionViewCell` ，以提供集合中指定資訊的呈現方式（例如影像及其標題）。

選擇性地，您可以將補充視圖加入至集合視圖的簡報，做為區段和儲存格的頁首和頁尾。 集合視圖的版面配置會負責定義這些視圖的位置，以及個別的儲存格。

[集合] 視圖可以使用委派（）來回應使用者互動 `UICollectionViewDelegate` 。 如果資料格已反白顯示，或已選取一個，則此委派也會負責判斷指定的儲存格是否可以取得焦點。 在某些情況下，委派會決定個別資料格的大小。

<a name="Collection-View-Layouts"></a>

## <a name="collection-view-layouts"></a>集合視圖版面配置

集合視圖的一項重要功能，就是它所呈現的資料與其版面配置之間的分隔。 集合視圖 `UICollectionViewLayout` 配置（）負責在集合視圖的螢幕簡報中提供組織和儲存格的位置（以及任何補充的視圖）。
...
呼叫其中任一種方法之前，您必須先註冊 `.xib` 用來以集合視圖建立儲存格視圖的類別、分鏡腳本或檔案。 例如：
...
- 它負責從其分鏡腳本或檔案載入集合視圖 `.xib` ，並具現化視圖。 如果是在程式碼中建立，則會自動建立新的、未設定的集合視圖。
- 一旦載入集合視圖，控制器就會嘗試從分鏡腳本或檔案載入其資料來源和委派 `.xib` 。 如果沒有可用的，它會將本身設定為兩者的來源。
- 確保資料會在第一次顯示時填入集合視圖之前載入，並在每次後續顯示時重載並清除選取。

此外，集合視圖控制器會提供可覆寫的方法，以便用來管理集合視圖的生命週期，例如 `AwakeFromNib` 和 `ViewWillDisplay` 。
...
1. 在 [**方案總管**中，按兩下該檔案 `Main.storyboard` ，然後在 iOS 設計工具中開啟它。
...
1. 從按鈕控制項拖曳至集合視圖控制器，然後從快顯視窗中選取 [**推送**]： 
    ...
    [![從快顯視窗中選取 [推送]](collection-views-images/collection05.png)](collection-views-images/collection05.png#lightbox)
...
1. 選取 [集合視圖控制器]，並將其類別設定為 `CityCollectionViewController` [ **Widget]** 索引標籤中的： 
...
1. 從按鈕控制項拖曳至集合視圖控制器，然後從快顯視窗中選取 [**推送**]： 
    ...
    [![從快顯視窗中選取 [推送]](collection-views-images/collection05vs.png)](collection-views-images/collection05vs.png#lightbox)
1. 當應用程式執行時，每當使用者按一下按鈕時，就會顯示集合視圖。
...
如果我們選擇了 `Custom` 集合視圖的**版面**配置，我們就可以指定自訂版面配置。 Apple 提供內建的功能，可讓您 `UICollectionViewFlowLayout` `UICollectionViewDelegateFlowLayout` 輕鬆地以方格為基礎的版面配置來呈現資料（版面配置樣式會使用這些功能 `flow` ）。 
...
現在，我們必須定義每個資料格的資料呈現方式。 編輯檔案 `CityCollectionViewCell.cs` （自動從分鏡腳本檔案建立），使其看起來如下所示：
...
讓我們詳細查看此類別。 首先，我們會繼承自 `UICollectionViewDataSource` 並提供資料格識別碼的快捷方式（我們在 IOS 設計工具中指派）：
...
取得類型的 [集合視圖] 資料格之後 `CityCollectionViewCell` ，我們會在其中填入指定的專案。
...
我們需要一種方法，將目前選取的專案從集合視圖中的相關聯回到呼叫視圖。 我們將在上使用自訂屬性 `AppDelegate` 。 編輯檔案 `AppDelegate.cs` ，並新增下列程式碼：
...
接下來，將新的 `CityViewDelegate` 類別新增至專案，使其看起來如下所示：
...
讓我們仔細看一下這個類別。 首先，我們會繼承自 `UICollectionViewDelegateFlowLayout` 。 我們繼承自這個類別而不是的原因 `UICollectionViewDelegate` 是，我們使用內建的 `UICollectionViewFlowLayout` 來呈現專案，而不是自訂的版面配置類型。
...
我們會檢查指定的支援資料片段是否將其旗標 `CanSelect` 設定為 `true` ，並傳回該值。 如需導覽和焦點的詳細資訊，請參閱我們[的使用導覽和焦點](~/ios/tvos/app-fundamentals/navigation-focus.md)和[Siri 遠端和藍牙控制器](~/ios/tvos/platform/remote-bluetooth.md)檔。
...
在此，我們 `SelectedCity` 會將的屬性設定 `AppDelegate` 為使用者選取的專案，並關閉集合視圖控制器，並返回呼叫我們的視圖。 我們 `ParentController` 尚未定義集合視圖的屬性，接下來我們將執行此動作。

<a name="Configuring-the-Collection-View"></a>

## <a name="configuring-the-collection-view"></a>設定集合視圖

現在，我們需要編輯集合視圖，並指派我們的資料來源和委派。 編輯檔案 `CityCollectionView.cs` （從我們的腳本中自動為我們建立），使其看起來如下所示：
...
編輯檔案 `CityCollectionViewController.cs` （從我們的腳本自動建立），使其看起來如下所示：
...
編輯檔案 `ViewController.cs` （從我們的腳本自動建立），使其看起來如下所示：
...
下列程式碼一開始會從的屬性顯示選取的專案 `SelectedCity` `AppDelegate` ，並在使用者從集合視圖中進行選取時重新顯示它：
...
其 `CanSelect` 屬性設為的任何城市 `false` 都會顯示為灰色，使用者將無法對其設定焦點。 當使用者反白顯示專案時（將其設為焦點），會顯示標題，而且可以使用視差效果奧妙以3D 傾斜影像。
