...
- 再次**使用標準控制項來進行流覽**，以建立簡單且熟悉的使用者體驗，盡可能使用內建 `UIKit` 控制項，例如頁面控制項、索引標籤列、分段控制項、表格視圖、集合視圖，以及應用程式導覽的分割視圖。 由於使用者已熟悉這些元素，因此可以直覺地流覽您的應用程式。
...
- **使用內建的 UI 控制項來進行動作效果**-藉由 `UIKit` 在使用者介面中使用和焦點 API，焦點模型會自動將預設的動作和視覺效果套用至您的 UI 元素。 這讓您的應用程式對 Apple TV 平臺的使用者具有原生和熟悉性，並允許在可設定的專案之間進行流暢且直覺的移動。
...
- **請確定焦點的專案是顯而易見**的，因為您的使用者會從 afar 與您的 UI 元素互動，所以目前的焦點專案就很重要。這通常會由內建元素自動處理 `UIKit` 。 若為自訂控制項，請使用 [專案大小] 或 [陰影] 之類的功能來顯示焦點。
- **使用視差讓焦點專案變得快速回應**-小型、Siri 遠端的迴圈手勢會以溫和、即時的方式移動焦點專案。 這種[視差效果](#Focus-and-Parallax)內建在 `UIKit` _分層影像_中，讓使用者有權連接到焦點的專案。
...
有時候，您可能會想要建立自訂控制項，使其成為可設定焦點的專案。 如果覆寫 `CanBecomeFocused` 屬性並傳回，則傳回 `true` ，否則傳回 `false` 。 例如：
...
您隨時都可以使用控制項的 `Focused` 屬性 `UIKit` ，查看它是否為目前的專案。 如果 `true` UI 專案目前具有焦點，則為，否則為。 例如：
...
雖然您無法透過程式碼直接將焦點移至另一個 UI 專案，但您可以在載入螢幕時，將其屬性設定為，以指定哪個 UI 元素會先取得焦點 `PreferredFocusedView` `true` 。 例如：
...
針對從或繼承的自訂元素 `UIView` `UIViewController` ，您可以覆寫數個方法來使用焦點更新事件：
...
> [!IMPORTANT]
> `SetNeedsUpdateFocus`只有當呼叫的視圖控制器包含目前具有焦點的視圖時，才會發出呼叫的效果。
...
首先， `UIFocusGuide` 會使用方法建立新的，並將其加入至視圖的版面配置指南集合 `AddLayoutGuide` 。
...
首先，此 `NextFocusedView` 程式碼會從傳入的 `UIFocusUpdateContext` （ `context` ）取得。 如果此視圖為 `null` ，則不需要處理，且方法會結束。

接下來， `nextFocusableItem` 會評估。 如果它符合 [**更多資訊**] 或 [**購買**] 按鈕，則會使用焦點指南的屬性，將焦點傳送至相反的按鈕 `PreferredFocusedView` 。 例如：
...
在決定是否可以在或中具有可設定焦點的專案時 `UICollectionView` `UITableView` ，您會分別覆寫或的 `UICollectionViewDelegate` 方法 `UITableViewDelegate` 。 例如：
...
`CanFocusItem` `true` 如果目前的專案可以是焦點，方法會傳回，否則會傳回 `false` 。

如果您想要在最後一個專案遺失並重新取得 `UICollectionView` `UITableView` 焦點時，或將焦點還原到其上，請將 `RemembersLastFocusedIndexPath` 屬性設定為 `true` 。
...
`UIImageView`控制項（或繼承自的任何控制項 `UIImageView` ）會自動支援視差效果。 根據預設，這項支援已停用，若要加以啟用，請使用下列程式碼：
