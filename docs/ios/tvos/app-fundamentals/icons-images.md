...
3. 按一下 [ **1X APPLE TV** ] 專案, 然後選取 [啟動] 影像, 或選擇性地從檔案系統中拖曳新的影像: 
    ...
    [![](icons-images-images/asset03.png "選取啟動映射")](icons-images-images/asset03.png#lightbox)
4. 儲存您的變更。
...
[![](icons-images-images/layered01.png "分層影像 Z 排序圖表")](icons-images-images/layered01.png#lightbox)
...
- **讓背景圖層不透明**-您的背景層 (圖層 1)**必須**是不透明的, 否則當您嘗試在 Apple 電視上使用多層式映射時, 您會收到錯誤。 所有其他圖層都可以包含多個透明度層級, 以增強3D 效果。
...
可能的話, 您應該一律使用內`UIKit`建控制項來顯示分層影像, 因為它們會在焦點出現時自動取得視差效果。
...
[![](icons-images-images/layered02.png "35圖元框線")](icons-images-images/layered02.png#lightbox)
...
- **LSR 影像**-這是 Apple 所建立的專屬影像格式。 使用[視差匯出工具 Adobe Photoshop 外掛程式](https://itunespartner.apple.com/assets/downloads/ParallaxExporter_Apps.zip)或[視差預覽](http://itunespartner.apple.com/assets/downloads/Parallax%20Previewer.dmg)程式, 以 LSR 格式來建立及預覽分層影像。
- **Assets.xcassets** -包含在資產目錄中的兩個 (2) 到五`.png`個 (5) 標準格式化影像, 會在編譯時期編譯成汽車或 LSR 格式化的分層影像。
- **LCR**檔案-這是 Apple 所建立的專屬檔案格式。 LCR 檔案主要是用來作為從您的其中一個內容伺服器下載的其他內容。 LCR 檔案永遠不會包含在您的應用程式套件組合中。 LCR 檔案是使用 Xcode 隨附的命令列工具, `layerutil`從 LSR 或 Photoshop 檔案產生。

<a name="The-Parallax-Previewer" />

### <a name="the-parallax-previewer"></a>視差預覽器

Apple 已建立[視差](http://itunespartner.apple.com/assets/downloads/Parallax%20Previewer.dmg)預覽程式來預覽並建立應用程式圖示和選擇性可設定焦點專案所需的多層式映射。 預覽程式會顯示形成已完成分層影像的每個圖層:

[![](icons-images-images/layered03.png "視差預覽器")](icons-images-images/layered03.png#lightbox)

預覽分層影像時, 您可以使用滑鼠來旋轉影像並預覽視差效果。 使用 **+** （加號） 及 **-** （減號） 按鈕來新增和移除的圖層。
...
1. 在 **方案總管**中, 按兩下`Assets.xcassets`以開啟它進行編輯: 
    ...
    [![](icons-images-images/asset01.png "資產. assets.xcassets fileg")](icons-images-images/asset01.png#lightbox)
2. 在 [**資產編輯器**] 中, `App Icon & Top Shelf Image`展開資產: 

    [![](icons-images-images/asset04.png "展開頂端的貨位影像資產")](icons-images-images/asset04.png#lightbox)
3. 接下來, 展開`App Icon - Small`資產: 

    [![](icons-images-images/asset05.png "展開應用程式圖示-小型資產")](icons-images-images/asset05.png#lightbox)
4. 然後展開`Back`資產, 並按一下該`Contents`專案: 

    [![](icons-images-images/asset06.png "然後展開後端資產")](icons-images-images/asset06.png#lightbox)
5. 按一下 [ **1X APPLE TV] 專案**, 然後選取影像檔案。
6. 針對`Front` 和`Middle`資產重複上述步驟。
...
[![](icons-images-images/topshelf01.png "主要貨位影像範例")](icons-images-images/topshelf01.png#lightbox)

您可以將最上層映射當做單一`.png`靜態或`.lsr`檔案提供 (請參閱[建立分層映射](#Creating-Layered-Images)), 或在執行時間動態建立為可設定焦點的專案的單一資料列 (請參閱下面的[動態主要貨位內容](#Dynamic-Top-Shelf-Content))。
...
2. 在 [**資產編輯器**] 中, `App Icon & Top Shelf Image`展開資產: 
    ...
    [![](icons-images-images/asset04.png "展開頂端的貨位影像資產")](icons-images-images/asset04.png#lightbox)
3. `Top Shelf Image`按一下資產: 

    [![](icons-images-images/asset07.png "最高的貨位影像資產")](icons-images-images/asset07.png#lightbox)
...
您可以使用靜態`.png`或分層`.lsr`檔案的方式, 來提供滾動內凹的橫幅。
...
- **排行榜圖案**-您必須為應用程式支援的每個排行榜, 提供一 (1) 到三 (3) 16:9 的外觀比例影像。 這些可能是靜態`.png`或多層`.lsr`檔案。 排行榜的作品可獲得焦點。
...
1. 在 **方案總管**中, 按兩下`Info.plist`以開啟它進行編輯: 
    ...
    [![](icons-images-images/info01.png "Plist 檔案")](icons-images-images/info01.png#lightbox)
2. 在 [ **Plist 編輯器**] 中, 選取**應用程式圖示**的 [資產] 類別目錄 (在 [[設定應用程式圖示](#Setting-the-App-Icons)] 區段中為上方所設定): 

    [![](icons-images-images/info02.png "Plist 編輯器")](icons-images-images/info02.png#lightbox)
