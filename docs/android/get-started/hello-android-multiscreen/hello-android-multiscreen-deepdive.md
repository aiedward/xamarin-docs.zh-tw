...
_在此由兩部分組成的指南中,基本電話文字應用程式(在 Hello,Android 指南中創建)將展開以處理第二個螢幕。在此過程中,介紹了基本的 Android 應用程式構建基塊。深入瞭解 Android 體系結構,以説明您更好地瞭解 Android 應用程式結構和功能。_
...
應用程式區塊形成了 Android 應用程式的骨幹，因為它們允許您執行通常無法使用一般類別完成的動作。 最重要的兩項區塊是「活動」__ 和「服務」__：
...
在 Phoneword 應用程式中，意圖用來啟動第二個螢幕 (活動)。 首先建立一個意圖,在目前*內容*傳遞`this`(,參考目前**內容的文字**)與要尋找的應用程式區塊的`TranslationHistoryActivity`型態 ( :
...
**字串資源** &ndash; 在 Phoneword 應用程式中，`TranslationHistoryButton` 的文字設為 `"@string/translationHistory"`。 `@string` 語法表示字串值會儲存在「字串資源檔」__**Strings.xml** 中。 `translationHistory` 字串的下列值已新增至 **Strings.xml**：
