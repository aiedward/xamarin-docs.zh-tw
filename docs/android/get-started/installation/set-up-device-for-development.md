...
在 Android 模擬器上測試後，建議您在 Android 裝置上查看並測試您的應用程式。 您必須啟用偵測，並將裝置連線到電腦。

這些步驟每一個都會在下列各節中更詳細地說明。

## <a name="enable-debugging-on-the-device"></a>啟用裝置上的偵錯

必須啟用裝置的偵錯工具，才能測試 Android 應用程式。 自4.2 版起，Android 上的開發人員選項預設為隱藏，而啟用這些選項可能會因 Android 版本而異。

### <a name="android-90"></a>Android 9.0 +

針對 Android 9.0 和更新版本，請遵循下列步驟來啟用偵錯工具：

1. 移至 [設定] 畫面。
2. 選取 [  **關於電話** ]。
3. **在您現在為開發人員之前，請先按****組建編號** 7 次！ 是可見的。
...
2. 選取 [ **系統**]。
3. 選取 [**關於電話**]
4. **在您現在為開發人員之前，請先按****組建編號** 7 次！ 是可見的。
...
在裝置上啟用開發人員模式之後，您必須確定已在裝置上啟用 USB 偵錯工具。 這也會因 Android 版本而異。
...
[![Android 上的開發人員選項畫面](set-up-device-for-development-images/usb-debugging-sml.png)](set-up-device-for-development-images/usb-debugging.png#lightbox)

這是要啟用開發人員選項的地方，例如 USB 偵錯，以及保持清醒模式。

## <a name="connect-the-device-to-the-computer"></a>將裝置連線到電腦

最後一個步驟是將裝置連接到電腦。 最簡單且最可靠的方式就是透過 USB 來這麼做。

如果您之前未使用電腦來進行偵錯工具，您將會收到信任裝置的提示。 您也可以選取 [ **永遠允許從此電腦** 進行]，以避免每次連接裝置時都需要此提示。
...
您可以透過 WiFi 將 Android 裝置連接到電腦，而不需要使用 USB 纜線。 這項技術需要更多的工作，但當裝置離電腦太遠而無法透過纜線持續插入時，可能會很有説明。 

### <a name="connecting-over-wifi"></a>透過 WiFi 連接

[Android Debug Bridge](https://developer.android.com/tools/help/adb.html) (*ADB*) 預設已設定為透過 USB 與 Android 裝置進行通訊。 可以將其重新設定為使用 TCP/IP，而不是 USB。 若要這樣做，裝置和電腦都必須在相同的 WiFi 網路上。 若要設定您的環境以透過 WiFi 進行偵錯工具，請從命令列完成下列步驟：

1. 判斷 Android 裝置的 IP 位址。 找出 IP 位址的其中一種方法，就是在 [ **設定] > Network & internet > wi-fi**，然後點一下裝置所連線的 WiFi 網路，再按一下 [ **Advanced**]。 這會開啟下拉式清單，顯示網路連線的相關資訊，類似于下列螢幕擷取畫面所示：
    ...
    在某些版本的 Android 上，IP 位址不會列在那裡，但可以在 [ **設定] > [關於電話 > 狀態**] 下找到。
...
5. 設定 ADB，讓它透過您在上述步驟 1 中所指定的連接埠連接到 Android 裝置：
    ...
    當您完成透過 WiFi 進行的偵錯工具時，您可以使用下列命令將 ADB 重設回 USB 模式：
    
    ```command
    adb usb
    ```
    
    您可以要求 ADB 列出連接到電腦的裝置。 不論裝置連接的方式為何，您都可以在命令提示字元發出下列命令以查看連接的裝置：
...
在某些情況下，您可能會發現裝置無法連線到電腦。 在此情況下，您可能會想要確認是否已安裝 USB 驅動程式。

## <a name="install-usb-drivers"></a>安裝 USB 驅動程式

MacOS 不需要此步驟;只需使用 USB 纜線將裝置連線到 Mac。
...
2. 在電腦上，以滑鼠右鍵按一下您的桌面或 Windows 檔案總管，然後選取 [ **管理** ]。
...
6. 選取 **[流覽電腦上的驅動程式軟體]** ，然後按一下  **[下一步** ]。

7. 按一下 [ **流覽]** ，並找出 (Google USB 驅動程式的 USB 驅動程式資料夾，位於 **[Android SDK 安裝路徑] \extras\google\ usb_driver**) 。
