...
ms.locfileid: "91454321"
---
# <a name="xamarin-for-java-developers"></a>適合 Java 開發人員的 Xamarin

_如果您是 JAVA 開發人員，您可以在 Xamarin 平臺上運用您的技能和現有程式碼，同時享受 c # 的程式碼重複使用優勢。您會發現 c # 語法與 JAVA 語法非常類似，而且這兩種語言都提供非常類似的功能。此外，您將探索 c # 特有的功能，讓您的開發工作更容易。_
...
- 在 Android 上使用的 JAVA () 不支援隱含類型的區域變數 (c # 支援 `var` 關鍵字) 。
...
- [關鍵字差異](#keywords) 可提供實用關鍵字對等專案的表格、僅限 c # 的關鍵字，以及 c # 關鍵字定義的連結。
...
- [非同步程式設計](#async) &ndash;C # (的非同步程式設計功能 `async` / `await`) 讓應用程式保持回應。
...
> [!NOTE]
> Android 程式設計會使用支援所有 JAVA 7 功能 [和 java 8 子集](https://developer.android.com/studio/write/java8-support.html)的特定 java 語言版本。
>
> 此頁面上所提及的部分功能 (例如 `var` c # 中的關鍵字（例如 c ) # 中的關鍵字）適用于較新版本的 java (例如[ `var` 在 java 10](https://developer.oracle.com/java/jdk-10-local-variable-type-inference.html)) 中，但 Android 開發人員仍無法使用。
...
### <a name="libraries-vs-assemblies"></a>程式庫與元件
...
如需組件的詳細資訊，請參閱[組件與全域組件快取](/dotnet/csharp/programming-guide/concepts/assemblies-gac/)主題。
...
如需泛型的詳細資訊，請參閱[泛型](/dotnet/csharp/programming-guide/generics/index)主題。
...
當您想要在 c # 中防止類別進一步進行子類別化時，請在類別名稱前面加上 `sealed` &ndash; JAVA，並在類別名稱前面加上 `final` 。

如需 C# 類別定義的詳細資訊，請參閱[類別](/dotnet/csharp/programming-guide/classes-and-structs/classes)與[繼承](/dotnet/csharp/programming-guide/classes-and-structs/inheritance)主題。
...
`rulerView`物件也有一個稱為範例程式碼的屬性，會 `DrawingCacheEnabled` &ndash; 將這個屬性設定為，以 `true` 啟用中的繪圖 `rulerView` 快取。 在幕後，會使用新值來更新相關聯的隱藏欄位，而且可能會修改 `rulerView` 狀態的其他層面。 例如，將 `DrawingCacheEnabled` 設為 `false` 時，`rulerView` 可能也會清除物件中已累積的任何繪圖快取資訊。

屬性的存取權可以是讀取/寫入、唯讀或唯寫的權限。 此外，您還能使用不同的存取修飾詞進行讀取和寫入。 例如，您可以定義具有公用讀取權限但具有私用寫入權限的屬性。

如需 C# 屬性的詳細資訊，請參閱[屬性](/dotnet/csharp/programming-guide/classes-and-structs/properties)主題。
...
- **`protected internal`**&ndash;類別成員可在定義元件、定義類別及衍生類別中存取， (元件內部和外部的衍生類別具有存取) 。

如需 C# 存取修飾詞的詳細資訊，請參閱[存取修飾詞](/dotnet/csharp/programming-guide/classes-and-structs/access-modifiers)主題。
...
如需對於多型的 C# 支援詳細資訊，請參閱[多型](/dotnet/csharp/programming-guide/classes-and-structs/polymorphism)主題。
...
在此範例中，Lambda 運算式程式碼 (大括號內的程式碼) 會遞增點按計數，並更新 `button` 文字來顯示點按計數。 這個 Lambda 運算式會使用 `button` 物件註冊，以作為每次點選按鈕時要呼叫的 Click 事件處理常式 以下將詳細說明 (事件處理常式 ) 。在這個簡單的範例中， `sender` `args` lambda 運算式程式碼不會使用和參數，但 lambda 運算式中必須要有這些參數，才能符合事件註冊的方法簽章需求。 背後的原理是，C# 編譯器會將 Lambda 運算式轉譯為匿名方法，每次發生按鈕 Click 事件時即會呼叫此方法。

如需 C# 和 Lambda 運算式的詳細資訊，請參閱 [Lambda 運算式](/dotnet/csharp/programming-guide/statements-expressions-operators/lambda-expressions)主題。
...
如需委派的詳細資訊，請參閱[委派](/dotnet/csharp/programming-guide/delegates/index)主題。
...
如需 C# 中 `async`/`await` 的簡介，請參閱[使用 Async 和 Await 進行非同步程式設計](/dotnet/csharp/async)主題。
...
|Java|C#|描述|
|---|---|---|
|`boolean`|[bool](/dotnet/csharp/language-reference/keywords/bool)|用來宣告布林值的真偽。|
|`extends`|`:`|優先要從中繼承的類別和介面。|
|`implements`|`:`|優先要從中繼承的類別和介面。|
|`import`|[使用](/dotnet/csharp/language-reference/keywords/using)|從命名空間匯入類型，也可用來建立命名空間別名。|
|`final`|[sealed](/dotnet/csharp/language-reference/keywords/sealed)|防止類別衍生；防止方法和屬性在衍生類別中遭到覆寫。|
|`instanceof`|[是](/dotnet/csharp/language-reference/keywords/is)|評估物件是否可與指定的類型相容。|
|`native`|[extern](/dotnet/csharp/language-reference/keywords/extern)|宣告在外部實作的方法。|
|`package`|[命名 空間](/dotnet/csharp/language-reference/keywords/namespace)|宣告一組相關物件的範圍。|
|`T...`|[params T](/dotnet/csharp/language-reference/keywords/params)|指定採用可變數目之引數的方法參數。|
|`super`|[base](/dotnet/csharp/language-reference/keywords/base)|用來存取衍生類別中父類別的成員。|
|`synchronized`|[鎖](/dotnet/csharp/language-reference/keywords/lock-statement)|包裝含有鎖定取得和釋放的重要程式碼區段。|

此外，有許多對 c # 而言是唯一的關鍵字，在 Android 上使用的 JAVA 沒有對應的關鍵字。 Xamarin.Android 程式碼通常會使用下列 C# 關鍵字 (當您透過[範例程式碼](/samples/browse/?products=xamarin&term=Xamarin.Android) \(英文\) 讀取時非常適合參考此表格)：

|C#|描述|
|---|---|
|[按照](/dotnet/csharp/language-reference/keywords/as)|在可相容的參考類型或可為 Null 的類型之間執行轉換。|
|[async](/dotnet/csharp/language-reference/keywords/async)|指定方法或 Lambda 運算式為非同步。|
|[await](/dotnet/csharp/language-reference/keywords/await)|暫停執行方法，直到工作完成為止。|
|[byte](/dotnet/csharp/language-reference/keywords/byte)|不帶正負號的 8 位元整數類型。|
|[delegate](/dotnet/csharp/language-reference/keywords/delegate)|用來封裝方法或匿名方法。|
|[枚舉](/dotnet/csharp/language-reference/keywords/enum)|宣告列舉，一組具名常數。|
|[event](/dotnet/csharp/language-reference/keywords/event)|宣告發行者類別中的事件。|
|[固定](/dotnet/csharp/language-reference/keywords/fixed-statement)|防止變數遭到重新配置。|
|`get`|定義可擷取屬性值的存取子方法。|
|[in](/dotnet/csharp/language-reference/keywords/in-generic-modifier)|讓參數能夠接受泛型介面中較少衍生的類型。|
|[object](/dotnet/csharp/language-reference/keywords/object)|.NET Framework 中 Object 型別的別名。|
|[out](/dotnet/csharp/language-reference/keywords/out)|參數修飾詞或泛型類型參數宣告。|
|[override](/dotnet/csharp/language-reference/keywords/override)|擴充或修改繼承成員的實作。|
|[部分](/dotnet/csharp/language-reference/keywords/partial-method)|宣告要分割成多個檔案的定義，或從它的實作分割方法定義。|
|[readonly](/dotnet/csharp/language-reference/keywords/readonly)|宣告類別成員只能在宣告期間或透過類別建構函式加以指派。|
|[ref](/dotnet/csharp/language-reference/keywords/ref)|導致引數會以傳址方式傳遞，而不是依值傳遞。|
|[set](/dotnet/csharp/language-reference/keywords/set)|定義可設定屬性值的存取子方法。|
|[string](/dotnet/csharp/language-reference/keywords/string)|.NET Framework 中 String 型別的別名。|
|[結構](/dotnet/csharp/language-reference/keywords/struct)|封裝相關變數群組的實值類型。|
|[typeof](/dotnet/csharp/language-reference/keywords/typeof)|取得物件類型。|
|[無 功](/dotnet/csharp/language-reference/keywords/var)|宣告隱含類型的區域變數。|
|[value](/dotnet/csharp/language-reference/keywords/value)|參考用戶端程式碼想要指派給屬性的值。|
|[virtual](/dotnet/csharp/language-reference/keywords/virtual)|允許在衍生類別中覆寫方法。|
...
MSDN [C# 程式設計手冊](/dotnet/csharp/programming-guide/)是開始學習 C# 程式設計語言的絕佳方法，而您可以使用 [C# 參考](/dotnet/csharp/language-reference/)來查閱特定的 C# 語言功能。
...
- [C # 程式設計指南](/dotnet/csharp/programming-guide/)
