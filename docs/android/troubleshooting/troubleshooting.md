...
在所有訊息中， *grefc*值是 xamarin 所建立的全域參考計數，而*Grefwc*值是 xamarin 所建立的弱式全域參考計數。 *控制碼*或*obj 控制碼*值是 JNI 控制碼值，而 ' */* ' 後面的字元是控制碼值的類型： */l*代表本機參考， */g*代表全域參考，而適用于弱式全域的 */w*提到.
...
為協助您處理這個問題，請移至 Visual Studio 並變更 MSBuild 詳細資訊層級，若要這樣做，請選取 **工具 > 選項 > 專案**和**方案 > 組建**並**執行 > MSBuild 專案組建輸出詳細**資訊並加以設定值設為**Normal**。
...
問題在於，Xamarin 不正確地封送處理嵌套的泛型型別。 `List<IDictionary<string, object>>` 會封送處理至[ArrrayList](xref:Java.Util.ArrayList)，但 `ArrayList` 包含 `mono.android.runtime.JavaObject` 實例（參考 `Dictionary<string, object>` 實例），而不是執行[util](xref:Java.Util.IMap)的專案，因此會產生下列內容異常
...
一般來說，有用的解決方案是找出哪些類型有太多配置 &ndash; Android 的實例。在上述傾印中，請 &ndash;，然後在您的原始程式碼中找出它們的建立位置，並適當地處置它們（使其 JAVA 物件存留期會縮短）。 這不一定適合（\#685215 是多執行緒，因此，簡單的解決方案會避免處置呼叫），但這是要考慮的第一件事。
