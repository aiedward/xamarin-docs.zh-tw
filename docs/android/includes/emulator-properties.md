...
|`disk.dataPartition.path`|**資料磁碟分割的路徑** &ndash; 指定使用者資料磁碟分割檔案。 若要設定持續性使用者資料檔案，請輸入檔案名稱和您開發電腦上的路徑。 如果該檔案不存在，模擬器就會從預設檔案 **userdata.img** 建立映像、將它儲存在 `disk.dataPartition.path` 所指定的檔案名稱中，然後在模擬器關閉時，將使用者資料保存至該檔案。 如果您未指定路徑，預設檔案會命名為 **userdata-qemu.img**。 特殊值 **<temp>** 會使得模擬器建立並使用暫存檔。 若已設定 `disk.dataPartition.initPath`，其內容在開機階段將會被複製到 `disk.dataPartition.path` 檔案。 請注意，這個選項不能留白。||
