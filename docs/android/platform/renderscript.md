...
2. **來自 Android Framework 的受控包裝**函式 &ndash; 受控類別，可讓 android 應用程式控制 Renderscript 執行時間和腳本並與之互動。 除了架構提供用來控制 Renderscript 執行時間的類別之外，Android 工具鏈也會檢查 Renderscript 的原始程式碼，並產生可供 Android 應用程式使用的受控包裝函式類別。
...
1. **內容**&ndash; Android SDK 所提供的受控 API，可將資源配置給 Renderscript，並允許 Android 應用程式傳遞和接收來自 Renderscript 的資料。

2. **_計算核心_** &ndash; 也稱為_根核心_或_核心_，這是執行工作的常式。 核心與 C 函數非常類似;它是一個可並行常式，會在配置記憶體中的所有資料上執行。
...
- **簡單類型**&ndash; 在概念上，這與 C 資料類型相同，`float` 或 `char`。

- &ndash; 此型別的**複雜型**別類似于 C `struct`。
...
- **`ScriptC_XXXXX`** &ndash; 也稱為_使用者腳本_，這些是由開發人員撰寫並封裝在 APK 中的腳本。 在編譯時期，Android 工具鏈會產生 managed 包裝函式類別，以允許在 Android 應用程式中使用腳本。
  這些產生的類別名稱是 Renderscript 檔案的名稱，前面加上 `ScriptC_`。 除了本指南的討論範圍之外，Xamarin 不會正式支援撰寫和併入使用者腳本。

在這兩種類型中，只有 `StringIntrinsic` 受到 Xamarin 的支援。 本指南將討論如何在 Xamarin Android 應用程式中使用內部腳本。
...
- [ScriptIntrinsicBLAS](https://developer.android.com/reference/android/renderscript/ScriptIntrinsicBLAS.html) &ndash; Provideshigh Performance Renderscript Api to [BLAS](http://www.netlib.org/blas/)。 BLAS （基本線性代數 Subprograms）是提供標準建立組塊來執行基本向量和矩陣作業的常式。 

- [ScriptIntrinsicBlend](xref:Android.Renderscripts.ScriptIntrinsicBlend) &ndash; 會同時混合兩個配置。

- [ScriptIntrinsicBlur](xref:Android.Renderscripts.ScriptIntrinsicBlur) &ndash; 會將高斯模糊套用至配置。

- [ScriptIntrinsicColorMatrix](xref:Android.Renderscripts.ScriptIntrinsicColorMatrix) &ndash; 會將色彩矩陣套用至配置（亦即變更 colours、調整色調）。

- [ScriptIntrinsicConvolve3x3](xref:Android.Renderscripts.ScriptIntrinsicConvolve3x3) &ndash; 會將3x3 色彩矩陣套用至配置。

- [ScriptIntrinsicConvolve5x5](xref:Android.Renderscripts.ScriptIntrinsicConvolve5x5) &ndash; 會將5x5 色彩矩陣套用至配置。

- [ScriptIntrinsicHistogram](xref:Android.Renderscripts.ScriptIntrinsicHistogram) &ndash; 內部長條圖篩選準則。

- [ScriptIntrinsicLUT](xref:Android.Renderscripts.ScriptIntrinsicLUT) &ndash; 會將每個通道的查閱資料表套用至緩衝區。

- [ScriptIntrinsicResize](xref:Android.Renderscripts.ScriptIntrinsicResize) &ndash; 腳本來執行2d 配置的調整大小。

- [ScriptIntrinsicYuvToRGB](xref:Android.Renderscripts.ScriptIntrinsicYuvToRGB) &ndash; 會將 YUV 緩衝區轉換成 RGB。

如需每個內建腳本的詳細資料，請參閱 API 檔。

接下來會說明在 Android 應用程式中使用 Renderscript 的基本步驟。

&ndash; [`Renderscript`](xref:Android.Renderscripts.RenderScript) **建立 Renderscript 內容**
...
**建立**配置 &ndash; 視內建腳本而定，您可能需要建立一或兩個 `Allocation`s。 [`Android.Renderscripts.Allocation`](xref:Android.Renderscripts.Allocation)
...
通常，您必須建立 `Allocation` 來保存腳本的輸出資料。 下列程式碼片段示範如何使用 `Allocation.CreateTyped` helper 來具現化與原始類型相同的第二個 `Allocation`：

```csharp
Android.Renderscripts.Allocation outputAllocation = Allocation.CreateTyped(renderScript, inputAllocation.Type);
```

具現**化腳本包裝**函式 &ndash; 每個內建腳本包裝函式類別都應該要有 helper 方法（通常稱為 `Create`），以具現化該腳本的包裝函式物件。 下列程式碼片段是如何具現化 `ScriptIntrinsicBlur` 模糊物件的範例。 `Element.U8_4` helper 方法會建立一個專案，描述屬於8位不帶正負號整數值4個欄位的資料類型，適合用來保存 `Bitmap` 物件的資料：

```csharp
Android.Renderscripts.ScriptIntrinsicBlur blurScript = ScriptIntrinsicBlur.Create(renderScript, Element.U8_4(renderScript));
```

**指派配置、設定參數、& 執行腳本**&ndash; `Script` 類別提供 `ForEach` 方法來實際執行 Renderscript。 這個方法會逐一查看包含輸入資料之 `Allocation` 中的每個 `Element`。 在某些情況下，可能需要提供保留輸出的 `Allocation`。
