...
Android 可讓您存取各種不同的位置技術，例如儲存格塔式位置、Wi-fi 和 GPS。 每個位置技術的詳細資料都是透過*位置提供者*來抽象化，讓應用程式以相同的方式取得位置，而不論所使用的提供者為何。 本指南介紹「融合的位置提供者」，這是 Google Play Services 的一部分，可根據可用的提供者和裝置的使用方式，以智慧方式判斷取得裝置位置的最佳方法。 Android 位置服務 API，並示範如何使用 `LocationManager`與系統位置服務通訊。 本指南的第二個部分會使用 `LocationManager`來探索 Android 定位服務 API。
...
- Gps**提供者**&ndash; gps 可提供最精確的位置、使用最多的功能，以及在戶外的運作效果最佳。 此提供者會使用 GPS 和輔助 GPS （[aGPS](https://en.wikipedia.org/wiki/Assisted_GPS)）的組合，這會傳回行動電話塔所收集的 GPS 資料。

- **網路提供者**&ndash; 提供 WiFi 和行動電話資料的組合，包括由資料格塔所收集的 aGPS 資料。 其使用的功能比 GPS 提供者少，但會傳回不同精確度的位置資料。

- **被動提供者**會使用其他應用程式或服務所要求的提供者，&ndash; 「可攜帶」選項，以在應用程式中產生位置資料。 這是較不可靠但省電選項，適用于不需要常數位置更新才能正常執行的應用程式。

位置提供者不一定可供使用。 例如，我們可能會想要針對我們的應用程式使用 GPS，但 GPS 可能會在 [設定] 中關閉，或者裝置可能完全沒有 GPS。 如果特定提供者無法使用，選擇該提供者可能會傳回 `null`。

### <a name="location-permissions"></a>位置許可權

有位置感知的應用程式需要存取裝置的硬體感應器，才能接收 GPS、Wi-fi 和行動資料。 存取權是透過應用程式的 Android 資訊清單中的適當許可權來控制。
有兩個可用的許可權 &ndash; 視您的應用程式需求和您選擇的 API 而定，您會想要允許一個：

- `ACCESS_FINE_LOCATION` &ndash; 允許應用程式存取 GPS。
    *GPS 提供者*和*被動提供者*選項的必要項（*被動提供者需要許可權才能存取其他應用程式或服務所收集的 GPS 資料*）。 *網路提供者*的選擇性許可權。

- `ACCESS_COARSE_LOCATION` &ndash; 允許應用程式存取行動電話和 Wi-fi 位置。 如果未設定 `ACCESS_FINE_LOCATION`，則為*網路提供者*所需。

針對以 API 第21版（Android 5.0 棒）或更高版本為目標的應用程式，您可以啟用 `ACCESS_FINE_LOCATION`，並繼續在沒有 GPS 硬體的裝置上執行。 如果您的應用程式需要 GPS 硬體，您應該明確地將 `android.hardware.location.gps` `uses-feature` 元素新增至 Android 資訊清單。 如需詳細資訊，請參閱 Android[使用-feature](https://developer.android.com/guide/topics/manifest/uses-feature-element.html)元素參考。
...
設定其中任一許可權會告訴 Android，您的應用程式需要使用者的許可權，才能存取位置提供者。 執行 API 層級22（Android 5.1）或更低版本的裝置，會要求使用者在每次安裝應用程式時授與這些許可權。 在執行 API 層級23（Android 6.0）或更高版本的裝置上，應用程式應該在提出位置提供者的要求之前，先執行執行時間許可權檢查。 

> [!NOTE]
>注意：設定 `ACCESS_FINE_LOCATION` 意味著可以存取粗略和精確的位置資料。 您永遠都不需要設定這兩個許可權，只有應用程式需要的*最低*許可權才能正常執行。

此程式碼片段是一個範例，說明如何檢查應用程式是否具有 `ACCESS_FINE_LOCATION` 許可權的許可權：
...
在 Xamarin Android 應用程式可以使用已融合的位置提供者之前，它必須先將**GooglePlayServices**套件新增至專案。 此外，下列 `using` 語句應新增至任何參考以下所述類別的原始程式檔：
...
若要與融合位置提供者互動，Xamarin. Android 應用程式必須具有 `FusedLocationProviderClient`的實例。 此類別會公開訂閱位置更新的必要方法，以及取得裝置的最後一個已知位置。

活動的 `OnCreate` 方法是取得 `FusedLocationProviderClient`參考的適當位置，如下列程式碼片段所示：
...
`FusedLocationProviderClient.GetLastLocationAsync()` 方法為 Xamarin Android 應用程式提供簡單且非封鎖的方式，以最少的編碼額外負荷，快速取得裝置的最後一個已知位置。
...
- **`Android.Gms.Location.LocationRequest`** &ndash; `LocationRequest` 物件是 Xamarin Android 應用程式如何傳遞已融合位置提供者應該如何使用的參數。 `LocationRequest` 會保留資訊，例如要求的執行頻率，或正確位置更新的重要性。 例如，在判斷位置時，重要的位置要求將會導致裝置使用 GPS，因而更強大。 此程式碼片段示範如何針對具有高精確度的位置建立 `LocationRequest`，並每隔五分鐘檢查一次位置更新（但不會在要求之間的兩分鐘內）。 已融合的位置提供者會使用 `LocationRequest` 做為嘗試判斷裝置位置時所要使用之位置提供者的指導方針：
...
- **`Android.Gms.Location.LocationCallback`** &ndash; 若要接收位置更新，則 Xamarin. Android 應用程式必須將 `LocationProvider` 抽象類別的子類別化。 此類別公開了兩種方法，可供融合位置提供者用來以位置資訊更新應用程式。 下面將更詳細地討論這一點。

若要通知 Xamarin Android 應用程式的位置更新，已融合的位置提供者將會叫用 `LocationCallBack.OnLocationResult(LocationResult result)`。 `Android.Gms.Location.LocationResult` 參數將包含更新位置資訊。

當融合的位置提供者偵測到位置資料的可用性變更時，它會呼叫 `LocationProvider.OnLocationAvailability(LocationAvailability
locationAvailability)` 方法。 如果 `LocationAvailability.IsLocationAvailable` 屬性傳回 `true`，則可假設 `OnLocationResult` 所報告的裝置位置結果是正確的，而且是 `LocationRequest`所需的最新狀態。 如果 `IsLocationAvailable` 為 false，則 `OnLocationResult`不會傳回位置結果。
...
Android 位置服務是舊版的 API，可在 Android 上使用位置資訊。 「位置資料」是由硬體感應器收集，並由系統服務收集，其可在具有 `LocationManager` 類別和 `ILocationListener`的應用程式中存取。

「位置」服務最適合必須在未安裝 Google Play Services 的裝置上執行的應用程式。

「位置服務」是系統所管理的特殊[服務](https://developer.android.com/guide/components/services.html)類型。 系統服務會與裝置硬體互動，而且一律會執行。 若要在我們的應用程式中切入位置更新，我們將使用 `LocationManager` 和 `RequestLocationUpdates` 呼叫，訂閱系統位置服務的位置更新。

若要使用 Android 位置服務來取得使用者的位置，需要執行幾個步驟：

1. 取得 `LocationManager` 服務的參考。
2. 執行 `ILocationListener` 介面，並在位置變更時處理事件。
3. 使用 `LocationManager` 來要求指定提供者的位置更新。 上一個步驟中的 `ILocationListener` 將用來接收來自 `LocationManager`的回呼。
4. 當應用程式不再適合接收更新時，停止位置更新。

### <a name="location-manager"></a>位置管理員

我們可以使用 `LocationManager` 類別的實例來存取系統位置服務。 `LocationManager` 是特殊類別，可讓我們與系統位置服務互動，並對其呼叫方法。 應用程式可以藉由呼叫 `GetSystemService` 並傳入服務類型來取得 `LocationManager` 的參考，如下所示：

```csharp
LocationManager locationManager = (LocationManager) GetSystemService(Context.LocationService);
```

`OnCreate` 是取得 `LocationManager`參考的最佳位置。
將 `LocationManager` 保留為類別變數是個不錯的主意，以便我們可以在活動生命週期的不同點呼叫它。

### <a name="request-location-updates-from-the-locationmanager"></a>從 LocationManager 要求位置更新

一旦應用程式具有 `LocationManager`的參考，它就必須告訴 `LocationManager` 所需的位置資訊類型，以及該資訊的更新頻率。 若要這麼做，請在 `LocationManager` 物件上呼叫 `RequestLocationUpdates`，並傳入一些更新準則和接收位置更新的回呼。 此回呼是必須實作為 `ILocationListener` 介面的型別（本指南稍後會詳細說明）。
...
應用程式從 `LocationManager`要求更新之後，就可以藉由執行[`ILocationListener`](xref:Android.Locations.ILocationListener)介面，從服務接收資訊。 這個介面提供四種方法來接聽位置服務和位置提供者，`OnLocationChanged`。 當使用者的位置變更時，系統會呼叫 `OnLocationChanged`，以便根據要求位置更新時所設定的準則，以符合位置的變更。 
...
為了節省系統資源，應用程式應該儘快取消訂閱位置更新。 `RemoveUpdates` 方法會告訴 `LocationManager` 停止將更新傳送至我們的應用程式。  例如，活動可能會在 `OnPause` 方法中呼叫 `RemoveUpdates`，以便在應用程式的活動不在畫面上時，可以節省電源：
...
上述應用程式會將 GPS 設定為位置提供者。 不過，GPS 可能無法在所有情況下使用，例如裝置為室內或沒有 GPS 接收器。 如果是這種情況，則結果會是提供者的 `null` 傳回。

若要讓您的應用程式在 GPS 無法使用時運作，您可以使用 `GetBestProvider` 方法，在應用程式啟動時要求最佳可用（裝置支援和使用者啟用）位置提供者。 除了傳入特定的提供者之外，您還可以告訴 `GetBestProvider` 提供者的需求，例如， [`Criteria` 物件](xref:Android.Locations.Criteria)的精確度和電源。 `GetBestProvider` 會傳回指定準則的最佳提供者。
