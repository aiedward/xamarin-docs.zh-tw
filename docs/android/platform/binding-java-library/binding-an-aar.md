...
範例應用程式會存取 `TextCounter` textanalyzer 的類別 **。 aar**：
...
此映射資源位於**textanalyzer. aar**中的**res/繪製/monkey.png** 。
...
如果您想要查看系結程式庫專案的內容，您可以按兩下該參考，在 [**物件瀏覽器**中開啟它。 您可以看到命名空間的對應內容 `Com.Xamarin.Textcounter` （從 JAVA `com.xamarin.textanalyzezr` 封裝對應），而且您可以查看類別的成員 `TextCounter` ：

[![查看物件瀏覽器](binding-an-aar-images/11-object-browser-vs-sml.png)](binding-an-aar-images/11-object-browser-vs.png#lightbox)

上述螢幕擷取畫面會反白顯示 `TextAnalyzer` 範例應用程式將會呼叫的兩個方法： `NumConsonants` （包裝基礎 java `numConsonants` 方法）和 `NumVowels` （包裝基礎 java `numVowels` 方法）。

### <a name="accessing-aar-types"></a>正在.AAR 類型

新增指向系結程式庫之應用程式的參考之後，您可以存取中的 JAVA 類型。AAR，如同您存取 c # 型別一樣（感謝 c # 包裝函式）。 C # 應用程式程式碼可以呼叫 `TextAnalyzer` 方法，如下列範例所示：
...
在上述範例中，我們要在類別中呼叫靜態方法 `TextCounter` 。 不過，您也可以具現化類別和呼叫實例方法。 例如，如果您的。AAR 會包裝名為且 `Employee` 具有實例方法的類別 `buildFullName` ，您可以具現化 `MyClass` 並使用它，如下所示：
...
將**MainActivity.cs**的內容取代為下列程式碼。 如本範例所示，按鈕事件處理常式會呼叫 `TextCounter` 位於的已包裝方法。AAR 並使用快顯通知來顯示結果。 請注意系結連結 `using` 庫命名空間的語句（在此案例中為 `Com.Xamarin.Textcounter` ）：
...
若要存取影像資源，請使用**資源。** 在內封裝之影像的可繪製名稱。AAR. 例如，您可以參考中的**image.png** 。使用下列方法 AAR 檔案 `@drawable/image` ：
...
編輯**bindingtest shoppingcart**版面配置（**axml**），並將新增 `ImageView` 至 `LinearLayout` 容器結尾。 這 `ImageView` 會顯示在可** \@ 繪製/猴子**中找到的影像; 此映射將會從**textanalyzer. aar**的 resource 區段載入：
...
編譯並執行**bindingtest shoppingcart**專案。 當您按下 [計數子音] 時，應用程式就會啟動並顯示左側的螢幕擷取畫面 &ndash; ，結果會如右側所示顯示： **COUNT CONSONANTS**
...
在本逐步解說中，我們已建立的系結程式庫。AAR 檔案，將系結程式庫新增至最小測試應用程式，並執行應用程式，以確認我們的 c # 程式碼可以呼叫位於的 JAVA 程式碼。AAR 檔案。
