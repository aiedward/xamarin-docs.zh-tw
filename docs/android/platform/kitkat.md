...
- [硬體](#hardware) &ndash;使用 NFC 主機型插卡模擬，將任何應用程式轉換為 NFC 卡;使用執行低電源感應器 `SensorManager` 。
...
若要讓 Android 在指定的時間間隔內喚醒應用程式，請 `SetWindow` 在上呼叫 [`AlarmManager`](xref:Android.App.AlarmManager) ，並傳入喚醒應用程式之前所經過的最小和最大時間（以毫秒為單位），以及要在喚醒執行的作業。
...
- 如果您是藉由呼叫上的方法取得檔案或目錄路徑 `Context` -例如，[`GetExternalFilesDir`](xref:Android.Content.Context.GetExternalFilesDir*)
  或[`GetExternalCacheDirs`](xref:Android.Content.Context.GetExternalCacheDirs)
  - 您的應用程式不需要額外的許可權。

- 如果您是藉由存取屬性或在上呼叫方法來取得檔案或目錄路徑 `Environment` ，例如[`GetExternalStorageDirectory`](xref:Android.OS.Environment.ExternalStorageDirectory)
...
[Web](xref:Android.Webkit.WebView)工作的 KitKat 中有改造。 最大的變更是新增將內容載入至的安全性 `WebView` 。 雖然大部分以舊版 API 為目標的應用程式應該如預期般運作，但強烈建議使用類別的測試應用程式 `WebView` 。 如需受影響的 Web 工作應用程式開發介面的詳細資訊，請參閱 android 4.4 檔中的 Android[遷移至 web](https://developer.android.com/guide/webapps/migrating.html)工作。
...
上述範例使用轉換架構，在變更的屬性值之間建立自動的預設轉換。 因為動畫是由一行程式碼處理，所以您可以輕鬆地使用舊版的 Android 進行相容性，方法是 `BeginDelayedTransition` 在系統版本檢查中包裝呼叫。 如需詳細資訊，請參閱將[您的應用程式遷移至 KitKat](#Migrating_Your_App_to_KitKat)一節。
...
裝載動態內容的靜態 UI 元素稱為*容器*或*場景基底*。 下列範例會使用 Android Designer 來建立名為 `RelativeLayout` 的 `container` ：

[![使用 Android Designer 建立 RelativeLayout 容器](kitkat-images/container.png)](kitkat-images/container.png#lightbox)

範例配置也會在下方定義名為 `sceneButton` 的按鈕 `container` 。 此按鈕將會觸發轉換。
...
上述範例使用 `merge` ，讓視圖程式碼更短，並簡化視圖階層。 您可以在這裡閱讀更多有關 `merge` 版面配置的資訊。 [here](https://android-developers.blogspot.com/2009/03/android-layout-tricks-3-optimize-by.html)
...
> [!NOTE]
> Android 轉換程式庫中有[已知的 bug](https://code.google.com/p/android/issues/detail?id=62450) ，會導致使用建立的場景在 `GetSceneForLayout` 第二次流覽活動時中斷。 [這裡](http://www.doubleencore.com/2013/11/new-transitions-framework/)會說明 java 因應措施。

##### <a name="custom-transitions-in-scenes"></a>場景中的自訂轉換

自訂轉換可以在下目錄中的 xml 資源檔中定義 `transition` `Resources` ，如下列螢幕擷取畫面所示：
...
- `fitsSystemWindows`-根據預設，將頂端或底部列設定為 [transcluent] 會將內容移至透明 UI 元素下。 將這個屬性設定為， `true` 是防止內容與半透明系統 UI 元素重迭的簡單方式。
...
這項變更在開發人員端需要兩個動作：首先，需要提供者內容的應用程式必須更新為新的要求內容方式。 第二，將資料寫入的應用程式 `ContentProvider` 必須經過修改，才能使用新的架構。 這兩種案例都取決於新的[`DocumentsProvider`](xref:Android.Provider.DocumentsProvider)
API。

#### <a name="documentsprovider"></a>DocumentsProvider

在 KitKat 中，與的互動 `ContentProviders` 會使用 `DocumentsProvider` 類別進行抽象化。 這表示 SAF 不在意資料實際的位置，只要可以透過 API 存取即可 `DocumentsProvider` 。 本機提供者、雲端服務和外部存放裝置全都使用相同的介面，並以相同的方式處理，提供使用者和開發人員一個可與使用者內容互動的位置。

本節涵蓋如何使用儲存體存取架構來載入和儲存內容。

#### <a name="request-content-from-a-provider"></a>從提供者要求內容

我們可以告訴 KitKat，我們想要使用 SAF UI 與意圖來挑選內容 `ActionOpenDocument` ，這表示我們想要連接到裝置可用的所有內容提供者。 您可以藉由指定來對此意圖新增一些篩選 `CategoryOpenable` ，這表示只會傳回可開啟的內容（也就是可存取的可用內容）。 KitKat 也可讓您使用篩選內容 `MimeType` 。 例如，下列程式碼會藉由指定映射來篩選影像結果 `MimeType` ：
...
在使用者選擇影像之後，會傳回所選檔案的 `OnActivityResult` `Android.Net.Uri` 。 下列程式碼範例會顯示使用者的影像選取範圍：
...
除了從 SAF UI 載入內容之外，KitKat 也可讓您將內容儲存至任何 `ContentProvider` 可執行 `DocumentProvider` API 的。 儲存內容會將 `Intent` 與搭配使用 `ActionCreateDocument` ：
...
當使用者按下 [**儲存**] 時， `OnActivityResult` `Android.Net.Uri` 會傳遞新建立之檔案的（可透過存取） `data.Data` 。 Uri 可以用來將資料串流至新檔案：
...
隨著[列印服務](xref:Android.PrintServices)和的引進，KitKat 中的列印內容已經過簡化 `PrintManager` 。 KitKat 也是第一個 API 版本，可使用[Google 雲端列印應用程式](https://play.google.com/store/apps/details?id=com.google.android.apps.cloudprint)完全利用[Google 的雲端列印服務 api](https://developers.google.com/cloud-print/) 。
...
KitKat 會自動 [`PrintDocumentAdapter`](xref:Android.Print.PrintDocumentAdapter) 使用建立 web view 的 `WebView.CreatePrintDocumentAdapter` 。 列印 web 內容是在 [`WebViewClient`](xref:Android.Webkit.WebViewClient) 等候 HTML 內容載入，並讓活動知道在 [選項] 功能表中提供 [列印] 選項，以及 [活動] （等待使用者選取 [列印] 選項，並在上呼叫）之間進行協調的工作 `Print` `PrintManager` 。 本節涵蓋列印螢幕上 HTML 內容所需的基本設定。
...
上述程式碼也會定義名為的變數 `dataLoaded` ，以追蹤 HTML 內容的狀態。 `WebViewClient`當所有內容都已載入時，會將此變數設定為 true，讓活動知道要將 [列印] 功能表項目新增至 [選項] 功能表。

##### <a name="webviewclient"></a>來處理

的工作 `WebViewClient` 是確保中的資料在 [ `WebView` 列印] 選項出現在功能表中之前完全載入，它會使用方法來執行 `OnPageFinished` 。 `OnPageFinished`會接聽 web 內容以完成載入，並告知活動重新建立其 [選項] 功能表 `InvalidateOptionsMenu` ：
...
呼叫會 `Print` 載入列印 UI，其中會列出列印工作的選項。 UI 可讓使用者選擇將 HTML 內容列印或儲存至 PDF，如下列螢幕擷取畫面所示：
...
以主機為基礎的卡片模擬（HCE）可讓應用程式在不依賴電訊廠商專屬安全元素的情況下，行為像是 NFC 卡或 NFC 記憶卡讀卡機。 在設定 HCE 之前，請確定裝置上有 HCE 可供使用 `PackageManager.HasSystemFeature` ：

```csharp
bool hceSupport = PackageManager.HasSystemFeature(PackageManager.FeatureNfcHostCardEmulation);
```

HCE 需要在 `Nfc` 應用程式的中註冊 HCE 功能和許可權 `AndroidManifest.xml` ：
...
若要運作，HCE 必須能夠在背景中執行，而且必須在使用者進行 NFC 交易時啟動，即使使用 HCE 的應用程式不在執行中也一樣。 我們可以撰寫 HCE 程式碼做為來完成這項工作 `Service` 。 HCE 服務會 `HostApduService` 執行介面，它會實作為下列方法：
...
HCE 服務也必須向應用程式的資訊清單註冊，並以適當的許可權、意圖篩選和中繼資料裝飾。 下列程式碼是 `HostApduService` 使用屬性向 Android 資訊清單註冊的範例 `Service` （如需有關屬性的詳細資訊，請參閱 Xamarin[使用 Android 資訊清單](~/android/platform/android-manifest.md)指南）：
...
上述服務提供一種方式讓 NFC 讀取器與應用程式互動，但 NFC 讀取器仍無法得知此服務是否正在模擬需要掃描的 NFC 卡。 為了協助 NFC 讀者識別服務，我們可以為服務指派一個唯一的*應用程式識別碼（輔助）*。 我們會在以屬性註冊的 xml 資源檔中，指定協助，以及與 HCE 服務相關的其他中繼資料 `MetaData` （請參閱上述程式碼範例）。 此資源檔會指定一個或多個輔助篩選器，這是十六進位格式的唯一識別碼字串，其對應于一或多個 NFC 讀取器裝置的輔助：
...
KitKat 透過提供裝置感應器的存取權 [`SensorManager`](xref:Android.Hardware.SensorManager) 。
...
您可以藉 `SensorManager` 由呼叫 `GetSystemService(SensorService)` 並將結果轉換為，來建立 `SensorManager` 。 若要使用 step 計數器，請 `GetDefaultSensor` 在上呼叫 `SensorManager` 。 您可以註冊感應器，並使用的協助來接聽步驟計數的變更[`ISensorEventListener`](xref:Android.Hardware.ISensorEventListener)
介面，如下列程式碼範例所示：
...
`OnSensorChanged`如果當應用程式在前景時更新步驟計數，則會呼叫。 如果應用程式進入背景，或裝置處於睡眠狀態，則 `OnSensorChanged` 不會呼叫; 不過，在呼叫之前，這些步驟仍會繼續計算 `UnregisterListener` 。

請記住，*步驟計數值在註冊感應器的所有應用程式中都是累計*的。 這表示即使您卸載並重新安裝應用程式，以及在 `count` 應用程式啟動時將變數初始化為0，感應器所報告的值仍會保留在註冊感應器時所採取的步驟總數（不論您的應用程式或其他）。 您可以藉由在上呼叫來防止應用程式新增至步驟計數器 `UnregisterListener` `SensorManager` ，如下列程式碼所示：
...
> [!NOTE]
> 雖然步驟偵測和計算的 API 隨附于 KitKat，但並非所有的電話都會與感應器科系。 您可以藉由執行來檢查感應器是否可用 `PackageManager.HasSystemFeature(PackageManager.FeatureSensorStepCounter);` ，或檢查以確定傳回的值 `GetDefaultSensor` 不是 `null` 。
...
- *在自訂間隔同步處理 ContentResolver* -KitKat 會在執行同步處理要求的時間內新增一些變動性。 藉 `ContentResolver` 由呼叫 `ContentResolver.RequestSync` 並傳入，在自訂時間或間隔同步 `SyncRequest` 。

- *區分控制器*-在 KitKat 中，控制器會被指派可透過裝置的屬性存取的唯一整數識別碼 `ControllerNumber` 。 這可讓您更輕鬆地分辨遊戲中的玩家。
