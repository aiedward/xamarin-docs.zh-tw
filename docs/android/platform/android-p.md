...
ms.locfileid: "39351902"
---
# <a name="android-p-preview"></a>Android P 預覽

_如何開始使用 Xamarin.Android 開發 Android 的最新版本的應用程式。_

![](~/media/shared/preview.png)

[Android P Developer Preview](https://developer.android.com/preview/)現在可從 Google。 許多新功能和 Api 可在此版本中，並有許多都是必要的最新的 Android 裝置利用新的硬體功能。

![Android 的 P 英雄影像](android-p-images/01-android-p-logo.png)

這篇文章是結構化，以協助您開始在 Android P preview 開發 Xamarin.Android 應用程式。 它會說明如何安裝必要的更新、 設定 SDK，以及準備的模擬器或裝置上的進行測試。 它也概述 Android P 中的新功能，並提供說明如何使用一些關鍵的 Android P 功能的範例原始程式碼。


## <a name="requirements"></a>需求

需要下列項目在以 Xamarin 為基礎的應用程式中使用 Android P 功能：

-   **Visual Studio** &ndash;如果您使用的 Windows，版本 15.8 Preview 5 或更新版本的 Visual Studio 是必要。  如果您使用 Mac，目前的 Beta 版本的 Visual Studio for Mac 或更新版本是必要的。

-   **Xamarin.Android** &ndash; Xamarin.Android 9.0.0.17 或更新版本必須與 Visual Studio 安裝。

-   **Java Developer Kit** &ndash; Xamarin Android 9.0 開發需要[JDK 8](http://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html) (或者，您可以嘗試的 microsoft 的預覽[OpenJDK](~/android/get-started/installation/openjdk.md))。

-   **Android SDK** &ndash;透過 Android SDK 管理員必須安裝 Android SDK API 28 或更新版本。

## <a name="getting-started"></a>快速入門

若要開始使用 Xamarin.Android 的 Android P，您必須下載並安裝最新的工具和 SDK 封裝，才能建立第一個 Android P 專案：

1. 更新 Visual Studio 15.8 Preview 5 或更新版本。 如果您使用 Visual Studio for Mac 中，切換至 Visual Studio for Mac [Beta](https://docs.microsoft.com/visualstudio/mac/update)通道。

2. 安裝**Android P (API 28)** 或更新版本的套件和工具透過 SDK 管理員。

3. 建立新的 Xamarin.Android 專案目標 Android P (API 28)。

4. 設定模擬器或裝置上的進行測試 Android P 應用程式。
...
若要新增至 Visual Studio 的 Android P 支援，請更新為 Visual Studio 2017 版本 15.8 Preview 5 或更新版本為中所述[Visual Studio 2017 更新至最新版本](https://docs.microsoft.com/visualstudio/install/update-visual-studio)。
若要將 Android P 支援新增至 Visual Studio for Mac 中，更新為最新的 Beta 版本的 Visual Studio 2017 for Mac 中所述[更新 Visual Studio for Mac](https://docs.microsoft.com/visualstudio/mac/update)。


### <a name="install-the-android-sdk"></a>Android SDK 安裝

Xamarin.Android 9.0 建立專案時，您必須先使用 Android SDK 管理員安裝的 SDK 平台**Android P （API 層級 28）** 或更新版本。
...
2. 在右下角，按一下齒輪圖示並選取**存放庫 > Google （不支援）**:
    ...
    [![將存放庫設定為 Google](android-p-images/vs/set-repo-sml.png)](android-p-images/vs/set-repo.png#lightbox)

3. 安裝**Android P**封裝，它會顯示為**Android SDK 平台 28**中**平台** 索引標籤 （如需使用 SDK 管理員的詳細資訊，請參閱 (AndroidSDK Setup)[~/android/get-started/installation/android-sdk.md]): 

    [![安裝 Android P 套件](android-p-images/vs/sdk-manager-sml.png)](android-p-images/vs/sdk-manager.png#lightbox)
...
當您建立 Android 專案時，您必須設定目標 Android P 的版本設定或更新版本。 比方說，若要將專案目標為 Android P，您必須設定您的專案的目標 Android API 層級**Android P (API 28)**。 建議您將您的目標架構層級 API 28 或更新版本。 如需有關設定 Android API 層級的層級的詳細資訊，請參閱[了解 Android API 層級](~/android/app-fundamentals/android-api-levels.md)。


### <a name="configure-a-device-or-emulator"></a>設定裝置或模擬器

如果您使用實體裝置，例如像素，您可以將裝置更新到 Android P 預覽中的指示[Android P Beta 裝置](https://developer.android.com/preview/devices/)。

如果您正使用模擬器，請建立 API 層級 28 使用 x86 為基礎的映像的虛擬裝置。 使用 Android 裝置管理員來建立和管理虛擬裝置的相關資訊，請參閱[管理的虛擬裝置的 Android 裝置管理員](~/android/get-started/installation/android-emulator/device-manager.md)。
如需使用 Android 模擬器來進行測試和偵錯資訊，請參閱[使用 「 Google Android 模擬器進行偵錯](~/android/deploy-test/debugging/android-sdk-emulator/index.md)。



## <a name="new-features"></a>新功能

Android P 導入了各種新功能。 部分新功能被要利用新的硬體功能，提供最新的 Android 裝置，而其他專為進一步強化的 Android 使用者體驗：
...
[![Android 模擬器模擬裁剪](android-p-images/02-example-cutout-sml.png)](android-p-images/02-example-cutout.png#lightbox)

若要管理您的應用程式視窗顯示裁剪的裝置上顯示其內容的方式，Android P 已新增新[LayoutInDisplayCutoutMode](https://developer.android.com/reference/android/view/WindowManager.LayoutParams.html#layoutInDisplayCutoutMode)視窗版面配置屬性。 此屬性可以設為下列值之一：
...
[![範例顯示在模擬器中的空心模式](android-p-images/03-cutout-modes-sml.png)](android-p-images/03-cutout-modes.png#lightbox)
...
[![使用映像的範例通知](android-p-images/04-example-notifications-sml.png)](android-p-images/04-example-notifications.png#lightbox)

下列範例說明如何在 Android P 通知中，包含影像，而且它會示範新的使用方式`Person`類別：
...
Android P 提供支援 IEEE 802.11mc (也稱為_WiFi Round 來回時間_或是_WiFi RTT_)，這可讓應用程式偵測到其中的距離或更多的 Wi-fi 存取點。 使用這項資訊，就可以針對您的應用程式，以善用*室內定位*，精確度為一到兩個計量。 IEEE 801.11mc 硬體支援的 Android 裝置，您的應用程式可以提供導覽功能，例如智慧型設備或透過市集開啟藉由開啟指示的位置為基礎的控制項：

[![使用 WiFi RTT 的室內巡覽的範例](android-p-images/05-wifi-rtt-sml.png)](android-p-images/05-wifi-rtt.png#lightbox)
...
Android P 也包含新[SessionConfiguration](https://developer.android.com/reference/android/hardware/camera2/params/SessionConfiguration.html)可用來協助減少延遲，在初始擷取期間並不需要啟動，並啟動相機資料流的類別。
...
如需有關最新的 Android P 功能的詳細資訊，請參閱[Android P 功能和 Api](https://developer.android.com/preview/features)。


## <a name="behavior-changes"></a>行為變更

當目標 Android 版本設定為 API 層級 28 時，有數個會影響您的應用程式行為，即使您不想要實作上面所述的新功能的平台變更。 這些變更的簡短摘要如下：
...
[AndroidPMiniDemo](https://github.com/xamarin/monodroid-samples/tree/master/android-p/AndroidPMiniDemo)示範如何設定的 Android P 顯示空心模式為 Xamarin.Android 範例應用程式如何使用新`Person`類別，以及如何傳送通知，其中包含映像。


## <a name="summary"></a>總結

這篇文章介紹 Android P 預覽，並說明如何安裝和設定使用 Android P 預覽的最新的工具和適用於 Xamarin.Android 開發的封裝。 它提供的許多功能的範例原始程式碼中 Android P，提供重要功能的概觀。 它包含 API 文件的連結和 Android 開發人員主題，以協助您開始建立應用程式的 Android p。它也以反白顯示的最重要的 Android P 行為變更可能會影響現有的應用程式。


## <a name="related-links"></a>相關連結

- [Android P 開發人員預覽](https://developer.android.com/preview/)
