...
在編譯時期，系統會針對衍生自[活動](xref:Android.App.Activity)的非`abstract` 類別掃描元件，並在其中宣告[`[Activity]`](xref:Android.App.ActivityAttribute)屬性。 然後，它會使用這些類別和屬性來建立資訊清單。 例如，請參考下列程式碼： 
...
這會導致**androidmanifest.xml**不會產生任何內容。 如果您想要產生 `<activity/>` 元素，則需要使用[`[Activity]`](xref:Android.App.Activity) 
自訂屬性： 
...
`[Activity]` 屬性不會影響 `abstract` 類型;`abstract` 類型會被忽略。
...
如果您想要覆寫此預設值，並明確指定活動的名稱，請使用[`Name`](xref:Android.App.ActivityAttribute.Name)屬性： 
...
> [!NOTE]
> 您應該只將 `Name` 屬性用於回溯相容性的原因，因為這類重新命名可能會在執行時間減緩類型查閱的速度。 如果您的舊版程式碼預期活動的預設型別名稱是根據小寫命名空間和類別名稱，請參閱 Android 可呼叫[包裝](https://github.com/xamarin/release-notes-archive/blob/master/release-notes/android/xamarin.android_5/xamarin.android_5.1/index.md#Android_Callable_Wrapper_Naming)函式命名，以取得維護相容性的秘訣。 
...
當您將許可權新增至 Android 資訊清單時（如[將許可權新增至 Android 資訊清單](https://github.com/xamarin/recipes/tree/master/Recipes/android/general/projects/add_permissions_to_android_manifest)中所述），這些許可權會記錄在**Properties/androidmanifest.xml**中。 例如，如果您設定 `INTERNET` 許可權，下列元素會加入**Properties/androidmanifest.xml**中： 
...
Android 資訊清單也提供一種方式，讓您宣告整個應用程式的屬性。 這項作業是透過 `<application>` 元素及其對應的[應用程式](xref:Android.App.ApplicationAttribute)自訂屬性來完成。 請注意，這些是全應用程式（全元件）設定，而不是每個活動設定。 一般來說，您會為整個應用程式宣告 `<application>` 屬性，然後根據每個活動來覆寫這些設定（視需要）。 

例如，下列 `Application` 屬性會新增至**AssemblyInfo.cs** ，以指出應用程式可以被調試、其使用者可讀名稱是**My App**，而且它會使用 `Theme.Light` 樣式做為所有活動的預設主題： 
...
在此範例中，應用程式中的所有活動都會預設為 `Theme.Light` 樣式。 如果您將活動的主題設定為 [`Theme.Dialog`]，則只有該活動會使用 `Theme.Dialog` 樣式，而應用程式中的所有其他活動都會預設為 `<application>` 元素中設定的 `Theme.Light` 樣式。 

`Application` 元素不是設定 `<application>` 屬性的唯一方式。 或者，您也可以將屬性直接插入**Properties/androidmanifest.xml**的 `<application>` 元素中。 這些設定會合並到位於**obj/Debug/android/androidmanifest.xml**中的最後一個 `<application>` 元素。 請注意， **Properties/androidmanifest.xml**的內容一律會覆寫自訂屬性所提供的資料。 
