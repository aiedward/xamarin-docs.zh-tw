...
對於目標為 API 層級最多12（Android 3.1）的應用程式，配置應該放在使用限定詞**小型**/**一般**/**大型**/ **m3.xlarge**的目錄中，做為的歸納大部分裝置中可用的各種螢幕大小。 例如，在下圖中，有四種不同的螢幕大小的替代資源：
...
- **避免**[AbsoluteLayout](xref:Android.Widget.AbsoluteLayout)可能的話，它會在 API 層級3（Android 1.5）中被取代，而且將會產生脆弱配置。 
