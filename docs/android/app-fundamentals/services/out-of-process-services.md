...
1. **共用功能**&ndash; 某些應用程式開發人員可能會有多個應用程式和功能，在所有應用程式之間共用。 在 Android 服務中封裝該功能以在自己的進程中執行，可以簡化應用程式維護。 您也可以在自己的獨立 APK 中封裝服務，並將其與應用程式的其餘部分分開部署。
2. **改善使用者體驗**&ndash; 有兩種可能的方式，跨進程服務可以改善應用程式的使用者體驗。 第一種方式是處理記憶體管理。 當垃圾收集（GC）迴圈發生時，Android 會暫停進程中的所有活動，直到 GC 完成為止。 使用者可能會將此暫停視為「斷斷續續」或「jank」。 當服務在它自己的進程中執行時，它就是暫停的服務程式，而不是應用程式進程。 當應用程式進程（也就是使用者介面）未暫停時，這段暫停時間對使用者來說會比較不明顯。
...
3. **改善應用程式效能**&ndash; UI 進程可以獨立于服務程式之外終止或關閉。 藉由將冗長的啟動工作移至跨進程服務，可能會改善 UI 的啟動時間（假設服務進程在啟動 UI 的時間之間保持運作）。
...
遠端進程中的 `Service` 類別將會經歷相同的生命週期回呼，在本機進程中系結的服務會通過，而涉及的許多 Api 都相同。 `Service.OnCreate` 是用來初始化 `Handler`，並將它插入 `Messenger` 物件中。 同樣地，會覆寫 `OnBind`，但不會傳回 `IBinder` 物件，服務會傳回 `Messenger`。  此圖說明當用戶端系結到服務時，會發生什麼情況：
...
本指南將討論如何執行跨進程服務的詳細資料。 它將討論如何實作為要在自己的進程中執行的服務，以及用戶端如何使用 `Messenger` 架構與該服務進行通訊。 它也會簡短討論雙向通訊：用戶端將訊息傳送至服務，以及將訊息傳送回用戶端的服務。 因為服務可以在不同的應用程式之間共用，所以本指南也會討論使用 Android 許可權來限制用戶端存取服務的一項技巧。

> [!IMPORTANT]
> [Bugzilla 51940/GitHub 1950-具有隔離進程和自訂應用程式類別的服務無法正確解析](https://github.com/xamarin/xamarin-android/issues/1950)多載，報告當 `IsolatedProcess` 設定為 `true`時，將無法正確啟動 Xamarin Android 服務。 本指南提供參考。 Xamarin Android 應用程式仍應能夠與以 JAVA 撰寫的跨進程服務進行通訊。

## <a name="requirements"></a>需求

本指南假設您已熟悉如何建立服務。

雖然可以使用隱含意圖搭配以舊版 Android Api 為目標的應用程式，但本指南將僅著重于使用明確意圖。 以 Android 5.0 （API 層級21）或更高版本為目標的應用程式必須使用明確的意圖來與服務系結;這是將在本指南中示範的技術。
...
- **建立 `Service` 子類別**，&ndash; 將 `Service` 類型的子類別化，並為系結的服務實作為生命週期方法。 您也必須設定中繼資料來通知 Android，服務會在自己的進程中執行。
- **執行 `Handler`** &ndash; `Handler` 負責分析用戶端要求、解壓縮從用戶端傳遞的任何參數，以及在服務上叫用適當的方法。
- 將 `Messenger`&ndash; 具現**化**（如上所述），每個 `Service` 都必須維護 `Messenger` 類別的實例，以將用戶端要求路由傳送至上一個步驟中所建立的 `Handler`。

要在自己的進程中執行的服務，基本上是系結服務。 服務類別會擴充基底 `Service` 類別，並以 `ServiceAttribute` 裝飾，其中包含 Android 需要在 Android 資訊清單中組合的中繼資料。 一開始，下列對跨進程服務而言很重要的 `ServiceAttribute` 屬性：

1. `Exported` &ndash; 此屬性必須設定為 [`true`]，才能讓其他應用程式與服務互動。 此屬性的預設值為 `false`。
2. `Process` &ndash; 必須設定此屬性。 它是用來指定服務將在其中執行的進程名稱。
3. `IsolatedProcess` &ndash; 此屬性將會啟用額外的安全性，指示 Android 在隔離的沙箱中執行服務，並以最少的許可權與系統的其餘部分互動。 請參閱[Bugzilla 51940-具有隔離進程和自訂應用程式類別的服務無法正確解析](https://bugzilla.xamarin.com/show_bug.cgi?id=51940)多載。
4. `Permission` &ndash; 您可以藉由指定用戶端必須要求（並授與）的許可權，來控制用戶端對服務的存取。

若要執行服務自己的進程，`ServiceAttribute` 上的 `Process` 屬性必須設定為服務的名稱。 若要與外部應用程式互動，`Exported` 屬性應該設定為 [`true`]。 如果 `false``Exported`，則只有相同 APK 中的用戶端（也就是相同的應用程式）並在相同的進程中執行，才能夠與服務互動。

服務將在哪種進程中執行，取決於 `Process` 屬性的值。 Android 會識別三種不同類型的進程：

- **私用程式 &ndash; 私**用進程，只有啟動它的應用程式才可使用。 若要將進程識別為私用，其名稱必須以 **：** （分號）開頭。 先前的程式碼片段中所描述的服務和螢幕擷取畫面是私用程式。 下列程式碼片段是 `ServiceAttribute`的範例：
...
- **獨立程式 &ndash; 隔離**的進程是在自己的沙箱中執行的進程，與系統的其餘部分隔離，而且沒有自己的特殊許可權。 若要在隔離的進程中執行服務，`ServiceAttribute` 的 `IsolatedProcess` 屬性會設定為 `true`，如下列程式碼片段所示：
...
下一個螢幕擷取畫面顯示在全域進程中執行 `Process="com.xamarin.xample.messengerservice.timestampservice_process"` 和服務：

![在全域進程中執行之服務的螢幕擷取畫面](out-of-process-services-images/ipc-05.png "在全域進程中執行之服務的螢幕擷取畫面。")

一旦設定了 `ServiceAttribute`，服務就必須執行 `Handler`。

### <a name="implementing-a-handler"></a>執行處理常式

若要處理用戶端要求，服務必須執行 `Handler` 並覆寫 `HandleMessage` 方法。 這是方法所採用的 `Message` 實例，它會從用戶端封裝方法呼叫，並將該呼叫轉譯為服務將執行的某個動作或工作。 `Message` 物件會公開名為 `What` 的屬性，這是一個整數值，其意義是在用戶端與服務之間共用，而且與服務要針對用戶端執行的某些工作相關聯。
...
您也可以在 `Message`中封裝服務的參數。 本指南稍後將討論此內容。 下一個要考慮的主題是建立 `Messenger` 物件來處理傳入的 `Message`s。
...
此時，最後一個步驟是要讓 `Service` 覆寫 `OnBind`。

### <a name="implementing-serviceonbind"></a>執行服務。 OnBind

所有系結的服務無論是否在自己的進程中執行，都必須執行 `OnBind` 方法。 這個方法的傳回值是用戶端可用來與服務互動的一些物件。 該物件確切取決於服務是本機服務還是遠端服務。 當本機服務傳回自訂 `IBinder` 執行時，遠端服務會傳回封裝，但在上一節中建立的 `Messenger` `IBinder`：
...
所有用戶端都必須執行一些程式碼，才能系結和使用遠端服務。 從概念上來說，從用戶端的觀點來看，系結至本機服務或遠端服務之間的差異非常少。 用戶端會叫用 `BindService` 方法，傳遞明確意圖來識別服務，以及協助管理用戶端與服務之間連接的 `IServiceConnection`。

此程式碼片段是如何建立系結至遠端服務之**明確意圖**的範例。 此意圖必須識別包含服務的封裝和服務的名稱。 設定這項資訊的其中一種方式是使用 `Android.Content.ComponentName` 物件，並將其提供給意圖。 此程式碼片段是其中一個範例：  
...
系結服務時，會叫用 `IServiceConnection.OnServiceConnected` 方法，並提供 `IBinder` 給用戶端。 不過，用戶端不會直接使用 `IBinder`。 相反地，它會從該 `IBinder`具現化 `Messenger` 物件。 這是用戶端將用來與遠端服務互動的 `Messenger`。

以下是非常基本的 `IServiceConnection` 實作為範例，示範用戶端如何處理與服務的連線和中斷連接。 請注意，`OnServiceConnected` 方法會接收並 `IBinder`，而用戶端會從該 `IBinder`建立 `Messenger`：
...
在用戶端成功系結至服務且 `Messenger` 可用之後，用戶端就可以將 `Messages` 傳送給服務。

## <a name="sending-messages-to-the-service"></a>將訊息傳送至服務

一旦用戶端連線並具有 `Messenger` 物件，就可以透過 `Messenger`分派 `Message` 物件，來與服務通訊。 這種通訊是單向的，用戶端會傳送訊息，但從服務到用戶端則不會傳回訊息。 就這一點而言，`Message` 是一種火災和忘的機制。

建立 `Message` 物件的慣用方法是使用[`Message.Obtain`](xref:Android.OS.Message) factory 方法。 這個方法會從 Android 維護的全域集區提取 `Message` 物件。 `Message.Obtain` 也有一些多載的方法，可讓 `Message` 物件使用服務所需的值和參數來初始化。  一旦 `Message` 具現化，它就會藉由呼叫 `Messenger.Send`分派給服務。 此程式碼片段是建立和分派 `Message` 至服務程式的其中一個範例：
...
您可以使用 `Bundle`，將更複雜的資料傳遞給服務。 在這種情況下，額外的值可以放在 `Bundle` 中，然後在傳送之前設定[`.Data` 屬性](xref:Android.OS.Message.Data)屬性，藉此與 `Message` 一起傳送。
...
> [!NOTE]
> 一般來說，`Message` 不應具有大於1MB 的承載。 大小限制可能會根據 Android 版本和廠商可能對其在裝置上執行的 Android 開放原始碼專案（AOSP）所做的任何專屬變更而有所不同。

## <a name="returning-values-from-the-service"></a>從服務傳回值

此點已討論過的訊息架構是單向的，用戶端會將訊息傳送給服務。 如果服務必須將值傳回給用戶端，則已對此點進行討論的所有內容都是相反的。 服務必須建立 `Message`、封裝任何傳回值，然後透過 `Messenger` 將 `Message` 分派至用戶端。 不過，服務不會建立自己的 `Messenger`;相反地，它會依賴用戶端具現化，並封裝 `Messenger` 做為初始要求的一部分。 服務將會使用此用戶端提供的 `Messenger``Send` 訊息。  

雙向通訊的事件順序如下：

1. 用戶端會系結至服務。 當服務和用戶端連線時，由用戶端維護的 `IServiceConnection` 將會有 `Messenger` 物件的參考，該物件是用來將 `Message`傳送至服務。 為了避免混淆，這會稱為「_服務 Messenger_」。
2. 用戶端會具現化 `Handler` （稱為_用戶端處理常式_），並使用它來初始化自己的 `Messenger` （_用戶端 Messenger_）。 請注意，服務 Messenger 和用戶端 Messenger 是兩個不同的物件，會以兩個不同的方向來處理流量。 服務 Messenger 會處理從用戶端到服務的訊息，而用戶端 Messenger 會處理從服務到用戶端的訊息。
3. 用戶端會建立一個 `Message` 物件，並使用用戶端 Messenger 來設定 `ReplyTo` 屬性。 接著，訊息會使用服務 Messenger 傳送至服務。
4. 服務會接收來自用戶端的訊息，並執行要求的工作。
5. 當服務將回應傳送至用戶端時，它會使用 `Message.Obtain` 建立新的 `Message` 物件。
6. 為了將此訊息傳送給用戶端，服務會從用戶端訊息的 `.ReplyTo` 屬性將用戶端 Messenger 解壓縮，並用來 `.Send` `Message` 回用戶端。
7. 當用戶端收到回應時，它會有自己的 `Handler`，它會檢查 `.What` 屬性來處理 `Message`，如有必要，請解壓縮 `Message`所包含的任何參數）。

這個範例程式碼會示範用戶端如何具現化 `Message`，並封裝服務應用於其回應的 `Messenger`：
...
服務必須對自己的 `Handler` 進行一些變更，才能將 `Messenger` 解壓縮，並使用它將回復傳送給用戶端。 此程式碼片段是服務 `Handler` 如何建立 `Message` 並將其傳回用戶端的範例：  
...
請注意，在上述的程式碼範例中，用戶端所建立的 `Messenger` 實例，*不*是服務所接收的相同物件。 這些是兩個不同的 `Messenger` 物件，在代表通道的兩個個別進程中執行。
...
許可權可以透過裝飾 `Service` 子類別之 `ServiceAttribute` 的 `Permission` 屬性來識別。 這會命名在系結至服務時，必須授與用戶端的許可權。 如果用戶端沒有適當的許可權，則當用戶端嘗試系結至服務時，Android 將會擲回 `Java.Lang.SecurityException`。

Android 提供四種不同的許可權等級：

- **一般**&ndash; 這是預設的許可權等級。 它可用來識別可由 Android 自動授與要求它的用戶端的低風險許可權。 使用者不需要明確授與這些許可權，但可以在應用程式設定中查看許可權。
- 簽**章 &ndash; 這**是特殊的許可權類別，會由 Android 自動授與所有以相同憑證簽署的應用程式。 此許可權的設計可讓應用程式開發人員輕鬆地在其應用程式之間共用元件或資料，而不需要中斷使用者進行持續核准。
- **signatureOrSystem** &ndash; 這與**上面所述的簽**章許可權非常類似。 除了自動授與由相同憑證簽署的應用程式之外，此許可權也會授與給已簽署相同憑證的應用程式，該憑證是用來簽署與 Android 系統映射一起安裝的應用程式。 此許可權通常僅供 Android ROM 開發人員使用，以允許其應用程式使用協力廠商應用程式。 這種應用程式通常不會使用這種方式，因為公用的一般散發是大型的。
- **危險**的 &ndash; 危險的許可權，這可能會對使用者造成問題。 基於這個理由，使用者必須明確核准**危險**的許可權。

因為 `signature` 和 `normal` 許可權會在 Android 安裝的時間自動授與，所以在 APK 包含用戶端之前，請務必**先**安裝 APK 裝載服務的程式。 如果先安裝用戶端，則 Android 不會授與許可權。 在此情況下，必須將用戶端 APK 卸載、安裝服務 APK，然後重新安裝用戶端 APK。

有兩個常見的方法可以使用 Android 許可權保護服務：

1. **執行簽章層級安全性**&ndash; 的簽章層級安全性，表示系統會自動將許可權授與給那些使用相同金鑰簽署的應用程式，而該索引鍵是用來簽署保留服務的 APK。 這是一種簡單的方法，讓開發人員保護其服務，同時讓他們可以從自己的應用程式中存取。 簽章層級許可權的宣告方式，是將 `ServiceAttribute` 的 `Permission` 屬性設定為 `signature`：
...
下一節將說明建立自訂 `normal` 許可權的簡單範例。 如需 Android 許可權的詳細資訊，請參閱 Google 的檔，以取得[& 安全性的最佳做法](https://developer.android.com/training/articles/security-tips.html)。 如需 Android 許可權的詳細資訊，請參閱應用程式資訊清單之 Android 檔的[許可權一節](https://developer.android.com/guide/topics/manifest/manifest-intro.html#perms)，以取得 android 許可權的詳細資訊。
