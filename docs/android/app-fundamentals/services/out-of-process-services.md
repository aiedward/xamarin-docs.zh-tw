...
- **signatureOrSystem**這與上面所述的簽**章許可權非常**類似。 &ndash; 除了自動授與由相同憑證簽署的應用程式之外，此許可權也會授與給已簽署相同憑證的應用程式，該憑證是用來簽署與 Android 系統映射一起安裝的應用程式。 此許可權通常僅供 Android ROM 開發人員使用，以允許其應用程式使用協力廠商應用程式。 這種應用程式通常不會使用這種方式，因為公用的一般散發是大型的。
