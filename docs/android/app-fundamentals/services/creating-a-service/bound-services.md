...
2. **建立一個執行`IServiceConnection`**  &ndash;此介面的類別，可提供由 Android 叫用的回呼方法，以在服務的連線變更時通知用戶端，也就是用戶端已連接或中斷連接到維護. 服務連線也會提供物件的參考，讓用戶端用來直接與服務互動。 此參考稱為系結器 _。_
3. **建立一個`IBinder`**  _實_作為系結程式的類別，提供用戶端用來與服務通訊的 API。 &ndash; 系結器可以提供系結服務的參考，允許直接叫用方法，或系結器可以提供用戶端 API 來封裝和隱藏應用程式中的系結服務。 必須`IBinder`提供遠端程序呼叫所需的程式碼。 不需要（或建議）直接執行`IBinder`介面。 相反地， `Binder`應用程式應該擴充類型，以提供所需`IBinder`的大部分基本功能。
