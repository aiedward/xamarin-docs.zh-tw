...
- <a name="fcm-in-action-sender-id"></a>**寄件者識別碼***寄件者識別碼*是當您建立 Firebase 專案時所指派的唯一數值。 &ndash; 寄件者識別碼是用來識別每個可以將訊息傳送至用戶端應用程式的應用程式伺服器。 寄件者識別碼也是您的專案編號;當您註冊專案時，您會從 Firebase 主控台取得寄件者識別碼。 寄件者識別碼的範例是`496915549731`。
