...
Android 專案如下所示 &ndash; 本節中所示的程式碼包含在**Orm**目錄中：
...
為了簡化應用程式的程式碼，會根據是否已設定 PrimaryKey 來進行插入或更新，提供單一 save 方法。 因為 `Id` 屬性是以 `[PrimaryKey]` 屬性標示，所以您不應該在程式碼中設定它。 這個方法會偵測先前是否已儲存值（藉由檢查 primary key 屬性），並據以插入或更新物件：
...
不同于 `Insert` 和 `Update` 方法，`Delete<T>` 方法可以只接受主要索引鍵值，而不是完整的 `Stock` 物件。 在此範例中，會將 `Stock` 物件傳遞至方法，但只會將 Id 屬性傳遞給 `Delete<T>` 方法。
...
若要確保某些程式碼會在 Android 應用程式中的任何其他專案之前執行，您可以將它放在要載入的第一個活動中，或者您可以建立在任何活動之前載入的 `Application` 子類別。 下列程式碼顯示一個 `Application` 子類別，可複製現有的資料庫檔案**資料。 sqlite**超出 **/Resources/Raw/** 目錄。
