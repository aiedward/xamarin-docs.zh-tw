...
新的**list_item**版面配置會顯示在設計工具中。 請注意，&ndash; 的*Design Surface*會顯示兩個窗格，而在右窗格中顯示**list_item**的 XML 來源時，就會看到該窗格。 您可以按一下位於兩個窗格之間的 [**交換窗格**] 圖示，交換 [ **Design Surface** ] 和 [**來源**] 窗格的位置：
...
下一個步驟是在此 `LinearLayout`中建立色彩瀏覽器應用程式的使用者介面。

### <a name="creating-the-list-item-user-interface"></a>建立清單專案使用者介面

如果沒有顯示 [**工具箱**] 窗格，請按一下左側的 [**工具箱**] 索引標籤。 在 [**工具箱**] 中，向下 **& [媒體**] 區段中上下移動，直到您找到 `ImageView`為止：
...
將此 `ImageView` 拖曳至 Design Surface （此 `ImageView` 將用來在色彩瀏覽器應用程式中顯示色樣）：

[在畫布上![ImageView](designer-walkthrough-images/vs/09-imageview-on-canvas-w158-sml.png)](designer-walkthrough-images/vs/09-imageview-on-canvas-w158.png#lightbox)

接下來，將 `LinearLayout (Vertical)` widget 從 **工具箱** 拖曳至設計工具。 請注意，藍色外框表示已加入 `LinearLayout`的界限。 [**檔大綱**] 顯示它是 `LinearLayout`的子系，位於 [`imageView1 (ImageView)`] 底下：
...
當您在設計工具中選取 `ImageView` 時，藍色外框會移動以環繞 `ImageView`。 此外，選取範圍會移至**檔大綱**中的 `imageView1 (ImageView)`：
...
接下來，從 **工具箱** 將 `Text (Large)` widget 拖曳到新增的 `LinearLayout`。 請注意，設計工具會使用綠色的反白顯示來指出將插入新 widget 的位置：

[![環保重點](designer-walkthrough-images/vs/12-green-highlight-w158-sml.png)](designer-walkthrough-images/vs/12-green-highlight-w158.png#lightbox)

接下來，在 `Text (Large)` widget 底下新增 `Text (Small)` widget：
...
如果兩個 `textView` widget 不在 `linearLayout1`內，您可以將它們拖曳至**檔大綱**中的 `linearLayout1`，並將它們放置起來，使其如先前的螢幕擷取畫面所示（在 [`linearLayout1`] 底下縮排）。

### <a name="arranging-the-user-interface"></a>排列使用者介面

下一個步驟是修改 UI，以在左側顯示 `ImageView`，並將兩個 `TextView` widget 堆疊到 `ImageView`的右邊。
...
另一個變更 `Width` 設定的方式，是按一下小工具右邊的三角形，將其寬度設定切換為 `wrap_content`：

![拖曳以設定寬度](designer-walkthrough-images/vs/15b-width-arrow-w158.png)

再次按一下三角形，會將 `Width` 設定傳回 `match_parent`。 接下來，移至 [**檔大綱**] 窗格，然後選取根 `LinearLayout`：
...
選取 [根 `LinearLayout`] 後，返回 [**屬性**] 窗格，在 [搜尋] 方塊中輸入*方向*，並找出 [**方向**] 設定。 將**方向**變更為 `horizontal`：

![選取水準方向](designer-walkthrough-images/vs/17-horizontal-orientation-w158.png)

此時，設計工具介面應該類似下列螢幕擷取畫面。
請注意，`TextView` widget 已移至 `ImageView`的右邊：
...
下一步是修改 UI 中的填補和邊界設定，以在 widget 之間提供更多空間。 選取設計介面上的 [`ImageView`]。 在 [**屬性**] 窗格的 [搜尋] 方塊中，輸入 `min`。 輸入最**小高度**的 `70dp`，並 `50dp`**最小寬度**：
...
在 [**屬性**] 窗格的 [搜尋] 方塊中，輸入 `padding`，並輸入 `10dp` 進行**填補**。 這些 `minHeight`、`minWidth` 和 `padding` 設定會在 `ImageView` 的兩側加上填補，並以垂直方式 elongate。 請注意，當您輸入這些值時，版面配置 XML 會變更：
...
例如，將 [**填補] 左邊**欄位設定為 [`5dp`]，並將 **[填補]、[** **右填補**] 和 [**填補前**] 欄位設為 `10dp`：
...
接下來，調整 `LinearLayout` widget 的位置，其中包含兩個 `TextView` 小工具。 在 [**檔大綱**] 中，選取 [`linearLayout1`]。 在 [**屬性**] 視窗的 [搜尋] 方塊中，輸入 `margin`。 將 [**版面配置邊界**]、[**版面**配置邊界] 和 [版面配置**邊界**] 設定為 [`5dp`]。 將 **版面配置邊界** 設定為 `0dp`：
...
由於 `ImageView` 是用來顯示色彩（而非影像），因此下一個步驟是移除範本新增的預設影像來源。

1. 在**設計工具介面**上選取 [`ImageView`]。
...
這會從來源 XML 移除該 `ImageView`的 `android:src="@android:drawable/ic_menu_gallery"`。

### <a name="adding-a-listview-container"></a>新增 ListView 容器

現在已定義**list_item**配置，下一個步驟是將 `ListView` 新增至主要版面配置。 此 `ListView` 將包含**list_item**的清單。 

在**方案總管**中，開啟**Resources/layout/activity_main. axml**。 在 [**工具箱**] 中找出 `ListView` widget，並將它拖曳至 [ **Design Surface**]。 設計工具中的 `ListView` 會是空白，但在選取時，會將其框線外框的藍色線條除外。 您可以查看**檔大綱**，確認已正確新增**ListView** ：
...
根據預設，`ListView` 會獲得 `@+id/listView1`的 `Id` 值。
當您仍在**檔大綱**中選取 `listView1` 時，請開啟 [**屬性**] 窗格，按一下 [**排列依據**]，然後選取 [**類別**]。
開啟 [ **Main**]，找到**Id**屬性，並將其值變更為 `@+id/myListView`：
...
這段程式碼會使用自訂的 `ListView` 介面卡來載入色彩資訊，並在剛建立的 UI 中顯示此資料。 為了讓此範例保持簡短，色彩資訊會以硬式編碼方式放入清單中，但是可以修改介面卡以從資料來源解壓縮色彩資訊，或即時計算。 如需 `ListView` 介面卡的詳細資訊，請參閱[ListView](~/android/user-interface/layouts/list-view/index.md)。
...
按一下畫面底部的 [**設計**工具] 索引標籤，返回**設計工具介面**。 在右側的 **工具箱** 窗格中，向下 **& 媒體** 區段中 `ImageView`，並找出 ：
...
[在畫布上![ImageView](designer-walkthrough-images/xs/08-imageview-on-canvas-m75-sml.png)](designer-walkthrough-images/xs/08-imageview-on-canvas-m75.png#lightbox)

接下來，從 **工具箱** 將 `LinearLayout (Vertical)` widget 拖曳至  **Design Surface**。 請注意，藍色外框表示已加入 `LinearLayout`的界限。 [**檔大綱**] 顯示它是 `LinearLayout`的子系，位於下列 `imageView1 (ImageView)`：
...
接下來，從 **工具箱** 將 `Text (Large)` widget 拖曳到新增的 `LinearLayout`。 請注意，當您將滑鼠拖曳至  **Design Surface**時，它會反白顯示新 widget 的插入位置。
`Text (Large)` widget 應位於 `linearLayout1` 內，如下所示：
...
接下來，在 `Text (Large)` widget 底下新增 `Text (Small)` widget。 此時， **Design Surface**應該類似下列螢幕擷取畫面：
...
如果兩個 `textView` widget 不在 `linearLayout1`內，您可以將它們拖曳至**檔大綱**中的 `linearLayout1`，並將它們放置起來，使其如先前的螢幕擷取畫面所示（縮排于 [`linearLayout1`] 底下）。

### <a name="arranging-the-user-interface"></a>排列使用者介面

下一個步驟是修改 UI，以在左側顯示 `ImageView`，並將兩個 `TextView` widget 堆疊到 `ImageView`的右邊。

1. 選取 `ImageView`，然後按一下 [**屬性**] 索引標籤。

2. 在 [**屬性**] 索引標籤正下方 **，按一下 [** 配置]。

3. 向下**ViewGroup** ，並將 `Width` 設定變更為 `wrap_content`：

[![設定包裝內容](designer-walkthrough-images/xs/15-wrap-content-m75-sml.png)](designer-walkthrough-images/xs/15-wrap-content-m75.png#lightbox)
...
選取 [根 `LinearLayout`] 後，返回 [**屬性**] 索引標籤，然後按一下 [ **Widget**]。 將 `Orientation` 設定變更為 `horizontal`，如下所示。 此時， **Design Surface**應該類似下列螢幕擷取畫面。 請注意，`TextView` widget 已移至 `ImageView`的右邊：
...
下一步是修改 UI 中的填補和邊界設定，以在 widget 之間提供更多空間。 選取 `ImageView`，然後按一下 [**屬性** **] 底下**的 [配置] 索引標籤。 將 `Min Width` 變更為 `50dp`、要 `70dp`的 `Min Height`，以及要 `Padding` 的 `10dp`。
這會在 `ImageView` 的兩側加上填補，並以垂直方式 elongates：
...
您可以分別在 [`Top`]、[`Right`]、[`Bottom`] 和 [`Left` 填補] 欄位中輸入值，獨立設定上、右、下和左填補設定。 例如，將 `Left` 填補 值設定為 `5dp`，將 `Top`、`Right` 和 `Bottom` 填補 值設為 `10dp`。 請注意，`Padding` 設定會變更為這些值的逗號分隔清單：
...
接下來，調整 `LinearLayout` widget 的位置，其中包含兩個 `TextView` 小工具。 在 [**檔大綱**] 中，選取 [`linearLayout1`]。 在 [**屬性**] 窗格中，選取 [**配置] 索引**標籤。向下**ViewGroup**區段，然後將 [`Left`]、[`Top`]、[`Right`] 和 [`Bottom` 邊界] 分別設定為 `5dp`、`5dp`、`0dp`和 `5dp`:
...
按一下 [`Button` widget] （如果有的話），並將它刪除。 在 [**工具箱**] 中找出 `ListView` widget，並將它拖曳至 [ **Design Surface**]。
設計工具中的 `ListView` 會是空白，但在選取時，會將其框線外框的藍色線條除外。 您可以查看**檔大綱**，確認已正確新增**ListView** ：
