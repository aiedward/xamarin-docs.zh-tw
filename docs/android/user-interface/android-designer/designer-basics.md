...
- **來源**這個按鈕 (類似兩個相反的角括弧) 會選取 [來源] 窗格。 &ndash; 按一下這個按鈕時, [**工具箱**] 和 [**屬性**] 窗格會停用, 而且 [**文字編輯器] 工具列**會顯示在 Visual Studio 的頂端。 按一下 [折迭] 按鈕 (如下所示) 時, 按一下 [**來源**] 按鈕會顯示 [**來源**] 窗格, 而不是 [**設計**] 窗格。
