...
Google Play 並未支援全球所有國家/地區。 您可以在下列連結中找到最新的國家/地區清單：

1. [開發人員 &amp; 支援的位置商家註冊](https://support.google.com/googleplay/android-developer/bin/answer.py?hl=en&amp;answer=150324) &ndash; ：這是一份清單，其中列出開發人員可註冊為商家和銷售付費應用程式的所有國家/地區。
...
1. **簡單整齊** &ndash;啟動器圖示應該保持簡單且整齊。 這意謂著從圖示中排除應用程式名稱。 較簡單的圖示較容易記住，且大小較小時也較容易辨識。

1. **圖示不應該是精簡** &ndash; 型過度精簡的圖示在所有背景上都無法正常運作。

1. **使用 Alpha** &ndash; 色板圖示應該使用 Alpha 色板，且不應該是完整框架的影像。
...
Google play 的應用程式至少需要2個和最多八個螢幕擷取畫面。 它們會顯示在 Google Play 的應用程式詳細資料頁面上。
...
在[封存以供發行](~/android/deploy-test/release-prep/index.md#archive)中，[散發通道] 對話方塊顯示了兩個用於散發的選項：[臨機操作] 和 [Google Play]。 如果改為顯示 [簽署身分識別] 對話方塊，請按一下 [上一步] 以返回 [散發通道] 對話方塊。 選取 **Google Play**：
...
[![簽署身分識別對話方塊](images/vs/02-select-identity-sml.png)](images/vs/02-select-identity.png#lightbox)
...
[![在 Google Play 帳戶中註冊 Google API 存取對話方塊。](images/vs/04-register-google-api-access-sml.png)](images/vs/04-register-google-api-access.png#lightbox)
...
[![選取 Android 散發對話方塊](images/xs/01-select-google-play-sml.png)](images/xs/01-select-google-play.png#lightbox)
...
此外，Google Play 開發人員 API [入門](https://developers.google.com/android-publisher/getting_started) \(英文\) 也說明了如何使用 Google Play 開發人員 API。 登入 Google Play 開發人員主控台後，按一下 [ **建立應用程式**]：

[![[建立新專案] 按鈕](images/01-create-new-project-sml.png)](images/01-create-new-project.png#lightbox)

建立新專案之後，它會連結到您的 Google Play 開發人員主控台帳戶。
...
在 [ **設定** ] 頁面中，選取 [ **API 存取** ]，然後按一下 [ **建立 oauth 用戶端** ] 以建立新的 oauth 用戶端：
...
[![顯示的用戶端識別碼](images/04-generated-client-id-sml.png)](images/04-generated-client-id.png#lightbox)

「用戶端識別碼」會與其名稱和建立日期一起顯示。 按一下 [編輯 OAuth 用戶端] 圖示，以檢視您應用程式的用戶端密碼：

[![查看應用程式認證](images/05-google-developer-console-sml.png)](images/05-google-developer-console.png#lightbox)

OAuth 用戶端的預設名稱為「Google Play Android 開發人員」。 此名稱可以變更為 Xamarin.Android 應用程式，或任何適合的名稱。 在此範例中，「OAuth 用戶端」已變更為應用程式的名稱 **MyApp**：

[![顯示的用戶端識別碼和秘密](images/06-client-id-and-secret-sml.png)](images/06-client-id-and-secret.png#lightbox)

按一下 [儲存] 來儲存變更。 這會返回 [ **認證** ] 頁面，按一下 [  **下載 JSON** ] 圖示以下載認證：
...
[![註冊 Google API 存取對話方塊](images/vs/05-enter-client-id-and-secret-sml.png)](images/vs/05-enter-client-id-and-secret.png#lightbox)

網頁瀏覽器將會開啟，並提示您登入「Google Play Android 開發人員」帳戶 (如果您尚未登入)。 登入之後，網頁瀏覽器中會顯示以下提示。
請按一下 [允許] 以授權應用程式：

[![授權應用程式對話方塊](images/vs/06-authorize-app-sml.png)](images/vs/06-authorize-app.png#lightbox)

#### <a name="publish"></a>發佈

按一下 [ **允許**] 之後，瀏覽器會報告 _收到的驗證碼。正在關閉 ..._ 然後，應用程式會新增至 Visual Studio 的 Google Play 帳戶清單中。 在 [Google Play 帳戶] 對話方塊中，按一下 [繼續]：

[![帳戶已新增至 Google Play 帳戶](images/vs/07-account-added-sml.png)](images/vs/07-account-added.png#lightbox)

接著，就會顯示 [Google Play 追蹤] 對話方塊。 Google Play 提供五個上傳應用程式的可能追蹤：

- **內部** &ndash; 用來快速散發您的應用程式，以進行內部測試和品質保證檢查。
- **Alpha** &ndash; 用來將您的應用程式早期版本上傳到少數的測試人員清單。
- **Beta 版** &ndash; 用來將您的應用程式早期版本上傳至較大的測試人員清單。
- **生產環境** &ndash; 用於 Google Play 存放區的完整散發。
- **自訂** &ndash; 用來透過電子郵件地址建立測試人員清單，以使用特定使用者測試應用程式的發行前版本。

請選擇要使用哪一個 Google Play 追蹤方式來上傳應用程式，然後按一下 [上傳]。

[![帳戶已新增至 Google Play 的追蹤](images/vs/08-google-play-track-sml.png)](images/vs/08-google-play-track.png#lightbox)

如需 Google Play 測試的詳細資訊，請參閱 [設定開啟/關閉/內部測試](https://support.google.com/googleplay/android-developer/answer/3131213?hl=en)。

接著，會顯示一個對話方塊，可供您輸入用來簽署憑證的密碼。
輸入密碼，然後按一下 **[確定]**：

[![簽署密碼對話方塊](images/vs/09-certificate-password-sml.png)](images/vs/09-certificate-password.png#lightbox)
...
[![發行專案已完成訊息](images/vs/11-published-sml.png)](images/vs/11-published.png#lightbox)

### <a name="troubleshooting"></a>疑難排解

如果您在選取 Google Play 的曲目時沒有看到您的自訂追蹤，請確定您已在 Google Play 開發人員主控台上建立該追蹤的版本。 如需有關如何建立發行的指示，請參閱 [準備 & 推出版本](https://support.google.com/googleplay/android-developer/answer/7159011?hl=en)。
...
發生此錯誤時，請透過 Google Play 開發人員主控台手動上傳 APK (例如臨機操作組建) ，並使用 [ **散發通道** ] 對話方塊來進行後續的 APK 更新。  如需詳細資訊，請參閱[手動上傳 APK](~/android/deploy-test/publishing/publishing-to-google-play/manually-uploading-the-apk.md)。 APK 的版本代碼必須隨著每次上傳變更，否則將會發生以下錯誤：
...
[![授權存取對話方塊](images/xs/03-register-sml.png)](images/xs/03-register.png#lightbox)

如果系統接受「用戶端識別碼」和「用戶端密碼」，就會顯示 **註冊成功** 訊息。 按 **[下一步]**：
...
[![選擇 Google 帳戶對話方塊](images/xs/05-choose-google-account-sml.png)](images/xs/05-choose-google-account.png#lightbox)

Google Play 提供五個上傳應用程式的可能追蹤：
...
如需 Google Play 測試的詳細資訊，請參閱 [設定 Alpha/Beta 測試](https://support.google.com/googleplay/android-developer/answer/3131213?hl=en)。
...
[![[Android 簽署識別] 對話方塊](images/xs/06-android-signing-identity-sml.png)](images/xs/06-android-signing-identity.png#lightbox)

此時已可將應用程式上傳到 Google Play。 [發行至 Google Play] 對話方塊會摘要列出有關您應用程式的資訊 &ndash; 請按一下 [發行] 以將應用程式發 行至 Google Play：

[![[發佈至 Google Play] 對話方塊](images/xs/07-publish-to-google-play-sml.png)](images/xs/07-publish-to-google-play.png#lightbox)

### <a name="troubleshooting"></a>疑難排解

如果您選取要上傳應用程式的 Google Play 播放軌時，沒有看到您的自訂追蹤，請確定您已在 Google Play 開發人員主控台上建立該追蹤的版本。 如需有關如何建立發行的指示，請參閱 [準備 & 推出版本](https://support.google.com/googleplay/android-developer/answer/7159011?hl=en)。

請注意，必須已將一個 APK 提交給 Google Play 商店，[發行至 Google Play] 才能運作。 如果未上傳 APK，可能會發生以下錯誤：

> _Google Play 需要您手動上傳此應用程式的第一個 APK。您可以針對此使用臨機操作 APK。_

或

> 「找不到指定之套件名稱的應用程式。[404]」

若要解決這個錯誤，請透過 Google Play 開發人員主控台手動上傳 APK (例如臨機操作組建) ，然後使用 [ **發行至 Google Play** ] 對話方塊來進行後續的 APK 更新。 如需有關如何手動上傳 APK 的資訊，請參閱[手動上傳 APK](~/android/deploy-test/publishing/publishing-to-google-play/manually-uploading-the-apk.md)。
