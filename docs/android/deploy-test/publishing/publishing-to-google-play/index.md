...
1. [開發人員 &amp; 商家註冊的支援位置](https://support.google.com/googleplay/android-developer/bin/answer.py?hl=en&amp;answer=150324)&ndash; 這是開發人員可以註冊為商家並銷售付費應用程式的所有國家/地區清單。

1. [支援散發至 Google Play 使用者的位置](https://support.google.com/googleplay/android-developer/bin/answer.py?hl=en&amp;answer=138294)&ndash; 這是可散發應用程式的所有國家/地區清單。
...
Google play 最少需要兩個，最多可有八個應用程式的螢幕擷取畫面。 它們會顯示在 Google Play 的應用程式詳細資料頁面上。
...
此外，Google Play 開發人員 API [入門](https://developers.google.com/android-publisher/getting_started) \(英文\) 也說明了如何使用 Google Play 開發人員 API。 登入 Google Play 開發人員主控台之後，請按一下 [**建立應用程式**]：

[![[建立新專案] 按鈕](images/01-create-new-project-sml.png)](images/01-create-new-project.png#lightbox)

建立新專案之後，它會連結到您 Google Play 的開發人員主控台帳戶。
...
按一下 [儲存] 來儲存變更。 這會返回 [**認證**] 頁面，您可以在其中按一下 [**下載 JSON** ] 圖示來下載認證：
...
按一下 [**允許**] 之後，瀏覽器會報告_收到的驗證碼。正在關閉 ..._ 然後，應用程式會新增至 Visual Studio 中的 Google Play 帳戶清單。 在 [Google Play 帳戶] 對話方塊中，按一下 [繼續]：

[![帳戶新增至 Google Play 帳戶](images/vs/07-account-added-sml.png)](images/vs/07-account-added.png#lightbox)

接著，就會顯示 [Google Play 追蹤] 對話方塊。 Google Play 提供上傳應用程式的五個可能的追蹤：

- **內部**&ndash; 用於快速散發您的應用程式，以進行內部測試和品質保證檢查。
- **Alpha** &ndash; 用來將應用程式的早期版本上傳至小型的測試者清單。
- 搶鮮**版（Beta** ） &ndash; 用來將應用程式的早期版本上傳至較大的測試者清單。
- **生產**&ndash; 用於 Google Play 存放區的完整散發。
- **自訂**&ndash; 用於測試應用程式的發行前版本，並透過電子郵件地址建立測試者清單，以供特定使用者使用。

請選擇要使用哪一個 Google Play 追蹤方式來上傳應用程式，然後按一下 [上傳]。

[![帳戶新增至 Google Play 帳戶](images/vs/08-google-play-track-sml.png)](images/vs/07-account-added.png#lightbox)
...
如果您在選取 [Google Play] 曲目時看不到您的自訂播放軌，請確定您已在 Google Play 開發人員主控台上建立該追蹤的版本。 如需如何建立發行的指示，請參閱[準備 & 推出](https://support.google.com/googleplay/android-developer/answer/7159011?hl=en)發行。
...
發生此錯誤時，請透過 Google Play 開發人員主控台手動上傳 APK （例如臨機操作組建），然後使用 [**散發通道**] 對話方塊來進行後續的 APK 更新。  如需詳細資訊，請參閱[手動上傳 APK](~/android/deploy-test/publishing/publishing-to-google-play/manually-uploading-the-apk.md)。 APK 的版本代碼必須隨著每次上傳變更，否則將會發生以下錯誤：
...
[![[授權存取] 對話方塊](images/xs/03-register-sml.png)](images/xs/03-register.png#lightbox)

如果系統接受「用戶端識別碼」和「用戶端密碼」，就會顯示**註冊成功**訊息。 按一下 [下一步]：

[![「註冊成功」訊息](images/xs/04-registration-successful-sml.png)](images/xs/04-registration-successful.png#lightbox)

在 [Google Play 帳戶] 對話方塊中，選取一個 Google 帳戶和追蹤方式來上傳應用程式：

[![[選擇 Google 帳戶] 對話方塊](images/xs/05-choose-google-account-sml.png)](images/xs/05-choose-google-account.png#lightbox)

Google Play 提供上傳應用程式的五個可能的追蹤：
...
[![[發行至 Google Play] 對話方塊](images/xs/07-publish-to-google-play-sml.png)](images/xs/07-publish-to-google-play.png#lightbox)

### <a name="troubleshooting"></a>疑難排解

當您選取要將應用程式上傳至的 Google Play 曲目時，如果看不到您的自訂播放軌，請確定您已在 Google Play 開發人員主控台上建立該追蹤的版本。 如需如何建立發行的指示，請參閱[準備 & 推出](https://support.google.com/googleplay/android-developer/answer/7159011?hl=en)發行。
