...
此圖中未顯示的是`Canvas` , `CanvasWatchFaceService`它會使用來繪製監&ndash;看`Canvas`臉部, 這會透過`OnDraw`方法傳入, 如下所述。
...
方法會`UnregisterTimezoneReceiver`呼叫和方法。`RegisterTimezoneReceiver` `OnVisibilityChanged`
