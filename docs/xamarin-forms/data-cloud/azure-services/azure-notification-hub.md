---
title: 使用 Azure 通知中樞傳送及接收推播通知 Xamarin.Forms
description: 本文說明如何使用 Azure 通知中樞將跨平臺推播通知傳送至 Xamarin.Forms 應用程式。
ms.prod: xamarin
...
依照下列步驟，將 Azure 通知中樞整合到 mobile apps：
...
1. [設定推播通知的原生 Android 專案](#configure-the-android-application-for-notifications)。
1. [設定推播通知的原生 iOS 專案](#configure-ios-for-notifications)。
1. [使用 Azure 通知中樞測試通知](#test-notifications-in-the-azure-portal)。
...
將 Azure 通知中樞與行動 Xamarin.Forms 應用程式整合，類似于將 Azure 通知中樞與 Xamarin 原生應用程式整合。 Firebase Firebase [使用 Azure 通知中樞遵循將推播通知推播通知](/azure/notification-hubs/xamarin-notification-hubs-push-notifications-android-gcm#create-a-firebase-project-and-enable-firebase-cloud-messaging)中的主控台步驟，設定雲端通訊 (FCM) 應用程式。 使用 Xamarin. Android 教學課程來完成下列步驟：
...
您將需要 macOS 機器才能完成 iOS 裝置的設定。 遵循 [使用 Azure 通知中樞將推播通知推送至 Xamarin](/azure/notification-hubs/xamarin-notification-hubs-ios-push-notification-apns-get-started#generate-the-certificate-signing-request-file)的初始步驟，來設定 Apple Push NOTIFICATION SERVICES (APNS) 。 使用 Xamarin 教學課程來完成下列步驟：
...
## <a name="register-templates-and-tags-with-the-azure-notification-hub"></a>向 Azure 通知中樞註冊範本和標記

Azure 通知中樞需要行動應用程式向中樞註冊、定義範本，以及訂閱標記。 註冊會將平臺特定的 PNS 控制碼連結至 Azure 通知中樞內的識別碼。 若要深入瞭解註冊，請參閱 [註冊管理](/azure/notification-hubs/notification-hubs-push-notification-registration-management)。

範本可讓裝置指定參數化的訊息範本。 您可以依每個標籤自訂每個裝置的內送訊息。 若要深入瞭解範本，請參閱 [範本](/azure/notification-hubs/notification-hubs-templates-cross-platform-push-messages)。

標記可以用來訂閱訊息類別，例如新聞、體育和氣象。 為了簡單起見，範例應用程式會定義具有單一參數的預設範本 `messageParam` ，而且會呼叫單一標記 `default` 。 在更複雜的系統中，使用者專屬的標記可以用來將裝置上的使用者訊息給個人化通知。 若要深入瞭解標記，請參閱 [路由和標記運算式](/azure/notification-hubs/notification-hubs-tags-segment-push-message)。

若要成功接收訊息，每個原生應用程式都必須執行下列步驟：

1. 從平臺 PNS 取得 PNS 控制碼或權杖。
1. 向 Azure 通知中樞註冊 PNS 控制碼。
1. 指定包含與外寄訊息相同參數的範本。
1. 訂閱外寄訊息的目標標記。

在 [ [設定 Android 應用程式的通知](#configure-the-android-application-for-notifications) 和 [設定 iOS 的通知](#configure-ios-for-notifications) ] 區段中，每個平臺的詳細說明這些步驟。

## <a name="no-locxamarinforms-application-functionality"></a>Xamarin.Forms 應用程式功能

範例 Xamarin.Forms 應用程式會顯示推播通知訊息的清單。 這可透過方法來達成 `AddMessage` ，這會將指定的推播通知訊息新增至 UI。 這個方法也會防止將重複的訊息加入至 UI 中，並在主執行緒上執行，因此可以從任何執行緒呼叫。 下列程式碼顯示 `AddMessage` 方法：
...
範例應用程式包含檔案 `AppConstants.cs` ，該檔案會定義平臺專案所使用的屬性。 您必須使用 Azure 通知中樞的值自訂此檔案。 下列程式碼顯示檔案 `AppConstants.cs` ：
...
1. 將 `google-services.json` 您在 FCM 安裝期間下載的檔案複製到專案中，並將組建動作設定為 `GoogleServicesJson` 。
...
`receiver`元素內的專案 `application` 可讓應用程式與進行通訊 Firebase 。 這些 `uses-permission` 元素可讓應用程式處理訊息，並向 Azure 通知中樞註冊。 完成的結果 `AndroidManifest.xml` 看起來應該會類似下列範例：
...
若要註冊 Firebase 和處理訊息，請將類別子類別化 `FirebaseMessagingService` 。 範例應用程式會定義子類別 `FirebaseService` 的類別 `FirebaseMessagingService` 。 這個類別是以 `IntentFilter` 包含篩選準則的屬性標記 `com.google.firebase.MESSAGING_EVENT` 。 此篩選器可讓 Android 將傳入訊息傳遞至此類別以進行處理：
...
當應用程式啟動時， Firebase SDK 會自動向伺服器要求唯一的權杖識別碼 Firebase 。 在成功的要求中， `OnNewToken` 會在類別上呼叫方法 `FirebaseService` 。 範例專案會覆寫此方法，並向 Azure 通知中樞註冊權杖：
...
方法會向 `SendRegistrationToServer` Azure 通知中樞註冊裝置，並使用範本訂閱標記。 範例應用程式會定義一個名為的標記 `default` ，以及一個在檔案中呼叫單一參數的範本 `messageParam` `AppConstants.cs` 。 如需註冊、標記和範本的詳細資訊，請參閱 [使用 Azure 通知中樞註冊範本和標記](#register-templates-and-tags-with-the-azure-notification-hub)。

收到訊息時， `OnMessageReceived` 會在類別上呼叫方法 `FirebaseService` ：
...
傳入的訊息會使用方法轉換成本機通知 `SendLocalNotification` 。 這個方法會建立新的 `Intent` ，並將訊息內容放入中 `Intent` 做為 `string` `Extra` 。 當使用者按下本機通知時，無論應用程式是在前景還是背景中， `MainActivity` 都會啟動並透過物件存取訊息內容 `Intent` 。
...
`Activity`屬性會將應用程式設定 `LaunchMode` 為 `SingleTop` 。 此啟動模式會告知 Android 作業系統只允許此活動的單一實例。 使用此啟動模式時， `Intent` 會將內送資料路由傳送至 `OnNewIntent` 方法，該方法會將訊息資料解壓縮，並透過方法將其傳送至 `MainPage` 實例 `AddMessage` 。 如果您的應用程式使用不同的啟動模式，它必須 `Intent` 以不同的方式處理資料。

`OnCreate`方法會使用呼叫的 helper 方法 `IsPlayServiceAvailable` 來確保裝置支援 Google Play 服務。 不支援 Google Play 服務的模擬器或裝置無法從接收推播通知 Firebase 。

## <a name="configure-ios-for-notifications"></a>設定 iOS 的通知

設定 iOS 應用程式以接收通知的程式如下：

1. 在檔案中設定套件組合 **識別碼** `Info.plist` ，以符合布建設定檔中使用的值。
1. 將 [ **啟用推播通知** ] 選項新增至檔案 `Entitlements.plist` 。
1. 將 `Xamarin.Azure.NotificationHubs.iOS` NuGet 套件新增至您的專案。
1. [註冊](#register-for-notifications-with-apns) 具有 APNS 的通知。
1. 向 Azure 通知中樞[註冊](#register-with-azure-notification-hub-and-subscribe-to-tags)應用程式，並訂閱標記。
1. [新增](#add-apns-notifications-to-xamarinforms-ui) 對 UI 的 APNS 通知 Xamarin.Forms 。

下列螢幕擷取畫面顯示 Visual Studio 中的檔案中選取的 [ **啟用推播通知** ] 選項 `Entitlements.plist` ：

![推播通知許可權的螢幕擷取畫面](azure-notification-hub-images/push-notification-entitlement.png "推播通知權利")

### <a name="register-for-notifications-with-apns"></a>使用 APNS 註冊通知

`FinishedLaunching`必須覆寫檔案中的方法 `AppDelegate.cs` ，才能註冊遠端通知。 註冊會因裝置上使用的 iOS 版本而有所不同。 範例應用程式中的 iOS 專案會覆寫 `FinishedLaunching` 要呼叫的方法， `RegisterForRemoteNotifications` 如下列範例所示：
...
當裝置在方法期間成功註冊遠端通知時 `FinishedLaunching` ，iOS 會呼叫 `RegisteredForRemoteNotifications` 方法。 應覆寫這個方法以執行下列動作：
...
> [!NOTE]
> 註冊遠端通知可能會在沒有網路連線的情況下失敗。 您可以選擇覆寫 `FailedToRegisterForRemoteNotifications` 方法以處理註冊失敗。

### <a name="add-apns-notifications-to-no-locxamarinforms-ui"></a>將 APNS 通知新增至 Xamarin.Forms UI

當裝置收到遠端通知時，iOS 會呼叫 `ReceivedRemoteNotification` 方法。 內送訊息 JSON 會轉換成 `NSDictionary` 物件，而 `ProcessNotification` 方法會從字典中解壓縮值，並將其傳送至 Xamarin.Forms `MainPage` 實例。 `ReceivedRemoteNotifications`方法會被覆寫為呼叫， `ProcessNotification` 如下列程式碼所示：
...
Azure 通知中樞可讓您檢查您的應用程式是否可以接收測試訊息。 通知中樞的 [ **測試傳送** ] 區段可讓您選擇目標平臺並傳送訊息。 將 **傳送至標記運算式** 設定為， `default` 會將訊息傳送至已註冊標記範本的應用程式 `default` 。 按一下 [ **傳送** ] 按鈕會產生一份報告，其中包含與訊息聯繫的裝置數目。 下列螢幕擷取畫面顯示 Azure 入口網站中的 Android 通知測試：
...
1. 測試應用程式是否可以接收推播通知時，您必須使用實體裝置。 Android 和 iOS 虛擬裝置可能未正確設定，無法接收推播通知。
1. 範例 Android 應用程式會在簽發權杖時註冊其權杖和範本一次 Firebase 。 在測試期間，您可能需要要求新的權杖，並重新向 Azure 通知中樞註冊。 強制執行這項工作的最佳方式，是清理您的專案、刪除 `bin` 和 `obj` 資料夾，然後從裝置卸載應用程式，然後再重建和部署。
1. 推播通知流程的許多部分會以非同步方式執行。 這可能會導致中斷點未被叫用，或以非預期的順序達到。 使用裝置或偵錯工具記錄來追蹤執行，而不中斷應用程式流程。 使用中指定的來篩選 Android 裝置記錄 `DebugTag` `Constants` 。
1. 在 Visual Studio 中停止偵錯工具時，應用程式會強制關閉。 任何做為偵錯工具一部分啟動的訊息接收者或其他服務都將會關閉，且不會回應訊息事件。

## <a name="create-a-notification-dispatcher"></a>建立通知發送器

Azure 通知中樞可讓您的後端應用程式將通知分派至跨平臺的裝置。 此範例會示範使用主控台應用程式的通知分派。 應用程式包含檔案，該檔案會 `DispatcherConstants.cs` 定義下列屬性：
...
主控台應用程式會在每個值之間執行迴圈 `SubscriptionTags` ，並使用類別的實例，將通知傳送給訂閱者 `NotificationHubClient` 。 下列程式碼顯示主控台應用程式 `Program` 類別：
...
執行範例主控台應用程式時，可以按下空格鍵來傳送訊息。 如果裝置已正確設定，則執行用戶端應用程式的裝置應該會收到編號的通知。
...
* [路由和標記運算式](/azure/notification-hubs/notification-hubs-tags-segment-push-message)。
