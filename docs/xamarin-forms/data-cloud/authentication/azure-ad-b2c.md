...
在範例專案中，編輯**Constants.cs**檔案，將 `clientId` 欄位設定為您的**應用程式識別碼**。 如果您的應用程式識別碼 `1234abcd`，下列程式碼會顯示應如何設定此值：
...
「Microsoft 驗證程式庫」（MSAL） NuGet 封裝必須加入至 [共用]、[.NET Standard] 專案，以及 [Xamarin] 表單方案中的平臺專案。 MSAL 包含 `PublicClientApplicationBuilder` 類別，它會將符合 `IPublicClientApplication` 介面的物件進行結構。 MSAL 會利用 `With` 子句，將額外的參數提供給函數和驗證方法。

在範例專案中， **app.xaml 的程式**代碼會定義名為 `AuthenticationClient` 和 `UIParent`的靜態屬性，並在此函式中具現化 `AuthenticationClient` 物件。 `WithIosKeychainSecurityGroup` 子句會提供 iOS 應用程式的安全性群組名稱。 `WithB2CAuthority` 子句會提供將用來驗證使用者的預設**授權**單位（或原則）。 如果指定多個 Uri，`WithRedirectUri` 子句會告訴 Azure 通知中樞實例要使用哪個 URI 重新導向。 下列範例示範如何具現化 `PublicClientApplication`：
...
> [!NOTE]
> 如果您的 Azure 通知中樞實例只定義了一個重新導向 URI，則 `AuthenticationClient` 實例可能會正常執行，而不需使用 `WithRedirectUri` 子句指定重新導向 URI。 不過，如果您的 Azure 設定擴充以支援其他用戶端或驗證方法，您應該一律指定此值。

**LoginPage.xaml.cs**程式碼後置中的 `OnAppearing` 事件處理常式會呼叫 `AcquireTokenSilentAsync`，以重新整理之前已登入之使用者的驗證權杖。 如果成功，驗證程式會重新導向至 `LogoutPage`，而不會在失敗時執行任何作業。 下列範例顯示 `OnAppearing`中的無訊息重新驗證程式：
...
`OnLoginButtonClicked` 事件處理常式（按一下 [登入] 按鈕時引發）會呼叫 `AcquireTokenAsync`。 MSAL 程式庫會自動開啟行動裝置瀏覽器，並流覽至登入頁面。 登入 URL （稱為「**授權**單位」）是**Constants.cs**檔案中定義的租使用者名稱和原則的組合。 如果使用者選擇 [忘記密碼] 選項，則會將其傳回至具有例外狀況的應用程式，以啟動忘記密碼體驗。 下列範例顯示驗證程式：
...
`OnForgotPassword` 方法類似于登入程式，但會執行自訂原則。 `OnForgotPassword` 使用不同的 `AcquireTokenAsync`多載，可讓您提供特定的**授權**單位。 下列範例顯示如何在取得權杖時提供自訂**授權**：
...
最後一項驗證就是登出程式。 當使用者按下 [登出] 按鈕時，會呼叫 `OnLogoutButtonClicked` 方法。 它會對所有帳戶執行迴圈，並確保其權杖已失效。 下列範例示範登出的執行方式：
...
當 Azure Active Directory B2C 完成授權要求時，它會重新導向至已註冊的重新導向 URL。 自訂 URL 配置會導致 iOS 啟動行動應用程式，並以啟動參數的形式傳入 URL，其中由應用程式 `AppDelegate` 類別的 `OpenUrl` 覆寫進行處理，並將體驗的控制權傳回給 MSAL。 下列程式碼範例顯示 `OpenUrl` 的執行：
...
`MainActivity` 類別必須經過修改，才能在 `OnCreate` 呼叫期間提供 `UIParent` 物件給應用程式。 當 Azure Active Directory B2C 完成授權要求時，它會從**androidmanifest.xml**重新導向至已註冊的 URL 配置。 已註冊的 URI 配置會導致 Android 呼叫 `OnActivityResult` 方法，並以 URL 作為啟動參數，由 `SetAuthenticationContinuationEventArgs` 方法處理。
