...
description: Xamarin.Forms 檢視是跨平台行動裝置的使用者介面的建置組塊。 本文列出在 Xamarin.Forms 中所包含的檢視。
ms.prod: xamarin
