...
ms.locfileid: "87918397"
---
# <a name="no-locxamarinforms-shapes-path-transforms"></a>Xamarin.Forms圖形：路徑轉換
...
`CenterX`和 `CenterY` 屬性會指定要旋轉物件的時間點 `Path` 。 這個中心點會以轉換之物件的座標空間來表示。 根據預設，旋轉會套用至 (0，0) ，這是物件的左上角 `Path` 。
