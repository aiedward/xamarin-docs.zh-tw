...
[`ListView`](xref:Xamarin.Forms.ListView)提供可滾動清單, 可以透過使用`ViewCell`進行自訂。 `ViewCells` 可以用於顯示文字和影像，表示 true/false 狀態且接收使用者輸入。
...
- 在中`ViewCell`, 版面配置可以由任何 Xamarin 版面配置來管理。 
