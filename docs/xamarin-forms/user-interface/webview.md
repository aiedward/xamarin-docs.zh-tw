...
- HTML & CSS 網站&ndash;的 web 程式可完全支援使用 HTML & css 撰寫的網站，包括 JavaScript 支援。
...
自第9版起，iOS 只會允許您的應用程式與預設執行最佳做法安全性的伺服器進行通訊。 您必須在中`Info.plist`設定值，以啟用與不安全伺服器的通訊。

> [!NOTE]
> 如果您的應用程式需要連線到不安全的網站，您應該一律使用`NSExceptionDomains`來輸入網域做為例外狀況，而不要使用`NSAllowsArbitraryLoads`來完全關閉 ATS。 `NSAllowsArbitraryLoads`應該僅用於極緊急的情況。
...
如果您想要呈現在程式碼中動態定義的 HTML 字串，您必須建立的[`HtmlWebViewSource`](xref:Xamarin.Forms.HtmlWebViewSource)實例：
...
> [!NOTE]
> 您可能需要設定`WidthRequest`的和`HeightRequest`屬性， [`WebView`](xref:Xamarin.Forms.WebView)以查看 HTML 內容，視配置`WebView`是的子系而定。 例如，在中，這是必要[`StackLayout`](xref:Xamarin.Forms.StackLayout)的。
...
雖然已載入第一頁， `WebView`但並不知道 HTML 來自何處。 這是處理參考本機資源的頁面時的問題。 當本機頁面彼此連結時，網頁會使用個別的 JavaScript 檔案，或網頁連結至 CSS 樣式表單，這可能會發生的情況。  

若要解決這個問題，您需要告訴`WebView`哪裡可以在檔案系統上尋找檔案。 若要這麼做， `BaseUrl`請在所`HtmlWebViewSource`使用的上`WebView`設定屬性。
...
請注意，在每個平臺上執行介面之前，應用程式都不會執行。 在一般專案中，請確定您記得`BaseUrl`使用來設定： `DependencyService`
...
在 Android 上，[**資產**] 資料夾中的檔案也可以透過由`MainActivity.Instance`屬性公開的目前 Android 內容來存取：
...
- **GoForward （）** &ndash;如果`CanGoForward`為 true，則`GoForward`呼叫會向前流覽至下一個造訪的頁面。
- **GoBack （）** &ndash;如果`CanGoBack`為 true，則`GoBack`呼叫將會流覽至最後一個造訪的頁面。
- **CanGoBack** &ndash; `true`如果瀏覽器位於起始 URL， `false` CanGoBack 是否有頁面可流覽回。
- **CanGoForward** &ndash; CanGoForward `true`使用者是否已回溯流覽，並可繼續前往已造訪的頁面。

在頁面內`WebView` ，不支援多點觸控手勢。 請務必確定內容已進行行動裝置優化，並出現而不需要縮放。
...
[`Navigating`](xref:Xamarin.Forms.WebView.Navigating)事件隨附的[`WebNavigatingEventArgs`](xref:Xamarin.Forms.WebNavigatingEventArgs)物件有四個屬性：
...
[`Navigated`](xref:Xamarin.Forms.WebView.Navigated)事件隨附的[`WebNavigatedEventArgs`](xref:Xamarin.Forms.WebNavigatedEventArgs)物件有四個屬性：
...
[`WebView`](xref:Xamarin.Forms.WebView)具有可`Reload`用於重載目前內容的方法：
...
叫用`Reload`方法時，會`ReloadRequested`引發事件，表示已提出要求來重載目前的內容。

## <a name="performance"></a>效能

熱門的 web 瀏覽器採用硬體加速轉譯和 JavaScript 編譯等技術。 在 Xamarin. 表單4.4 之前，會透過`WebView` `UIWebView`類別在 iOS 上執行 xamarin。 不過，這其中許多技術在此實行中無法使用。 因此，自 Xamarin. 表單4.4 之後，即`WebView`會在 iOS 上透過`WkWebView`類別來執行 xamarin，以支援更快速的流覽。

> [!NOTE]
> 在 iOS 上， `WkWebViewRenderer`具有接受`WkWebViewConfiguration`引數的函數多載。 這可讓轉譯器在建立時設定。

基於`WebView`相容性的考慮，應用程式`UIWebView`可以傳回使用 IOS 類別來執行 Xamarin。 將下列程式碼新增至應用程式的 iOS 平臺專案中的**AssemblyInfo.cs**檔案，即可達成此目的：
...
為了讓`WebView`能夠正常執行，您必須確定已針對每個平臺設定許可權。 請注意，在某些平臺`WebView`上，會在「偵錯工具」模式下工作，但不會在建立發行時使用。 這是因為某些許可權（例如 Android 上的網際網路存取）預設會在處於「偵錯工具」模式時 Visual Studio for Mac 來設定。

- 在顯示網路內容時， **UWP** &ndash;需要網際網路（用戶端 & 伺服器）功能。
...
[`WebView`](xref:Xamarin.Forms.WebView)包含從 c # 叫用 JavaScript 函式，並將任何結果傳回給呼叫 c # 程式碼的功能。 這是使用[`WebView.EvaluateJavaScriptAsync`](xref:Xamarin.Forms.WebView.EvaluateJavaScriptAsync*)方法來完成，如 [ [web](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/userinterface-webview)工作] 範例中的下列範例所示：
...
[`WebView.EvaluateJavaScriptAsync`](xref:Xamarin.Forms.WebView.EvaluateJavaScriptAsync*)方法會評估指定為引數的 JavaScript，並以的`string`形式傳回任何結果。 在此範例中， `factorial`會`number`叫用 JavaScript 函式，此函式會傳回結果的階乘。 這個 JavaScript 函式會在載入的[`WebView`](xref:Xamarin.Forms.WebView)本機 HTML 檔案中定義，如下列範例所示：
...
Cookie 可以在上設定[`WebView`](xref:Xamarin.Forms.WebView)，然後透過 web 要求傳送至指定的 URL。 這是藉由將`Cookie`物件新增至`CookieContainer`，然後將它設定為可系結屬性`WebView.Cookies`的值來完成。 下列程式碼顯示這種情況的範例：
...
在此範例中，會`Cookie`將單一新增至`CookieContainer`物件，然後將它設定為`WebView.Cookies`屬性的值。 當將[`WebView`](xref:Xamarin.Forms.WebView) web 要求傳送至指定的 URL 時，cookie 會隨要求一起傳送。

## <a name="uiwebview-deprecation-and-app-store-rejection-itms-90809"></a>UIWebView 淘汰和 App Store 拒絕（ITMS-90809）

從2020年4月開始， [Apple 會拒絕](https://developer.apple.com/news/?id=12232019b)仍使用已被`UIWebView`取代之 API 的應用程式。 雖然 Xamarin 已切換為`WKWebView`預設值，但是在 xamarin 中，仍有舊版 SDK 的參考。 目前的[iOS 連結器](~/ios/deploy-test/linker.md)行為並不會移除此動作，因此， `UIWebView`當您提交至 app Store 時，已淘汰的 API 仍會顯示為從您的應用程式參考。

連結器的預覽版本可用來修正此問題。 若要啟用預覽，您必須提供連結器的額外自`--optimize=experimental-xforms-product-type`變數。
...
1. 更新**其他 mtouch 引數**中**的其他 mtouch 引數** &ndash;加入此`--optimize=experimental-xforms-product-type`旗標（除了其中可能已存在的任何值）。 注意：此旗標可搭配設定為 [**僅限 SDK** ] 或 [**全部連結**] 的**連結器行為**一起運作。 如果您基於任何原因，在將連結器行為設定為 [全部] 時看到錯誤，這很可能是應用程式代碼或不是連結器安全之協力廠商程式庫內的問題。 如需連結器的詳細資訊，請參閱[連結 Xamarin IOS 應用程式](~/ios/deploy-test/linker.md)。
1. **更新所有組建** &ndash;設定使用視窗頂端**的 [設定**] 和 [**平臺**] 清單來更新所有組建設定。 更新的最重要設定是**Release/iPhone**設定，因為這通常是用來建立 App Store 提交的組建。
...
1. **流覽至 [ios 組建] 區段** &ndash;選取 [ **ios 組建**] 區段。
1. **更新_mtouch_ ** **其他_mtouch_引數**中的其他 mtouch 引數&ndash;加入此`--optimize=experimental-xforms-product-type`旗標（除了其中可能已存在的任何值）。 注意：此旗標可搭配設定為 [**僅限 SDK** ] 或 [**全部連結**] 的**連結器行為**一起運作。 如果您基於任何原因，在將連結器行為設定為 [全部] 時看到錯誤，這很可能是應用程式代碼或不是連結器安全之協力廠商程式庫內的問題。 如需連結器的詳細資訊，請參閱[連結 Xamarin IOS 應用程式](~/ios/deploy-test/linker.md)。
1. **更新所有組建** &ndash;設定使用視窗頂端**的 [設定**] 和 [**平臺**] 清單來更新所有組建設定。 更新的最重要設定是**Release/iPhone**設定，因為這通常是用來建立 App Store 提交的組建。
