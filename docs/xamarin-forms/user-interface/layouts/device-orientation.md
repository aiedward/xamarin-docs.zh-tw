...
- 直向**會** &ndash;強制應用程式方向為直向，而不論感應器資料為何。
- **使用者** &ndash;會使用使用者慣用的方向來呈現應用程式。
- **後方** &ndash;會導致應用程式的方向與其背後[活動](xref:Android.App.Activity)的方向相同。
- **感應器** &ndash;會導致應用程式的方向由感應器決定，即使使用者已停用自動輪替也一樣。
- **SensorLandscape** &ndash;會讓應用程式在使用感應器資料時，使用橫向方向來變更螢幕的方向（如此一來，螢幕看起來就不會變）。
- **SensorPortrait** &ndash;會讓應用程式在使用感應器資料時，使用直向方向來變更螢幕的方向（如此一來，螢幕看起來就不會變）。
- **ReverseLandscape** &ndash;會讓應用程式使用橫向方向，朝向與平常相反的方向，因此顯示「倒置」。
- **ReversePortrait** &ndash;會讓應用程式使用直向方向，從平常朝相反方向，因此顯示「倒置」。
- **FullSensor** &ndash;會讓應用程式依賴感應器資料來選取正確的方向（可能是4）。
- **FullUser** &ndash;會讓應用程式使用使用者的方向喜好設定。 如果已啟用自動旋轉，則可以使用所有4個方向。
- **UserLandscape** &ndash; _UserLandscape \[不支援\] _會導致應用程式使用橫向方向，除非使用者已啟用自動旋轉，在此情況下，它會使用感應器來判斷方向。 此選項會中斷編譯。
- **UserPortrait** &ndash; _UserPortrait \[不支援\] _會導致應用程式使用直向方向，除非使用者已啟用自動旋轉，在此情況下，它會使用感應器來判斷方向。 此選項會中斷編譯。
- **Locked** &ndash; [_鎖定\[不\]支援_] 會導致應用程式在啟動時使用螢幕方向，而不會回應裝置實體方向的變更。 此選項會中斷編譯。
...
Xamarin 不會提供任何原生事件來通知您的應用程式在共用程式碼中的方向變更。 不過，[Xamarin](~/essentials/index.md)包含一個可提供方向`DeviceDisplay`變更通知的 [] 類別。

若要偵測沒有 Xamarin 的方向，請監視`SizeChanged`的事件`Page`，這是在`Page`變更的寬度或高度時引發。 當的寬度`Page`大於高度時，裝置會處於橫向模式。 如需詳細資訊，請參閱[根據螢幕方向顯示影像](https://github.com/xamarin/recipes/tree/master/Recipes/xamarin-forms/Controls/screen-orientation)。

或者，也可以覆寫上的[`OnSizeAllocated`](xref:Xamarin.Forms.Page.OnSizeAllocated*)方法`Page`，並在該處插入任何版面配置變更邏輯。 每當`OnSizeAllocated` `Page`配置新的大小時，就會呼叫方法，這會在裝置旋轉時發生。 請注意，的基底`OnSizeAllocated`實作用會執行重要的版面配置函式，因此請務必在覆寫中呼叫基底實作為：
...
請注意， `OnSizeAllocated`在旋轉裝置時，可能會多次呼叫方法。 每次變更版面配置會浪費資源，而且可能會導致閃爍。 請考慮使用頁面中的執行個體變數來追蹤方向是橫向或縱向，而且只有在發生變更時才重新繪製：
...
- 請**注意，** &ndash;方向的變更可能會在對比率的特定假設進行時造成問題。 例如，在直向螢幕垂直空間的1/3 中有足夠空間的視圖，可能無法放入橫向的垂直空間1/3。
- 請小心，在直向中合理的**絕對值** &ndash; （圖元）值在橫向可能沒有意義。 當需要絕對值時，請使用嵌套配置來隔離其影響。 例如， `TableView` `ItemTemplate`當專案範本具有保證的統一高度時，在中使用絕對值是合理的。

上述規則也適用于實作為多種螢幕大小的介面時，通常會被視為最佳做法。 本指南的其餘部分將使用 Xamarin 中的每個主要版面配置，說明回應式版面配置的特定範例。

> [!NOTE]
> 為了清楚起見，下列各節將示範如何一次只使用一種類型`Layout`來執行回應式版面配置。 在實務上，使用每個元件的`Layout`更簡單或最直覺`Layout` ，混合來達到所需的版面配置通常會比較簡單。
...
有些 c # 是用來根據裝置的`outerStack`方向來變更的方向：
