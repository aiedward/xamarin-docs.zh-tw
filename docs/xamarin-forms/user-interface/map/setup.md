...
[`Map`](xref:Xamarin.Forms.Maps.Map)控制項是由[Xamarin](https://www.nuget.org/packages/Xamarin.Forms.Maps/) NuGet 封裝所提供，該套件應新增至方案中的每個專案。

安裝[Xamarin](https://www.nuget.org/packages/Xamarin.Forms.Maps/) NuGet 套件之後，必須在每個平臺專案中將其初始化。

在 iOS 上，這應該會**在 AppDelegate.cs**中， `Xamarin.FormsMaps.Init`藉由在`Xamarin.Forms.Forms.Init`方法*之後*叫用方法來進行：

```csharp
Xamarin.FormsMaps.Init();
```

在 Android 上，這應該會**在 MainActivity.cs**中， `Xamarin.FormsMaps.Init`藉由在`Xamarin.Forms.Forms.Init`方法*之後*叫用方法來進行：

```csharp
Xamarin.FormsMaps.Init(this, savedInstanceState);
```

在通用 Windows 平臺（UWP）上，這應該會在**MainPage.xaml.cs**中執行， `Xamarin.FormsMaps.Init`方法是從`MainPage`函式叫用方法：
...
若要支援 iOS 11 和更早版本，您可以包含所有`NSLocationWhenInUseUsageDescription`三`NSLocationAlwaysAndWhenInUseUsageDescription`個金鑰`NSLocationAlwaysUsageDescription`：、和。
...
1. 選擇性在資訊清單中指定位置許可權。
1. 選擇性在`MainActivity`類別中要求執行時間位置許可權。
...
一旦取得 API 金鑰，就必須在**Properties/androidmanifest.xml**檔案的`<application>`元素中新增它：
...
在`<application>` **androidmanifest.xml**的元素中新增下列宣告：
...
如果您的 Xamarin 應用程式以 API 28 或更高版本為目標，您必須在`<application>` **androidmanifest.xml**的元素中新增下列宣告：

```xml
<uses-library android:name="org.apache.http.legacy" android:required="false" />    
```

這會告訴應用程式使用 Apache Http 用戶端程式庫，其已從 Android 9 `bootclasspath`中的移除。

#### <a name="specify-the-write_external_storage-permission"></a>指定 WRITE_EXTERNAL_STORAGE 許可權

如果您的`<manifest>`應用程式以 API 22 或更低的目標，則可能`WRITE_EXTERNAL_STORAGE`需要將許可權新增至資訊清單，做為元素的子系：
...
如果您的應用程式需要存取使用者的位置，您必須將`ACCESS_COARSE_LOCATION`或`ACCESS_FINE_LOCATION`許可權新增至資訊清單（或兩者），做為`<manifest>`元素的子系，以要求許可權：
...
在 UWP 上，您的應用程式必須經過驗證，才能顯示地圖和取用地圖服務。 若要驗證您的應用程式，您必須指定對應驗證金鑰。 如需詳細資訊，請參閱[要求對應的驗證金鑰](/windows/uwp/maps-and-location/authentication-key)。 接著，您應該在`FormsMaps.Init("AUTHORIZATION_TOKEN")`方法呼叫中指定驗證權杖，以向 Bing 地圖服務驗證應用程式。

> [!NOTE]
> 在 UWP 上，若要使用對應服務（例如地理編碼），您`MapService.ServiceToken`也必須將屬性設定為驗證金鑰值。 這可以透過下列程式程式碼來完成： `Windows.Services.Maps.MapService.ServiceToken = "INSERT_AUTH_TOKEN_HERE";`。
...
UWP 發行組建使用 .NET 原生編譯，將應用程式直接編譯成機器碼。 不過，結果是，UWP 上[`Map`](xref:Xamarin.Forms.Maps.Map)控制項的轉譯器可能會從可執行檔連結出去。 您可以使用`Forms.Init` **App.xaml.cs**中方法的 UWP 特定多載來修正此問題：
...
> [!IMPORTANT]
> 若未這麼做，將會導致[`Map`](xref:Xamarin.Forms.Maps.Map)在執行發行組建時，控制項不會出現。
