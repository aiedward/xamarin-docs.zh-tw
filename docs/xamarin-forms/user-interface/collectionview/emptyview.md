...
[![在 iOS 和 Android 上，已交換空白視圖之 CollectionView 垂直清單的螢幕擷取畫面]已(emptyview-images/swap.png "交換空白視圖的 CollectionView 垂直清單")](emptyview-images/swap-large.png#lightbox "已交換空白視圖的 CollectionView 垂直清單")
...
`SearchTermTemplateSelector` 類別定義設定為不同資料範本的 `DefaultTemplate` 與 `OtherTemplate` [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) 屬性。 當搜尋查詢`DefaultTemplate`不等於 "xamarin" 時，覆寫會傳回，以向使用者顯示訊息。`OnSelectTemplate` 當搜尋查詢等於 "xamarin" `OnSelectTemplate`時，覆`OtherTemplate`寫會傳回，以向使用者顯示基本訊息：
