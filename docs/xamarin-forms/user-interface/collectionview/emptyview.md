...
`SearchTermTemplateSelector` 類別會定義設定為不同資料範本的 `DefaultTemplate` 和 `OtherTemplate` [`DataTemplate`](xref:Xamarin.Forms.DataTemplate)屬性。 當搜尋查詢不等於 "xamarin" 時，`OnSelectTemplate` 覆寫會傳回 `DefaultTemplate`，它會向使用者顯示訊息。 當搜尋查詢等於 "xamarin" 時，`OnSelectTemplate` 覆寫會傳回 `OtherTemplate`，以向使用者顯示基本訊息：
