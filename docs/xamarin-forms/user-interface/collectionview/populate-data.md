...
會[`CollectionView`](xref:Xamarin.Forms.CollectionView)藉由將資料的[`ItemsSource`](xref:Xamarin.Forms.ItemsView.ItemsSource)屬性設定為任何可執行檔集合`IEnumerable`，來填入資料。 藉由從字串陣列初始化`ItemsSource`屬性，可以在 XAML 中加入專案：
...
> [!WARNING]
> [`CollectionView`](xref:Xamarin.Forms.CollectionView)如果 UI 執行緒已更新， [`ItemsSource`](xref:Xamarin.Forms.ItemsView.ItemsSource)將會擲回例外狀況。
...
> [!IMPORTANT]
> [`CollectionView`](xref:Xamarin.Forms.CollectionView)如果在基礎集合中加入、移除或變更專案時需要重新整理，基礎集合應該是傳送屬性變更通知的`IEnumerable`集合，例如`ObservableCollection`。

如需如何變更版面配置的[`CollectionView`](xref:Xamarin.Forms.CollectionView)詳細資訊，請參閱[CollectionView 版面](layout.md)配置。 如需如何在中定義每個專案外觀的詳細資訊`CollectionView`，請參閱[定義專案外觀](#define-item-appearance)。

### <a name="data-binding"></a>資料繫結

[`CollectionView`](xref:Xamarin.Forms.CollectionView)可以使用資料系結將其[`ItemsSource`](xref:Xamarin.Forms.ItemsView.ItemsSource)屬性系結至`IEnumerable`集合，以填入資料。 在 XAML 中，這是透過`Binding`標記延伸來達成：
...
將[`CollectionView`](xref:Xamarin.Forms.CollectionView) [`CollectionView.ItemTemplate`](xref:Xamarin.Forms.ItemsView.ItemTemplate)屬性設定為，即可定義中每個專案的外觀[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)：
...
在中指定的元素[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)會定義清單中每個專案的外觀。 在此範例中，中的`DataTemplate`配置是由所[`Grid`](xref:Xamarin.Forms.Grid)管理。 `Grid`包含[`Image`](xref:Xamarin.Forms.Image)物件和兩個[`Label`](xref:Xamarin.Forms.Label)物件，全都系結至`Monkey`類別的屬性：
...
藉由將[`CollectionView.ItemTemplate`](xref:Xamarin.Forms.ItemsView.ItemTemplate)屬性設定為[`DataTemplateSelector`](xref:Xamarin.Forms.DataTemplateSelector)物件[`CollectionView`](xref:Xamarin.Forms.CollectionView) ，可在執行時間根據專案值選擇中每個專案的外觀：
...
`MonkeyDataTemplateSelector`類別會定義`AmericanMonkey`設定`OtherMonkey`為不同資料範本的和[`DataTemplate`](xref:Xamarin.Forms.DataTemplate)屬性。 此`OnSelectTemplate`覆寫會`AmericanMonkey`傳回範本，當猴子名稱包含 "北美洲" 時，會以青色顯示猴子名稱和位置。 當猴子名稱不包含 "北美洲" 時，覆`OnSelectTemplate`寫會傳回`OtherMonkey`範本，以顯示銀級中的猴子名稱和位置：
...
[`CollectionView`](xref:Xamarin.Forms.CollectionView)透過來支援資料項目目的內容功能表`SwipeView`，這會以滑動手勢顯示內容功能表。 `SwipeView`是一個容器控制項，它會包裝內容專案，並提供該內容專案的快顯功能表專案。 因此，會藉`CollectionView`由建立`SwipeView`定義要`SwipeView`包裝之內容的，以及由滑動手勢所顯示的操作功能表項目，來為執行的操作功能表。 將設定`SwipeView`為中的根視圖[`DataTemplate`](xref:Xamarin.Forms.DataTemplate) ，以定義中每個資料項目目的外觀，即可達成此目的： `CollectionView`
...
在此範例中， `SwipeView`內容是[`Grid`](xref:Xamarin.Forms.Grid) ，它會定義中每個專案的外觀[`CollectionView`](xref:Xamarin.Forms.CollectionView)。 [滑動] 專案是用來對`SwipeView`內容執行動作，並在從左側撥動控制項時顯示：
...
[`CollectionView`](xref:Xamarin.Forms.CollectionView)支援透過提取至重新整理功能`RefreshView`，可讓您藉由在專案清單上向下拉出來重新整理顯示的資料。 `RefreshView`是容器控制項，可提供將功能的提取重新整理到其子系，前提是子系支援可滾動的內容。 因此，藉`CollectionView`由將提取重新整理設定為的子系，來為重新`RefreshView`整理執行：
...
[`CollectionView`](xref:Xamarin.Forms.CollectionView)也會定義`RemainingItemsThresholdReached`當向上滾動到`CollectionView`尚未顯示專案的`RemainingItemsThreshold`程度時，所引發的事件。 您可以處理這個事件，以載入更多專案。 此外，當`RemainingItemsThresholdReached`事件引發時， `RemainingItemsThresholdReachedCommand`會執行，以便在 viewmodel 中進行累加式資料載入。
