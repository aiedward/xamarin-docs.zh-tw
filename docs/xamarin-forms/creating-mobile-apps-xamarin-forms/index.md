---
title: '使用書籍建立 Mobile Apps :::no-loc(Xamarin.Forms):::'
description: '透過 :::no-loc(Xamarin.Forms)::: Charles Petzold 的《建立 Mobile Apps 的電子版本來學習應用程式開發 :::no-loc(Xamarin.Forms)::: 。'
ms.prod: xamarin
...
ms.locfileid: "86997393"
---
# <a name="creating-mobile-apps-with-no-locxamarinforms-book"></a>使用書籍建立 Mobile Apps :::no-loc(Xamarin.Forms):::

[![下載範例](~/media/shared/download.png) 下載範例](https://github.com/xamarin/xamarin-forms-book-samples)

<img src="images/cover-sml.png" title="使用書籍建立 Mobile Apps :::no-loc(Xamarin.Forms):::" alt="Creating Mobile Apps with :::no-loc(Xamarin.Forms)::: Book" align="left" />建立 Charles Petzold 的*Mobile Apps :::no-loc(Xamarin.Forms)::: *一書是學習如何撰寫 :::no-loc(Xamarin.Forms)::: 應用程式的指南。 唯一的先決條件是 c # 程式設計語言的知識。 本書提供使用者介面的廣泛探索 :::no-loc(Xamarin.Forms)::: ，同時涵蓋動畫、MVVM、觸發程式、行為、自訂版面配置、自訂轉譯器等等。
...
這些範例[可在 github 上](https://github.com/xamarin/xamarin-forms-book-samples)取得，並包含 IOS、Android 和通用 WINDOWS 平臺（UWP）的專案。 （ :::no-loc(Xamarin.Forms)::: 不再支援 windows 10 行動裝置版，但 :::no-loc(Xamarin.Forms)::: 應用程式將會在 windows 10 桌面上執行）。
...
- API 檔的連結，以取得更詳細的 :::no-loc(Xamarin.Forms)::: 類別、結構、屬性、列舉等描述
...
| 章節 | 完成文字 | 摘要 |
| ------- | ------------- | ------- |
| 第1章。 如何 :::no-loc(Xamarin.Forms)::: 融入？ | [下載 PDF](https://download.xamarin.com/developer/xamarin-forms-book/XamarinFormsBook-Ch01-Apr2016.pdf) | [總結](summaries/chapter01.md) |
...
由於*建立 Mobile Apps :::no-loc(Xamarin.Forms)::: *的發行集後，已在中新增數個新功能 :::no-loc(Xamarin.Forms)::: 。 這些新功能將在檔中的個別文章中加以說明 [:::no-loc(Xamarin.Forms):::](/xamarin/) 。

其他變更導致書籍的某些內容過期：

### <a name="net-standard-20-libraries-have-replaced-portable-class-libraries"></a>.NET Standard 2.0 程式庫已取代可移植的類別庫

:::no-loc(Xamarin.Forms):::應用程式通常會使用程式庫，在不同的平臺之間共用程式碼。 原本，這是可移植的類別庫（PCL）。 本書和章節摘要中有許多 Pcl 的參考。

可移植的類別庫已取代為 .NET Standard 2.0 程式庫，如[中 :::no-loc(Xamarin.Forms)::: 的 .NET Standard 2.0 支援](~/xamarin-forms/internals/net-standard.md)一文所述。 本書中的所有[範例程式碼](https://github.com/xamarin/xamarin-forms-book-samples)都已更新為使用 .NET Standard 2.0 程式庫。

對於 .NET Standard 2.0 程式庫而言，有關可攜性類別庫角色的書籍中，大部分資訊都保持不變。 其中一個差異是只有 PCL 具有數值「設定檔」。 此外，.NET Standard 2.0 程式庫還有一些優點。 例如，第20章， [Async 和 File i/o](summaries/chapter20.md)說明如何使用基礎平臺來執行檔案 i/o。 現已不再需要這麼做。 .NET Standard 2.0 程式庫支援適用于所有平臺的熟悉[System.IO](xref:System.IO)類別 :::no-loc(Xamarin.Forms)::: 。

.NET Standard 2.0 程式庫也可讓 :::no-loc(Xamarin.Forms)::: 應用程式用 [`HttpClient`](xref:System.Net.Http.HttpClient) 來透過網際網路（而不是 [`WebRequest`](xref:System.Net.WebRequest) 或其他類別）存取檔案。

### <a name="the-role-of-xaml-has-been-elevated"></a>XAML 的角色已提高許可權

*建立 Mobile Apps :::no-loc(Xamarin.Forms)::: *一開始會說明如何 :::no-loc(Xamarin.Forms)::: 使用 c # 撰寫應用程式。 在第7章之前，不會引進 Extensible Application Markup Language （XAML） [。XAML 與程式碼](summaries/chapter07.md)。

XAML 現在在中有更大的角色 :::no-loc(Xamarin.Forms)::: 。 :::no-loc(Xamarin.Forms):::隨 Visual Studio 散發的解決方案範本會建立以 XAML 為基礎的分頁檔。 使用的開發人員 :::no-loc(Xamarin.Forms)::: 應該儘早熟悉 XAML。 檔集的可延伸[應用程式標記語言（xaml）](~/xamarin-forms/xaml/index.yml)一節 :::no-loc(Xamarin.Forms)::: 包含幾篇有關 XAML 的文章，讓您開始著手。

### <a name="supported-platforms"></a>支援的平台

:::no-loc(Xamarin.Forms):::不再支援 Windows 8.1 和 Windows Phone 8.1。

本書有時會參考_Windows 執行階段_。 這是一個詞彙，其中包含數個 Windows 版本和 Windows Phone 所使用的 Windows API。 較新的版本 :::no-loc(Xamarin.Forms)::: 會將本身限制為支援通用 Windows 平臺，這是適用于 windows 10 和 windows 10 Mobile 的 API。

.NET Standard 2.0 程式庫不支援任何版本的 Windows 10 Mobile。 因此， :::no-loc(Xamarin.Forms)::: 使用 .NET Standard 程式庫的應用程式不會在 Windows 10 Mobile 裝置上執行。 :::no-loc(Xamarin.Forms):::應用程式會繼續在 Windows 10 desktop （10.0.16299.0 和更新版本）上執行。

:::no-loc(Xamarin.Forms):::具有[Mac](~/xamarin-forms/platform/other/mac.md)、 [WPF](~/xamarin-forms/platform/other/wpf.md)、 [GTK #](~/xamarin-forms/platform/other/gtk.md)和[Tizen](~/xamarin-forms/platform/other/tizen.md)平臺的預覽支援。

### <a name="chapter-summaries"></a>章節摘要

章節摘要包含 :::no-loc(Xamarin.Forms)::: 自本書撰寫後有關中變更的資訊。 這些通常是以附注的形式呈現：

> [!NOTE]
> 每個頁面上的附注會指出 :::no-loc(Xamarin.Forms)::: 從書籍中所呈現之材質分歧的位置。
