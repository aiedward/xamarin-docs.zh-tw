...
@No__t_0 方法會傳回 `boolean`，指出值引數是 `null`、空白，或只包含空白字元。
...
@No__t_0 方法會傳回 `boolean`，指出值引數是否為有效的電子郵件地址。 藉由在 `Regex` 的函式中指定的正則運算式模式第一次出現時，搜尋 value 引數來達成此目的。 是否在輸入字串中找到正則運算式模式，可以藉由檢查 `Match` 物件的 `Success` 屬性的值來判斷。
...
@No__t_0 方法藉由在每個 `ValidatableObject<T>` 實例上叫用 Validate 方法，來執行使用者在 `LoginView` 上輸入的使用者名稱和密碼驗證。 下列程式碼範例顯示來自 `ValidatableObject<T>` 類別的 Validate 方法：
...
@No__t_0 附加的行為可用來反白顯示驗證錯誤發生所在的[`Entry`](xref:Xamarin.Forms.Entry)控制項。 下列程式碼範例示範如何將 `LineColorBehavior` 附加行為附加至 `Entry` 控制項：
...
[@No__t_1](xref:Xamarin.Forms.Entry)控制項會使用明確的樣式，如下列程式碼範例所示：
...
@No__t_0 會將[`RoutingEffect`](xref:Xamarin.Forms.RoutingEffect)類別子類別化，如下列程式碼範例所示：
...
[@No__t_1](xref:Xamarin.Forms.RoutingEffect)類別代表以平臺為獨立的效果，它會包裝平臺特定的內部效果。 這可簡化效果移除程序，因為對於平台特定效果，並不存在對類型資訊的編譯時間資訊存取。 @No__t_0 會呼叫基類處理常式，傳入包含解析組名串連的參數，以及在每個平臺特定效果類別上指定的唯一識別碼。
...
@No__t_0 方法會抓取 Xamarin [`Entry`](xref:Xamarin.Forms.Entry)控制項的原生控制項，並藉由呼叫 `UpdateLineColor` 方法來更新線條色彩。 @No__t_0 覆寫會藉由更新附加的 `LineColor` 屬性變更時的線條色彩，或 `Entry` 變更的[`Height`](xref:Xamarin.Forms.VisualElement.Height)屬性，來回應 `Entry` 控制項上可系結的屬性變更。 如需效果的詳細資訊，請參閱[效果](~/xamarin-forms/app-fundamentals/effects/index.md)。
...
[@No__t_1](xref:Xamarin.Forms.Entry)控制項也已將[`DataTrigger`](xref:Xamarin.Forms.DataTrigger)新增至其[`Triggers`](xref:Xamarin.Forms.VisualElement.Triggers)集合。 下列程式碼範例顯示 `DataTrigger`：
...
每個[`Label`](xref:Xamarin.Forms.Label)都會系結至要驗證之視圖模型物件的 `Errors` 屬性。 @No__t_0 屬性是由 `ValidatableObject<T>` 類別所提供，且屬於類型 `List<string>`。 因為 `Errors` 屬性可以包含多個驗證錯誤，所以 `FirstValidationErrorConverter` 實例會用來從集合中取出第一個錯誤，以供顯示。

## <a name="summary"></a>總結

EShopOnContainers 行動應用程式會執行 view model 屬性的同步用戶端驗證，並藉由反白顯示包含無效資料的控制項來通知使用者任何驗證錯誤，以及顯示通知使用者的錯誤訊息資料不正確原因。

需要驗證的視圖模型屬性是 `ValidatableObject<T>` 類型，而且每個 `ValidatableObject<T>` 實例都已將驗證規則新增至其 `Validations` 屬性。 藉由呼叫 `ValidatableObject<T>` 實例的 `Validate` 方法，從視圖模型叫用驗證，它會抓取驗證規則，並針對 `ValidatableObject<T>` `Value` 屬性加以執行。 任何驗證錯誤都會放入 `ValidatableObject<T>`instance 的 `Errors` 屬性，並更新 `ValidatableObject<T>` 實例的 `IsValid` 屬性，以指出驗證成功或失敗。
