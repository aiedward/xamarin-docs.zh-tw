...
這個方法會呼叫 `CreateHttpClient` 方法，它會傳回已 `HttpClient` 設定適當標頭之類別的實例。 然後，它會將非同步刪除要求提交給 URI 所識別的資源。 如需方法的詳細資訊 `CreateHttpClient` ，請參閱[提出 GET 要求](#making-a-get-request)。
