...
原生表單可讓 Xamarin [`ContentPage`](xref:Xamarin.Forms.ContentPage)衍生的頁面由原生專案使用，包括 GTK # 專案。 這可以藉由建立[`ContentPage`](xref:Xamarin.Forms.ContentPage)衍生頁面的實例，然後使用 `CreateContainer` 擴充方法將它轉換成原生 GTK # 型別來完成：
