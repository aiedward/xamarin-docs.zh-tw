---
title： "GTK # 平臺安裝程式" description： " Xamarin.Forms 現已提供 GTK # platform" assetid：3417FB95-3E4B-47DA-85D0-F34832747236 的預覽支援。技術： xamarin-forms author： davidbritch ms-chap： dabritch ms. date： 04/10/2018 no-loc： [ Xamarin.Forms ， Xamarin.Essentials ]
---
...
Xamarin.Forms現在提供 GTK # 應用程式的預覽支援。 GTK # 是一種圖形化使用者介面工具組，可連結 GTK + 工具組和各種 GNOME 程式庫，允許使用 Mono 和 .NET 開發完全原生的 GNOME 圖形應用程式。 本文示範如何將 GTK # 專案新增至 Xamarin.Forms 方案。

> [!IMPORTANT]
> Xamarin.Forms對 GTK # 的支援是由「社區」提供。 如需詳細資訊，請參閱[ Xamarin.Forms 平臺支援](https://github.com/xamarin/Xamarin.Forms/wiki/Platform-Support)。

在您開始之前，請先建立新的 Xamarin.Forms 解決方案，或使用現有的 Xamarin.Forms 解決方案，例如[**GameOfLife**](https://docs.microsoft.com/samples/xamarin/xamarin-forms-samples/boxview-gameoflife)。
...
2. 在 [**新增專案**] 視窗中，選取左側的 [ **Visual c #** ] 和 [ **Windows 傳統桌面**]。 在專案類型清單中，選擇 [**類別庫（.NET Framework）**]，並確定 [ **Framework** ] 下拉式清單已設定為最小值 .NET Framework 4.7。
...
4. 在 [**方案總管**中，以滑鼠右鍵按一下新的 GTK 專案，然後選取 [**管理 NuGet 套件**]。 選取 [**流覽**] 索引標籤，然後搜尋 **Xamarin.Forms** 3.0 或更高版本。

    ![選取 Xamarin.Forms NuGet 套件](gtk-images/win/select-forms-nuget-package.png "選取 [！OP.無 LOC （Xamarin）] NuGet 套件")

    選取套件，然後按一下 [**安裝**] 按鈕。

5. 現在，搜尋** Xamarin.Forms 。Platform.string** 3.0 封裝或更新版本。

    ![選取 [] Xamarin.Forms 。平臺 GTK NuGet 套件](gtk-images/win/select-forms-platform-nuget-package.png "選取 [！OP.無-LOC （Xamarin）]。平臺 GTK NuGet 套件")
...
10. 在 GTK 專案中，編輯**Program.cs**檔案，使其類似下列程式碼：
    ...
    此程式碼會初始化 GTK #，並 Xamarin.Forms 建立應用程式視窗，並執行應用程式。
...
1. 在 Visual Studio for Mac 中，以滑鼠右鍵按一下 Xamarin.Forms 方案，然後選擇 [新增] **> [加入新專案**]。

2. 在 [**新增專案**] 視窗中，選擇 [**其他 > .net > Gtk # 2.0 專案**，然後按 **[下一步]**。

3. 輸入具有**GTK**延伸模組的專案名稱，例如**GameOfLife**，然後按 [**建立**]。

4. 在 [ **Solution Pad**中，以滑鼠右鍵按一下 GTK 專案的 [**封裝] > [新增套件 ...** ]，然後新增 Xamarin.Forms 3.0 發行前版本 NuGet 封裝或更新版本。

    ![選取 Xamarin.Forms NuGet 套件](gtk-images/mac/select-forms-nuget-package.png "選取 [！OP.無 LOC （Xamarin）] NuGet 套件")

5. 在 [ **Solution Pad**中，以滑鼠右鍵按一下 GTK 專案的 [**封裝] > [新增套件 ...** ]，然後新增 Xamarin.Forms 。Platform.string 3.0 預先發行版本 NuGet 套件或更新版本。

    ![選取 [] Xamarin.Forms 。平臺 GTK NuGet 套件](gtk-images/mac/select-forms-platform-nuget-package.png "選取 [！OP.無-LOC （Xamarin）]。平臺 GTK NuGet 套件")

6. 更新其他平臺專案，以使用 Xamarin.Forms GTK 專案所使用的相同版本。

7. 在 [ **Solution Pad**中，以滑鼠右鍵按一下 GTK 專案 **> 編輯參考 ...** ] 的 [參考]，然後加入專案的參考 Xamarin.Forms （[.NET Standard] 或 [共用專案]）。
...
您可以 Xamarin.Forms 從 XAML 或程式碼判斷您的應用程式在哪一個平臺上執行。 這可讓您在 GTK # 上執行程式特性時，加以變更。 在程式碼中，將的值 `Device.RuntimePlatform` 與 `Device.GTK` 常數（等於字串 "GTK"）進行比較。 如果有相符的，應用程式就會在 GTK # 上執行。
...
GTK # 有各種可用的主題，可以從 Xamarin.Forms 應用程式中使用：
...
原生表單可讓 Xamarin.Forms [`ContentPage`](xref:Xamarin.Forms.ContentPage) 衍生的頁面由原生專案使用，包括 GTK # 專案。 這可以藉由建立 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 衍生頁面的實例，並使用擴充方法將它轉換成原生 GTK # 類型來完成 `CreateContainer` ：
