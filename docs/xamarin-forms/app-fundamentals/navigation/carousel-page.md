...
下列 XAML 程式碼範例會顯示一個 [`CarouselPage`](xref:Xamarin.Forms.CarouselPage)，其建構方式是透過將 [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) 指派給 [`ItemTemplate`](xref:Xamarin.Forms.MultiPage`1.ItemTemplate) 屬性，來傳回集合中物件的頁面：
