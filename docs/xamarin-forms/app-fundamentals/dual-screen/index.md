...
## <a name="dual-screen-layout"></a>[雙畫面版面配置](twopaneview.md)

Xamarin.Forms `TwoPaneView` 類別 (因同名 UWP 控制項產生的靈感) 為針對雙螢幕裝置最佳化的跨平台版面配置。

## <a name="dual-screen-device-capabilities"></a>[雙畫面裝置功能](dual-screen-info.md)

`DualScreenInfo` 類別可讓您決定檢視所在的窗格、窗格大小、裝置方向、鉸鏈角度等。

## <a name="dual-screen-platform-helpers"></a>[雙畫面平臺協助程式](dual-screen-helper.md)

`DualScreenHelper` 類別可讓您確認平台是否支援以子母畫面形式開啟新視窗。 若為 Neo，這可讓您在裝置處於撰寫模式時，開啟會以觸控條顯示的視窗。

## <a name="dual-screen-triggers"></a>[雙畫面觸發程式](triggers.md)

[`Xamarin.Forms.DualScreen`](xref:Xamarin.Forms.DualScreen)命名空間包含兩個狀態觸發程式，會在附加的配置或視窗的視圖模式變更時，觸發[`VisualState`](xref:Xamarin.Forms.VisualState)變更。
