...
如有需要，可以透過可系結屬性將飛出視窗的背景色彩設定為 [`Color`](xref:Xamarin.Forms.Color) `Shell.FlyoutBackgroundColor` 。 這個屬性也可以從階層式樣式表 (CSS) 進行設定。 如需詳細資訊，請參閱[ Xamarin.Forms Shell 特定屬性](~/xamarin-forms/user-interface/styles/css/index.md#xamarinforms-shell-specific-properties)。

## <a name="flyout-icon"></a>飛出視窗圖示

根據預設，Shell 應用程式有個漢堡圖示，按下圖示時，即會開啟飛出視窗。 您可以將類型的可系結 `Shell.FlyoutIcon` 屬性設定 [`ImageSource`](xref:Xamarin.Forms.ImageSource) 為適當的圖示，以變更此圖示：
...
或者，您可以藉由將屬性設定為，來定義飛出視窗標題的外觀 `Shell.FlyoutHeaderTemplate` [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) ：
...
飛出視窗可有選擇性的背景影像，它會出現在飛出視窗的標題下方，和所有飛出視窗項目和功能表項目的後方。 您可以將類型的可系結 `FlyoutBackgroundImage` 屬性設定為檔案 [`ImageSource`](xref:Xamarin.Forms.ImageSource) 、內嵌資源、URI 或資料流程，藉以指定背景影像。

您可以設定背景影像的外觀比例，方法是將類型的可系結 `FlyoutBackgroundImageAspect` 屬性設定 [`Aspect`](xref:Xamarin.Forms.Aspect) 為其中一個 `Aspect` 列舉成員：

- [`AspectFill`](xref:Xamarin.Forms.Aspect.AspectFill) -裁剪影像，使其填滿顯示區域，同時保留外觀比例。
- [`AspectFit`](xref:Xamarin.Forms.Aspect.AspectFit) -letterboxes 影像（如有必要），讓影像符合顯示區域，並根據影像的寬度或高度，將空格新增至頂端/底部或側邊。
...
在此範例中，每個都只能 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 透過飛出視窗專案來存取：

[![螢幕擷取畫面：在 iOS 和 Android 上具有飛出視窗專案的 Shell 兩頁應用程式](flyout-images/two-page-app-flyout.png "具有飛出視窗專案的 Shell 兩頁應用程式")](flyout-images/two-page-app-flyout-large.png#lightbox "具有飛出視窗專案的 Shell 兩頁應用程式")
...
這項隱含的轉換會自動 [`ContentPage`](xref:Xamarin.Forms.ContentPage) 將物件中的每個物件包裝在物件中，而這些物件會包裝在物件中 `ShellContent` `Tab` `FlyoutItem` 。
...
- `IsVisible`型別為的， `bool` 表示 `FlyoutItem` 是否隱藏在飛出視窗功能表上。 其預設值為 `true`。
...
除了屬性以外的所有屬性 `Route` 都是由物件所支援 [`BindableProperty`](xref:Xamarin.Forms.BindableProperty) ，這表示屬性可以是資料系結的目標。
...
您可以藉由將附加屬性設定為，來指定飛出視窗的背景（飛出視窗重迭的外觀） `Shell.FlyoutBackdrop` `Brush` ：
...
在此範例中，會以銀級繪製飛出視窗的背景 `SolidColorBrush` 。

> [!IMPORTANT]
> 您 `FlyoutBackdrop` 可以在任何 Shell 元素上設定 Th 附加的屬性，但是只有在其設定為 `Shell` 、或物件時才會套用 `FlyoutItem` `TabBar` 。

下列範例顯示將的飛出視窗背景設定 `FlyoutItem` 為 `LinearGradientBrush` ：
...
根據預設，當飛出視窗專案不符合飛出視窗時，可以垂直捲動飛出視窗。 藉由將可系結屬性設定 `Shell.FlyoutVerticalScrollMode` 為其中一個列舉成員，即可變更此行為 `ScrollMode` ：

- `Disabled` –表示將停用垂直捲動。
- `Enabled` –表示將啟用垂直捲動。
- `Auto` –表示如果飛出視窗中的飛出視窗專案無法容納，則會啟用垂直捲動。 此為 `Shell.FlyoutVerticalScrollMode` 屬性的預設值。
...
將 `FlyoutItem` 附加屬性設定為，即可自訂每個的外觀 `Shell.ItemTemplate` [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) ：
...
因為 `Shell.ItemTemplate` 是附加屬性，所以可以將不同的範本附加至特定 `FlyoutItem` 物件。

> [!NOTE]
> Shell 會提供的 `Title` 和 `FlyoutIcon` 屬性至的 [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext) `ItemTemplate` 。

此外，Shell 還包含三個會自動套用至物件的樣式類別 `FlyoutItem` 。 如需詳細資訊，請參閱 [FlyoutItem 和 MenuItem 樣式類別](#flyoutitem-and-menuitem-style-classes)。

### <a name="default-template-for-flyoutitems"></a>FlyoutItems 的預設範本

每個使用的預設值如下 [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) `FlyoutItem` 所示：
...
對等的 c # 程式碼而言，假設有一個名為的 `ShellContent` 物件 `aboutItem` ，就是：

```csharp
CurrentItem = aboutItem;
```

在此範例中， `CurrentItem` 屬性是在子類別化 `Shell` 類別中設定。 或者，您 `CurrentItem` 可以透過靜態屬性在任何類別中設定屬性 `Shell.Current` ：
...
您可以選擇性地將功能表項目加入至飛出視窗，每個功能表項目都由一個物件來表示 [`MenuItem`](xref:Xamarin.Forms.MenuItem) 。 飛出視窗上 `MenuItem` 物件的位置，取決於其在 Shell 視覺階層中的宣告順序。 因此，在 `FlyoutItem` 物件之前宣告的任何 `MenuItem` 物件將出現在飛出視窗的頂端，而在 `FlyoutItem` 物件之後宣告的任何 `MenuItem` 物件將出現在飛出視窗的底部。
...
此程式碼會將兩個 [`MenuItem`](xref:Xamarin.Forms.MenuItem) 物件加入至飛出視窗的所有飛出視窗專案底下：
...
第一個 [`MenuItem`](xref:Xamarin.Forms.MenuItem) 物件會執行 `ICommand` 名為 `RandomPageCommand` 的，它會流覽至應用程式中的隨機頁面。 第二個 `MenuItem` 物件會執行名為 `HelpCommand` 的 `ICommand`，這會在網頁瀏覽器中開啟 `CommandParameter` 屬性所指定的 URL。

> [!NOTE]
> [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext)每個都繼承自子類別化的 `MenuItem` `Shell` 物件。

## <a name="define-menuitem-appearance"></a>定義 MenuItem 的外觀

將 `MenuItem` 附加屬性設定為，即可自訂每個的外觀 `Shell.MenuItemTemplate` [`DataTemplate`](xref:Xamarin.Forms.DataTemplate) ：
...
> [!NOTE]
> Shell 會提供的 [`Text`](xref:Xamarin.Forms.MenuItem.Text) 和 [`IconImageSource`](xref:Xamarin.Forms.MenuItem.IconImageSource) 屬性至的 [`BindingContext`](xref:Xamarin.Forms.BindableObject.BindingContext) `MenuItemTemplate` 。 您也可以使用 `Title` 取代 `Text` 和取代， `Icon` `IconImageSource` 讓您針對功能表項目和飛出視窗專案重複使用相同的範本
...
> [!NOTE]
> 物件的預設範本 `FlyoutItem` 也可用於 `MenuItem` 物件。 如需詳細資訊，請參閱 [FlyoutItems 的預設範本](#default-template-for-flyoutitems)。

## <a name="flyoutitem-and-menuitem-style-classes"></a>FlyoutItem 和 MenuItem 樣式類別

Shell 包含三個樣式的類別，這些類別會自動套用至 `FlyoutItem` 和 `MenuItem` 物件。 樣式類別名稱如下：
...
此外，您可以定義自訂樣式類別，並將其套用至 `FlyoutItem` 和 `MenuItem` 物件。 如需樣式類別的詳細資訊，請參閱[ Xamarin.Forms 樣式類別](~/xamarin-forms/user-interface/styles/xaml/style-class.md)。
