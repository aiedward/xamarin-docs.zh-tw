...
但對於為何 `Slider` 仍繼續運作，您可能會感到驚訝，如 Android 螢幕擷取畫面所示。 這似乎表示由於初始化如我們預期般運作，因此當繫結目標是 `Slider` 而非 `Label` 時，資料繫結運作效果較佳。
...
- [`TwoWay`](xref:Xamarin.Forms.BindingMode.TwoWay) &ndash; 資料會在來源與目標之間雙向移動
...
- [`OneTime`](xref:Xamarin.Forms.BindingMode.OneWayToSource) &ndash; 資料會從來源移到目標，但只有在 `BindingContext` 變更時才會發生 (Xamarin.Forms 3.0 的新功能)
...
當您操作滑桿時，`BoxView` 與 `Label` 會隨之更新，如 Android 螢幕擷取畫面所示。
