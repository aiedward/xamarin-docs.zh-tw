...
_Xamarin.Forms ListView 是一個檢視,它將數據集合顯示為垂直清單。本文演示如何創建自定義呈現器,該呈現器封裝特定於平臺的清單控制項和本機單元格佈局,從而允許對本機清單控制性能進行更多控制。_

每個 Xamarin.Forms 檢視都隨附每個平台的轉譯器，這些平台可建立原生控制項的執行個體。 [`ListView`](xref:Xamarin.Forms.ListView)當 由 Xamarin.Forms 應用程式呈現 時,在`ListViewRenderer`iOS 中, 類將實例化`UITableView`,從而實例化本機 控件。 在 Android 平台上，`ListViewRenderer` 類別會具現化原生 `ListView` 控制項。 在通用 Windows 平台 (UWP) 上，`ListViewRenderer` 類別會具現化原生 `ListView` 控制項。 如需 Xamarin.Forms 控制項對應的轉譯器和原生控制項類別詳細資訊，請參閱[轉譯器基底類別和原生控制項](~/xamarin-forms/app-fundamentals/custom-renderer/renderers.md)。

下圖說明了[`ListView`](xref:Xamarin.Forms.ListView)控制項與實現控制項的相應本機控制項之間的關係:

![](listview-images/listview-classes.png "Relationship Between the ListView Control and the Implementing Native Controls")

渲染過程可以通過為每個平臺上的 創建[`ListView`](xref:Xamarin.Forms.ListView)自定義 呈現器來實現特定於平臺的自定義。 執行這項作業的流程如下：
...
可以通過[`ListView`](xref:Xamarin.Forms.ListView)`ListView`對 類進行子類類化來創建自定義控制項,如以下代碼範例所示:
...
> [!NOTE]
> 由於`NativeListView`自訂控制項將使用包含捲動功能的平台特定清單控制項進行呈現,因此自訂控制項不應託管在可捲動的佈局控制項(如[`ScrollView`](xref:Xamarin.Forms.ScrollView))
...
1. 覆寫轉譯自訂控制項的 `OnElementChanged` 方法，並撰寫自訂方法的邏輯。 創建相應的 Xamarin.表[`ListView`](xref:Xamarin.Forms.ListView)單時,將調用此方法。
