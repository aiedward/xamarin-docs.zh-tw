---
title: 反白顯示在地圖上的路由
description: 本文說明如何將聚合線條重疊影像加入至地圖。 聚合線條覆疊為一系列連接的直線線段，通常可用來在對應上，顯示路由，或形成所需的任何圖形。
ms.prod: xamarin
...
---
# <a name="highlighting-a-route-on-a-map"></a>反白顯示在地圖上的路由

_本文說明如何將聚合線條重疊影像加入至地圖。聚合線條覆疊為一系列連接的直線線段，通常可用來在對應上，顯示路由，或形成所需的任何圖形。_

## <a name="overview"></a>總覽

重疊，在地圖上的多層式的圖形。 覆疊支援會隨著與地圖已縮放的繪圖圖形內容。 下列螢幕擷取畫面顯示新增至對應的聚合線條重疊的結果：
...
建立的子類別[ `Map` ](https://developer.xamarin.com/api/type/Xamarin.Forms.Maps.Map/)將新增的類別`RouteCoordinates`屬性：
...
`RouteCoordinates`屬性會儲存這些座標會定義要反白顯示之路由的集合。
...
- 緯度和經度座標的集合會從擷取`CustomMap.RouteCoordinates`屬性並儲存成陣列的`CLLocationCoordinate2D`執行個體。
- 將聚合線條由呼叫靜態`MKPolyline.FromCoordinates`方法，其指定的緯度與經度的每個點。
- 將聚合線條，會藉由呼叫加入至地圖`MKMapView.AddOverlay`方法。
...
建立的子類別`MapRenderer`類別並覆寫其`OnElementChanged`和`OnMapReady`方法，將聚合線條重疊：
...
`OnElementChanged`方法會擷取的緯度和經度座標，從集合`CustomMap.RouteCoordinates`屬性並將它們儲存在成員變數中。 然後它會呼叫`MapView.GetMapAsync`方法，取得基礎`GoogleMap`，為繫結至檢視，前提是自訂轉譯器附加至新的 Xamarin.Forms 項目。 一次`GoogleMap`執行個體可供使用，`OnMapReady`方法將會叫用，聚合線條由具現化`PolylineOptions`物件，指定的緯度與經度的每個點。 將聚合線條然後藉由呼叫加入至地圖`NativeMap.AddPolyline`方法。
...
- 緯度和經度座標的集合會從擷取`CustomMap.RouteCoordinates`屬性和已轉換成`List`的`BasicGeoposition`座標。
- 將聚合線條由具現化`MapPolyline`物件。 `MapPolygon`類別用來在地圖上顯示一條線，藉由設定其`Path`屬性`Geopath`包含線條座標的物件。
- 將聚合線條會轉譯在地圖上將它加入至`MapControl.MapElements`集合。

## <a name="summary"></a>總結

本文說明如何將聚合線條重疊影像加入至地圖中，若要在地圖中，顯示路由，或形成所需的任何圖形。
