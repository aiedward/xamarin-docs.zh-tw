---
title: Xamarin 中的字串和影像當地語系化
description: Xamarin。您可以使用 .NET 資源檔來當地語系化表單應用程式。
...
使用資源檔來當地語系化 Xamarin。 Forms 應用程式會要求您執行下列步驟：

1. [建立](#create-resx-files)包含翻譯文字的 Resx 檔案。
1. 指定共用專案中[的預設文化](#specify-the-default-culture)特性。
1. [在 [Xamarin. 表單] 中將文字當地語系化](#localize-text-in-xamarinforms)。
...
建立預設資源檔之後，您可以在資源檔的 `root` 元素中建立 `data` 元素來新增文字：
...
將**自訂工具**設定為**publicresxfilecodegenerator**會產生具有 `public` 存取權的類別。 將**自訂工具**設定為**InternalResXFileCodeGenerator**會產生具有 `internal` 存取權的產生類別。 空白的**自訂工具**值不會產生類別。 產生的類別名稱會符合資源檔名稱。 例如， **appresources.resx properties .resx**檔案會導致在名為**AppResources.designer.cs**的檔案中建立 `AppResources` 類別。
...
若要讓資源檔正確運作，應用程式必須指定 `NeutralResourcesLanguage`。 在共用的專案中，應自訂**AssemblyInfo.cs**檔案以指定預設的文化特性。 下列程式碼顯示如何在**AssemblyInfo.cs**檔案中將 `NeutralResourcesLanguage` 設定為**en-us** ：
...
> [!WARNING]
> 如果您未指定 `NeutralResourcesLanguage` 屬性，`ResourceManager` 類別就會傳回任何不含特定資源檔之文化特性的 `null` 值。 指定預設文化特性時，`ResourceManager` 會從預設的 Resx 檔案傳回不支援文化特性的結果。 因此，建議您一律指定 `NeutralResourcesLanguage`，以便針對不支援的文化特性顯示文字。
...
## <a name="localize-text-in-xamarinforms"></a>在 Xamarin 中將文字當地語系化

文字在 Xamarin 中是使用產生的 `AppResources` 類別來當地語系化。 這個類別是根據預設資源檔名稱來命名。 由於範例專案資源檔的名稱為**AppResources.cs**，Visual Studio 會產生一個稱為 `AppResources`的相符類別。 系統會針對資源檔中的每個資料列，在 `AppResources` 類別中產生靜態屬性。 下列靜態屬性會在範例應用程式的 `AppResources` 類別中產生：
...
`AppResources` 類別中的屬性會使用 `System.Globalization.CultureInfo.CurrentUICulture` 的目前值來判斷要從哪一個文化特性資源檔抓取值。
...
由於每個平臺都使用唯一的檔案結構來儲存影像，因此 XAML 會使用 `OnPlatform` 類別，根據目前的平臺來設定 `ImageSource` 屬性：
...
> [!NOTE]
> `OnPlatform` 標記延伸提供更精簡的方式來指定平臺特定的值。 如需詳細資訊，請參閱[OnPlatform 標記延伸](~/xamarin-forms/xaml/markup-extensions/consuming.md#onplatform-markup-extension)。

可以根據程式碼中的 `Device.RuntimePlatform` 屬性來設定影像來源：
...
您可以藉由變更您的裝置語言，來測試當地語系化的效果。 您可以在程式碼中設定 `System.Globalization.CultureInfo.CurrentUICulture` 的值，但在不同的平臺上行為不一致，因此不建議用於測試。
