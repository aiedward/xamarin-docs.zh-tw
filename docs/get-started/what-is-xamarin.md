...
1. **行動裝置跨平台支援** – Xamarin 針對三個主要平台 (iOS、Android，以及 Windows) 提供複雜的跨平台支援。 您可以撰寫應用程式來共用最多90% 的程式碼，並 Xamarin.Essentials 提供統一的 API 來存取所有三個平臺上的通用資源。 共用程式碼可以大幅降低行動裝置開發人員的開發成本以及上市所需時間。
...
Xamarin.Essentials 是可為原生裝置功能提供跨平臺 Api 的程式庫。 和 Xamarin 本身一樣， Xamarin.Essentials 也是簡化存取原生功能程式的抽象概念。 提供的一些功能範例 Xamarin.Essentials 包括：
...
Xamarin.Forms 是開放原始碼 UI 架構。 Xamarin.Forms 可讓開發人員從單一共用程式碼基底建立 Xamarin. iOS、Xamarin. Android 和 Windows 應用程式。 Xamarin.Forms 可讓開發人員使用 c # 中的程式碼後端，在 XAML 中建立使用者介面。 這些使用者介面會於每個平台上轉譯為高效能的原生控制項。 提供的一些功能範例 Xamarin.Forms 包括：
...
- [入門 Xamarin.Forms](~/xamarin-forms/index.yml)
