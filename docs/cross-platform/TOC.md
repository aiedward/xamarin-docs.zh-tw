# [Cross-Platform](index.yml)
## [快速入門](get-started/index.md)
### [Requirements](get-started/requirements.md)
### [安裝](get-started/installation/index.md)
#### [在 Windows 上的 Visual Studio 安裝 Xamarin](get-started/installation/windows.md)
#### [安裝 Visual Studio for Mac](/visualstudio/mac/installation/)
#### [Xamarin 防火牆組態指示](get-started/installation/firewall.md)
#### [解除安裝 Xamarin](get-started/installation/uninstalling-xamarin.md)
### [行動裝置應用程式開發簡介](get-started/introduction-to-mobile-development.md)
### [行動裝置 SDLC 簡介](get-started/introduction-to-mobile-sdlc.md)
## [應用程式基本概念](app-fundamentals/index.md)
### [協助工具選項](app-fundamentals/accessibility.md)
### [建置跨平台應用程式](app-fundamentals/building-cross-platform-applications/index.md)
#### [概觀](app-fundamentals/building-cross-platform-applications/part-0-overview.md)
#### [部分 1 - 了解 Xamarin 行動裝置平台](app-fundamentals/building-cross-platform-applications/part-1-understanding-the-xamarin-mobile-platform.md)
#### [部分 2 - 架構](app-fundamentals/building-cross-platform-applications/part-2-architecture.md)
#### [部分 3 - 設定 Xamarin 跨平台解決方案](app-fundamentals/building-cross-platform-applications/part-3-setting-up-a-xamarin-cross-platform-solution.md)
#### [部分 4 - 處理多個平台](app-fundamentals/building-cross-platform-applications/part-4-platform-divergence-abstraction-divergent-implementation.md)
#### [部分 5 - 實用的程式碼共用策略](app-fundamentals/building-cross-platform-applications/part-5-practical-code-sharing-strategies.md)
#### [部分 6 - 測試和 App Store 核准](app-fundamentals/building-cross-platform-applications/part-6-testing-and-app-store-approvals.md)
#### [案例研究：Tasky](app-fundamentals/building-cross-platform-applications/case-study-tasky.md)
### [程式碼共用選項](app-fundamentals/code-sharing.md)
### [可攜式類別庫](app-fundamentals/pcl.md)
### [共用的專案](app-fundamentals/shared-projects.md)
### [.NET Standard](app-fundamentals/net-standard.md)
### [NuGet 套件 (自動)](app-fundamentals/nuget-multiplatform-libraries/index.md)
#### [現有的程式庫專案](app-fundamentals/nuget-multiplatform-libraries/existing-library.md)
#### [新的多平台程式庫](app-fundamentals/nuget-multiplatform-libraries/single-codebase.md)
#### [新的平台特定程式庫](app-fundamentals/nuget-multiplatform-libraries/platform-specific.md)
#### [中繼資料指南](app-fundamentals/nuget-multiplatform-libraries/metadata.md)
### [NuGet 套件 (手動)](app-fundamentals/nuget-manual.md)
### [當地語系化](app-fundamentals/localization.md)
### [傳輸層安全性 (TLS)](app-fundamentals/transport-layer-security.md)
## [平台功能](platform/index.md)
### [非同步支援概觀](platform/async.md)
### [C# 6 語言功能](platform/csharp-six.md)
### [F#](platform/fsharp/index.md)
#### [快速入門](platform/fsharp/overview.md)
#### [F# 範例](platform/fsharp/samples.md)
### [Visual Basic.NET](platform/visual-basic/index.md)
#### [Xamarin iOS 和 Android](platform/visual-basic/native-apps.md)
#### [Xamarin.Forms](platform/visual-basic/xamarin-forms.md)
### [Razor HTML 範本](platform/razor-html-templates/index.md)
## [資料和雲端服務](data-cloud/index.md)
### [已連線的服務](data-cloud/connected-services.md)
### [Azure Active Directory](data-cloud/active-directory/index.md)
#### [快速入門](data-cloud/active-directory/get-started/index.md)
##### [步驟 1：註冊](data-cloud/active-directory/get-started/register.md)
##### [步驟 2：設定](data-cloud/active-directory/get-started/configure.md)
#### [存取圖形 API](data-cloud/active-directory/graph.md)
### [Azure 行動裝置應用程式](data-cloud/mobile-apps.md)
### [Web 服務簡介](data-cloud/web-services/index.md)
#### [逐步解說 - 使用 WCF](data-cloud/web-services/walkthrough-working-with-wcf.md)

## [部署與測試](deploy-test/index.md)
### [自訂連結器設定](deploy-test/linker.md)
### [效能](deploy-test/memory-perf-best-practices.md)
### [使用 NuGet 安裝 NUnit 2.6.4](deploy-test/installing-nunit-using-nuget.md)
### [多重處理序偵錯](deploy-test/multi-process-debugging.md)
## [Apple 平台](macios/index.md)
### [Apple 帳戶管理](macios/apple-account-management.md)
### [Unified API](macios/unified/index.md)
#### [概觀](macios/unified/overview.md)
#### [更新現有的應用程式](macios/unified/updating-apps.md)
#### [更新現有的 iOS 應用程式](macios/unified/updating-ios-apps.md)
#### [更新現有的 Mac 應用程式](macios/unified/updating-mac-apps.md)
#### [更新現有 Xamarin.Forms 應用程式](macios/unified/updating-xamarin-forms-apps.md)
#### [將繫結移轉至 Unified API](macios/unified/update-binding.md)
#### [將程式碼更新至 Unified API 的祕訣](macios/unified/updating-tips.md)
### [繫結 Objective-C](macios/binding/index.md)
#### [概觀](macios/binding/overview.md)
#### [繫結 Objective-C 程式庫](macios/binding/objective-c-libraries.md)
#### [繫結型別參考](macios/binding/binding-types-reference.md)
#### [Objective Sharpie](macios/binding/objective-sharpie/index.md)
##### [快速入門](macios/binding/objective-sharpie/get-started.md)
##### [工具與命令](macios/binding/objective-sharpie/tools.md)
##### [功能](macios/binding/objective-sharpie/platform/index.md)
###### [ApiDefinitions 與 StructsAndEnums](macios/binding/objective-sharpie/platform/apidefinitions-structsandenums.md)
###### [繫結原生架構](macios/binding/objective-sharpie/platform/native-frameworks.md)
###### [確認屬性](macios/binding/objective-sharpie/platform/verify.md)
##### [範例](macios/binding/objective-sharpie/examples/index.md)
###### [Xcode](macios/binding/objective-sharpie/examples/xcode.md)
###### [CocoaPod](macios/binding/objective-sharpie/examples/cocoapod.md)
###### [進階 (手動)](macios/binding/objective-sharpie/examples/advanced.md)
##### [發行記錄](macios/binding/objective-sharpie/releases.md)
#### [疑難排解](macios/binding/troubleshooting.md)
### [原生參考](macios/native-references.md)
### [原生型別](macios/nativetypes.md)
### [32 和 64 位元平台的考量](macios/32-and-64/index.md)
#### [將 Xamarin.Mac 通用應用程式更新為 64 位元](macios/32-and-64/mac-64-bit.md)
### [在跨平台應用程式中使用原生型別](macios/native-types-cross-platform.md)
### [HttpClient 堆疊和 SSL/TLS 實作選擇器](macios/http-stack.md)
## [內部項目](internals/index.md)
### [可用的組件](internals/available-assemblies.md)
## [疑難排解](troubleshooting/index.md)
### [常見問題集](troubleshooting/questions/index.md)
#### [是否可以搭配使用 Visual Studio 2017 候選版與 Xamarin？](troubleshooting/questions/visualstudio-2017-rc.md)
#### [如何檢視 PCL 中所支援的程式庫？](troubleshooting/questions/pcl-support-libraries.md)
#### [PCL 映射 API](troubleshooting/questions/pcl-reflection.md)
#### [PCL 案例研究：如何解決 Microsoft TPL Dataflow NuGet 套件有關 System.Diagnostics.Tracing 的問題？](troubleshooting/questions/pcl-case-study.md)
#### [如何更新 NuGet？](troubleshooting/questions/nuget-update.md)
#### [如何下載 NuGet 套件？](troubleshooting/questions/nuget-package-downgrade.md)
#### [更新 Nuget 套件之後的遺失套件錯誤](troubleshooting/questions/nuget-packages-missing.md)
#### [統一 Google Play 服務元件和 NuGet](troubleshooting/questions/gps-components-nuget.md)
#### [元件儲存在我電腦上的何處？](troubleshooting/questions/component-storage.md)
#### [可以在哪裡找到我的版本資訊和記錄檔？](troubleshooting/questions/version-logs.md)
#### [提出錯誤報告的時機與方式為何？](troubleshooting/questions/howto-file-bug.md)
#### [Xamarin 為什麼不支援 Jenkins？](troubleshooting/questions/xamarin-jenkins.md)
#### [偵錯工具需要哪些專案設定？](troubleshooting/questions/debugger-settings.md)
### [將元件參考更新為 NuGet](troubleshooting/component-nuget.md)
### [支援選項](troubleshooting/support-options.md)
### [舊版的授權問題](troubleshooting/legacy-licenses/index.md)
#### [Xamarin 平台團隊管理](troubleshooting/legacy-licenses/team-management.md)
#### [如何變更我的帳戶電子郵件地址？](troubleshooting/legacy-licenses/change-email.md)
#### [如何手動重新同步處理 Xamarin 授權？](troubleshooting/legacy-licenses/resync-licenses.md)
#### [某些特定的授權錯誤](troubleshooting/legacy-licenses/licensing-errors.md)
#### [為什麼無法在 Visual Studio 或 Visual Studio for Mac 上登入 Xamarin？](troubleshooting/legacy-licenses/login-troubleshooting.md)
## [範例](samples/index.yml)
