...
此優化需要啟用連結器，而且只會套用至具有 `[BindingImpl (BindingImplOptions.Optimizable)]` 屬性的方法。

根據預設，它會針對發行組建啟用。

藉由將 `--optimize=[+|-]remove-uithread-checks` 傳遞至 mtouch/mmp，可以覆寫預設行為。

[1]: https://docs.microsoft.com/dotnet/api/UIKit.UIApplication.EnsureUIThread

## <a name="inline-intptrsize"></a>內嵌 IntPtr. 大小

根據目標平臺，內嵌 `IntPtr.Size` 的常數值。
...
藉由將 `--optimize=[+|-]inline-intptr-size` 傳遞至 mtouch/mmp，可以覆寫預設行為。

## <a name="inline-nsobjectisdirectbinding"></a>內嵌 NSObject。 IsDirectBinding

`NSObject.IsDirectBinding` 是實例屬性，可判斷特定實例是否為包裝函式類型（包裝函式類型是對應至原生類型的 managed 類型）; 例如，managed `UIKit.UIView` 類型會對應至原生 `UIView` 類型，相反的是使用者類型，在此案例中 `class MyUIView : UIKit.UIView` 會是使用者類型）。

呼叫目標-C 時，必須知道 `IsDirectBinding` 的值，因為值會決定要使用哪一個版本的 `objc_msgSend`。
...
我們可以判斷 `UIView.SomeProperty` `IsDirectBinding` 的值不是常數，而且無法內嵌：
...
不過，您可以查看應用程式中的所有類型，並判斷沒有繼承自 `NSUrl`的類型，因此可安全地將 `IsDirectBinding` 值內嵌至常數 `true`：
...
特別是，這項優化會變更下列類型的程式碼（這是 `NSUrl.AbsoluteUrl`的系結程式碼）：
...
藉由將 `--optimize=[+|-]inline-isdirectbinding` 傳遞至 mtouch/mmp，可以覆寫預設行為。
...
將 `--optimize=[+|-]inline-runtime-arch` 傳遞至 mtouch，即可覆寫預設行為。
...
並判斷運算式 `8 == 8` 是永遠為 true，並將其減少為：

```csharp
Console.WriteLine ("Doing this");
```

這是與內嵌優化搭配使用時的強大優化，因為它可以轉換下列類型的程式碼（這是 `NFCIso15693ReadMultipleBlocksConfiguration.Range`的系結程式碼）：
...
至此（為64位裝置建立時，以及同時能夠確保應用程式中不會有 `NFCIso15693ReadMultipleBlocksConfiguration` 子類別）：
...
藉由將 `--optimize=[+|-]dead-code-elimination` 傳遞至 mtouch/mmp，可以覆寫預設行為。

## <a name="optimize-calls-to-blockliteralsetupblock"></a>優化對 BlockLiteral 的呼叫。 SetupBlock

在建立 managed 委派的目標 C 區塊時，您必須知道此 Xamarin iOS/Mac 執行時間的區塊簽章。 這可能是相當耗費資源的作業。 此優化會在組建階段計算區塊簽章，並修改 IL 以呼叫接受簽章做為引數的 `SetupBlock` 方法。 這麼做可避免在執行時間計算簽章的需求。
...
藉由將 `--optimize=[+|-]blockliteral-setupblock` 傳遞至 mtouch/mmp，可以覆寫預設行為。
...
藉由將 `--optimize=-register-protocols` 傳遞至 mtouch/mmp，可以覆寫預設行為。
...
藉由將 `--optimize=[+|-]remove-dynamic-registrar` 傳遞至 mtouch/mmp，可以覆寫預設行為。
...
內嵌在組建時間決定的 `Runtime.DynamicRegistrationSupported` 值。

如果已移除動態註冊機構（請參閱[移除動態註冊機構](#remove-the-dynamic-registrar)優化），這是常數 `false` 值，否則它是常數 `true` 值。
...
藉由將 `--optimize=[+|-]inline-dynamic-registration-supported` 傳遞至 mtouch/mmp，可以覆寫預設行為。
...
系結產生器所產生的系結程式碼將包含一個 `[BlockProxy]` 屬性，它會使用可執行此動作的 `Create` 方法來指定型別。
...
當目標-C 呼叫 `[ObjCBlockTester callClassCallback]`時，就會在參數上查看 `[BlockProxy (typeof (Trampolines.NIDActionArity1V0))]` 屬性。 然後，它會在該型別上查詢 `Create` 方法，並呼叫該方法來建立委派。

這種優化會在組建階段尋找 `Create` 方法，而靜態註冊機構會產生程式碼，以使用元資料標記在執行時間查閱方法，而不是使用屬性和反映（這會更快，而且也允許連結器移除對應的執行時間程式碼，讓應用程式變得更小。
...
藉由將 `--optimize=[+|-]static-delegate-to-block-lookup` 傳遞至 mtouch/mmp，可以覆寫預設行為。
