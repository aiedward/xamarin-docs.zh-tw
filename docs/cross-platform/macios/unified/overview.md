...
- **Classic API：** 限制為32位（僅限），並在 `monotouch.dll` 和元件中公開 `XamMac.dll` 。
- **Unified API：** 使用和元件中提供的單一 API，同時支援32和64位開發 `Xamarin.iOS.dll` `Xamarin.Mac.dll` 。
...
在**傳統**中，已有數個舊/已中斷/已淘汰的 api 無法用於**整合**api。 在 `CS0616` 啟動您的（手動或自動）升級之前，您可以更輕鬆地修正警告，因為您會有 `[Obsolete]` 屬性訊息（警告的一部分）引導您到正確的 API。
...
為了符合此行為，我們的 Unified API 上，我們會將先前的使用 `int` （在 .net 中定義為 always as `System.Int32` ）取代為新的資料類型： `System.nint` 。  您可以將 "n" 視為意義「原生」，因此是平臺的原生整數類型。

我們正引進 `nint` ，並在必要時，也 `nuint` `nfloat` 提供以其為基礎的資料類型。
...
因為 c # 索引子預期的類型為，所以您必須明確地將 `int` `nint` 值轉換為， `int` 才能存取集合或陣列中的元素。 例如：
...
這是預期的行為，因為從轉換 `int` 成 `nint` 會在64位上遺失，因此不會執行隱含轉換。

### <a name="converting-datetime-to-nsdate"></a>將 DateTime 轉換成 NSDate

使用整合 Api 時， `DateTime` `NSDate` 不會再執行對值的隱含轉換。 這些值必須明確轉換成另一種類型。 您可以使用下列擴充方法來自動化此程式：
...
在 Xamarin 中，會 `[Obsolete]` 以兩種不同的方式使用屬性：
...
每個 `NSObject` 子類別都有一個可接受的函式 `IntPtr` 。 這就是我們可以從原生 ObjC 控制碼將新的受控實例具現化的方式。

在傳統中，這是一個函式 `public` 。 不過，在使用者程式碼中誤用這項功能很容易，例如，為單一 ObjC 實例建立數個受控實例，*或*建立不需要預期受控狀態的受控實例（適用于子類別）。

為了避免這類問題，這些函式 `IntPtr` 現在 `protected` 位於**整合**API 中，僅用於子類別化。 這可確保使用正確/安全的 API，從控制碼建立受控實例，亦即
...
請注意， `.ctor(NSObjectFlag)` 現在也是， `protected` 但這種情況很少用於子類別化以外。

<a name="NSAction"></a>

### <a name="nsaction-replaced-with-action"></a>NSAction 已取代為 Action

透過統一的 Api，已 `NSAction` 移除，以改用標準的 .net `Action` 。 這是很大的改進 `Action` ，因為是常見的 .net 類型，而 `NSAction` 專屬於 Xamarin. iOS。 它們會執行完全相同的動作，但它們是不同且不相容的類型，因此需要撰寫更多程式碼才能達到相同的結果。
...
先前這會是編譯器錯誤 `Action` ，因為無法指派給 `NSAction` ，但因為現在會 `UITapGestureRecognizer` 接受， `Action` 而不是 `NSAction` 它在統一 api 中是有效的。
...
### <a name="taskbool-replaced-with-taskbooleannserror"></a>工作 \<bool> 已取代為 task<Boolean，NSError>>

在**傳統**中，有一些非同步 api 會傳回 `Task<bool>` 。 不過，其中有些是簽章的一部分時要使用的 `NSError` ，也就是說， `bool` 已經是 `true` ，而您必須攔截例外狀況才能取得 `NSError` 。
...
在少數情況下，某些常數必須從變更 `string` 為 `NSString` ，例如`UITableViewCell`
...
一般來說，我們偏好使用 .NET `System.String` 型別。 不過，儘管 Apple 方針，某些原生 API 會比較常數指標（而不是字串本身），而且只有在將常數公開為時才能使用 `NSString` 。

 <a name="protocols"></a>

### <a name="objective-c-protocols"></a>目標-C 通訊協定

原始的 MonoTouch 並沒有完整的 ObjC 通訊協定支援，而且部分、非最佳的 API 已新增來支援最常見的案例。 這項限制已不再存在，但為了回溯相容性，會在和中保留數個 Api `monotouch.dll` `XamMac.dll` 。
...
我們的原始系結包含每種類型的 .ctor （NSCoder），即使它不支援此 `NSCoding` 通訊協定。  `Encode(NSCoder)`中出現單一方法 `NSObject` ，以將物件編碼。
但是，只有當實例符合 NSCoding 通訊協定時，這個方法才可行。

在 [Unified API 我們已修正此問題。  只有在類型符合時，新元件才會有 `.ctor(NSCoder)` `NSCoding` 。 此外，這類類型現在具有 `Encode(NSCoder)` 符合介面的方法 `INSCoding` 。
