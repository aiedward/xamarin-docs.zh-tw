---
title: 系結目標-C 程式庫
description: '本檔提供如何建立 c # 系結至目標 C 程式碼的高階總覽，並說明如何系結事件、方法、自訂控制項等等。'
ms.prod: xamarin
...
使用 Xamarin 或 Xamarin 時，您可能會遇到需要使用協力廠商目標-C 程式庫的情況。 在這些情況下，您可以使用 Xamarin 系結專案來建立原生目標 C 程式庫的 c # 系結。 專案會使用我們用來將 iOS 和 Mac Api 帶入 c # 的相同工具。
...
`ApiDefinition.cs`是您將定義 API 合約的位置，這是描述基礎目標-c API 如何投射至 c # 的檔案。 此檔案的語法和內容是本檔討論的主要主題，而其內容僅限於 c # 介面和 c # 委派宣告。 檔案 `StructsAndEnums.cs` 是您將在其中輸入介面和委派所需之任何定義的檔案。 這包括您的程式碼可能會使用的列舉值和結構。
...
若要系結您的程式庫，您通常會從 API 定義檔開始。 API 定義檔只是一種 c # 原始程式檔，其中包含的 c # 介面已標注一些可協助驅動系結的屬性。  此檔案會定義 c # 與目標-C 之間的合約。
...
上述範例會定義一個 `Cocos2D.Camera` 衍生自 `NSObject` 基底類型（此類型來自）的類別， `Foundation.NSObject` 並定義靜態屬性（ `ZEye` ）、兩個不接受引數的方法，以及接受三個引數的方法。
...
- 選擇性：可能會展開所產生之系結的額外來源，或提供更多 c # 易記 API （您新增至專案的任何 c # 檔案）。
- 您要系結的原生程式庫。
...
您需要的任何額外程式碼（如列舉或支援類別）都應該裝載在不同的檔案上，在上述範例中，"CameraMode" 是不存在於 CS 檔案且應裝載于個別檔案中的列舉值，例如 `StructsAndEnums.cs` ：
...
檔案 `APIDefinition.cs` 會與 `StructsAndEnum` 類別結合，並用於產生程式庫的核心系結。 您可以使用所產生的程式庫，但一般來說，您會想要調整產生的程式庫，以新增一些 c # 功能以供您的使用者使用。 其中一些範例包括實作為 `ToString()` 方法、提供 c # 索引子、在某些原生類型之間新增隱含的轉換，或提供某些方法的強型別版本。 這些改良功能會儲存在額外的 c # 檔案中。 只要將 c # 檔案加入至您的專案，這些檔案就會包含在此組建程式中。

這會顯示如何在您的檔案中執行程式碼 `Extra.cs` 。 請注意，您將使用部分類別，因為這些會增強從和核心系結的組合產生的部分類別 `ApiDefinition.cs` `StructsAndEnums.cs` ：
...
現在當您建立專案時，將會得到一個檔案 `MagicChords.dll` ，其中包含系結和原生程式庫。 您可以將此專案或產生的 DLL 散發給其他開發人員，以供自己使用。
...
您可以執行的最簡單系結是系結方法。 只要使用 c # 命名慣例在介面中宣告方法，並使用來裝飾方法[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)
屬性中。 [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)屬性會連結您的 c # 名稱與 Xamarin iOS 執行時間中的目標-C 名稱。 的參數。[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
...
上述範例會示範如何系結實例方法。 若要系結靜態方法，您必須使用 [`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute) 屬性，如下所示：
...
這是必要的，因為合約是介面的一部分，而介面沒有靜態與實例宣告的概念，因此必須再次使用屬性。 如果您想要隱藏系結中的特定方法，您可以使用屬性來裝飾方法 [`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute) 。
...
使用關鍵字匯出參考型別時， [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 您也可以指定配置的語義。 這是確保不會遺漏任何資料的必要動作。
...
屬性並直接對應至 c # 屬性。 如同方法，屬性可以使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)
和[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
特性.

當您在包含 btouch 的屬性上使用屬性時，會實際系結 [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 兩個方法： getter 和 setter。 您提供給匯出的名稱是**basename** ，而 setter 則是藉由在前面加上 "set" 這個字來計算，將**basename**的第一個字母轉換成大寫，然後讓選取器接受引數。 這表示在屬性上套用的會實際系結 `[Export ("label")]` "label" 和 "setLabel：" 目標 C 方法。
...
在上述的和系結中，會將 getter 和 setter 明確定義為 `name` `setName` 。

除了使用的靜態屬性支援以外 [`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute) ，您還可以用來裝飾執行緒靜態屬性 [`[IsThreadStatic]`](~/cross-platform/macios/binding/binding-types-reference.md#IsThreadStaticAttribute) ，例如：
...
就像方法允許某些參數加上旗標 [`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute) ，您可以套用[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)
...
目標-C 架構會使用某些類別不會與可變子類別變的用法。 例如 `NSString` ，是不可變的版本，而 `NSMutableString` 是允許變化的子類別。

在這些類別中，通常會看到不可變的基類包含具有 getter 的屬性，但沒有 setter。 和，讓可變動的版本引進 setter。 由於 c # 並不能這麼做，因此我們必須將此方法對應到可與 c # 搭配使用的用法。

這種對應至 c # 的方式是在基類上加入 getter 和 setter，但使用[`[NotImplemented]`](~/cross-platform/macios/binding/binding-types-reference.md#NotImplementedAttribute)
屬性中。
...
此 `btouch-native` 工具會在您的類別中自動產生 fours 的函式，針對指定的類別 `Foo` ，它會產生：
...
針對您所定義的函式，必須在介面定義內使用下列簽章來宣告這些函式：它們必須傳回 `IntPtr` 值，而且方法的名稱應該是「構造函式」。 例如，若要系結此函式 `initWithFrame:` ，這就是您要使用的：
...
因為這是定義上的選擇性方法，所以不 `UIAccelerometerDelegate` 需要執行其他動作。 但如果通訊協定上有必要的方法，您應該新增[`[Abstract]`](~/cross-platform/macios/binding/binding-types-reference.md#AbstractAttribute)
方法的屬性。 這會強制執行程式的使用者實際提供方法的主體。

一般來說，通訊協定是在回應訊息的類別中使用。 這通常是在目標-C 中完成，方法是指派給 "delegate" 屬性物件的實例，以回應通訊協定中的方法。

在 Xamarin 中的慣例是支援目標-C 鬆散結合的樣式，其中的任何實例 `NSObject` 都可以指派給委派，同時也會公開它的強型別版本。 基於這個理由，我們通常會提供強型別的 `Delegate` 屬性，以及鬆散型別的 `WeakDelegate` 。 我們通常會將鬆散類型的版本與系結 [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) ，而我們會使用 [`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute) 屬性來提供強型別版本。

這會顯示我們如何系結 `UIAccelerometer` 類別：
...
針對目標-C 中的每個通訊協定定義 `MyProtocol` ，現在有一個 `IMyProtocol` 介面會列出來自通訊協定的所有必要方法，以及提供所有選擇性方法的擴充類別。  上述的與 Xamarin Studio 編輯器中的新支援結合，可讓開發人員不需要使用先前抽象模型類別的個別子類別，就能執行通訊協定方法。

包含屬性的任何定義 [`[Protocol]`](~/cross-platform/macios/binding/binding-types-reference.md#ProtocolAttribute) 實際上都會產生三個支援的類別，以大幅改善您取用通訊協定的方式：
...
**類別執行**會提供完整的抽象類別，您可以覆寫個別的方法，並取得完整的型別安全。  但由於 c # 不支援多重繼承，因此在某些情況下，您可能需要有不同的基類，但您仍想要執行介面，也就是
...
在 [目標-C] 中，您可以使用新的方法來擴充類別，這與 c # 擴充方法的精神類似。 當其中一個方法存在時，您可以使用[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute) 
用來將方法標示為目標-C 訊息接收者的屬性。

例如，在 Xamarin 中，我們系結了在匯 `NSString` `UIKit` 入做為中的方法時所定義的擴充方法 `NSStringDrawingExtensions` ，如下所示：
...
若要系結欄位，請將屬性加入至您的介面定義檔，並使用屬性裝飾屬性 [`[Field]`](~/cross-platform/macios/binding/binding-types-reference.md#FieldAttribute) 。 這個屬性接受一個參數：要查閱之符號的 C 名稱。 例如：
...
如果您想要將不同的欄位包裝在不是衍生自的靜態類別中 `NSObject` ，您可以使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute_Class) 
類別上的屬性，如下所示：
...
上述會產生不是 `LonelyClass` 衍生自的 `NSObject` ，而且會包含與 `NSSomeEventNotification` 
 `NSString` 公開為的 `NSString` 系結。
...
如果您要以靜態方式連結，則不需要系結至程式庫，因此您必須使用 `__Internal` 名稱：
...
您可以直接將系結檔案新增至您的系結檔案 `enum` ，以便更輕鬆地在 API 定義中使用它們，而不需要使用不同的原始程式檔（必須在系結和最終專案中編譯）。
...
在上述範例中，您可以決定 `void Perform (NSString mode);` 使用屬性來裝飾 [`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute) 。 這會從您的系結取用者**隱藏**以常數為基礎的 API。

不過，這會限制類型的子類別化，因為更好 API 的替代方法會使用 [`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute) 屬性。 這些產生的方法不是，也就是您無法覆 `virtual` 寫它們-這可能是不錯的選擇。

替代方式是將原始以為 `NSString` 基礎的定義標示為 `[Protected]` 。 這會在必要時允許子類別化工作，而 wrap'ed 版本仍然可以使用並呼叫覆寫的方法。

### <a name="binding-nsvalue-nsnumber-and-nsstring-to-a-better-type"></a>將 `NSValue` 、 `NSNumber` 和系結至較 `NSString` 佳的類型

[`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)屬性允許系結 `NSNumber` ， `NSValue` 並將 `NSString` （列舉）轉換成更精確的 c # 類型。 屬性可以透過原生 API，用來建立更好、更精確的 .NET API。

您可以使用來裝飾方法（在傳回值上）、參數和屬性 [`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute) 。 唯一的限制是您的成員**不得**位於[`[Protocol]`](~/cross-platform/macios/binding/binding-types-reference.md#ProtocolAttribute) 
...
[`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)也支援和的陣列 `NSNumber` `NSValue` `NSString` （列舉）。
...
`CAScroll`是 `NSString` 支援的列舉，我們將會提取正確的 `NSString` 值，並處理類型轉換。
...
此屬性可用於不含任何承載之通知的引數，或者您可以指定 `System.Type` 參考 API 定義中另一個介面的，其名稱通常會以 "EventArgs" 結尾。 產生器會將介面轉換為子類別， `EventArgs` 且會包含此處列出的所有屬性。 [`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)屬性應該用於 EventArgs 類別中，以列出用來查閱目標-C 字典以提取值的索引鍵名稱。
...
上述將會產生 `MyScreenChangedEventArgs` 具有和屬性的類別， `ScreenX` 而 `ScreenY` 這些屬性會分別使用索引鍵 "ScreenXKey" 和 "ScreenYKey" 來提取[NSNotification](xref:Foundation.NSNotification.UserInfo)中的資料，並套用適當的轉換。 `[ProbePresence]`如果在中設定索引鍵，則會使用屬性來進行探查 `UserInfo` ，而不是嘗試將值解壓縮。 這適用于有索引鍵為值的情況（通常用於布林值）。
...
類別目錄是一個目標 C 機制，用來擴充類別中可用的方法和屬性集合。   實際上，它們是用來擴充基類的功能（例如 `NSObject` ），當特定架構連結在中時（例如） `UIKit` ，讓其方法可用，但只有在新架構連結時才會使用。   在某些其他情況下，它們是用來依功能來組織類別中的功能。   它們與 c # 擴充方法的精神很相似。這是類別在目標-C 中的樣子：
...
如果在程式庫上找到，則上述範例會使用方法來擴充的實例 `UIView` `makeBackgroundRed` 。
...
上述會建立 `MyUIViewExtension` 包含 `MakeBackgroundRed` 擴充方法的類別。  這表示您現在可以在任何子類別上呼叫 "MakeBackgroundRed" `UIView` ，提供您在目標上取得的相同功能。 在某些其他情況下，則不會使用類別來擴充系統類別，而是用來組織功能，純粹用於裝飾用途。  例如：
...
區塊是 Apple 引進的新結構，可將功能對等的 c # 匿名方法帶入目標-C。 例如， `NSSet` 類別現在會公開這個方法：

```csharp
- (void) enumerateObjectsUsingBlock:(void (^)(id obj, BOOL *stop) block
```

上述描述會宣告名 `enumerateObjectsUsingBlock:` 為的方法，其採用一個名為的引數 `block` 。 這個區塊類似于 c # 匿名方法，因為它支援捕獲目前的環境（"this" 指標、區域變數和參數的存取權）。 中的上述方法會叫 `NSSet` 用具有兩個參數 `NSObject` （ `id obj` 元件）和布林（）部分指標的區塊 `BOOL *stop` 。

若要使用 btouch 系結這類 API，您必須先將區塊類型簽章宣告為 c # 委派，然後從 API 進入點參考它，如下所示：
...
系結產生器可以將特定類別的方法變成非同步易懂的方法（傳回工作或工作 T 的方法 &lt; &gt; ）。

您可以使用[`[Async]`](~/cross-platform/macios/binding/binding-types-reference.md#AsyncAttribute) 
傳回 void 之方法的屬性，其最後一個引數是回呼。  當您將此套用至方法時，系結產生器會使用後置詞來產生該方法的版本 `Async` 。  如果回呼不接受任何參數，則傳回值會是 `Task` ，如果回呼接受參數，則結果會是 `Task<T>` 。  如果回呼接受多個參數，您應該設定 `ResultType` 或， `ResultTypeName` 以指定將保留所有屬性之產生類型的所需名稱。
...
在目標-C API 的許多位置中，參數會以 `NSDictionary` 具有特定索引鍵和值的弱型別 api 來傳遞，但這些是容易出錯的（您可以傳遞不正確索引鍵，而且不會收到警告; 您可以傳遞不正確值，而不會收到警告）和挫折，因為他們需要多次行程檔以查閱可能的索引鍵名稱和值

解決方案是提供強型別的版本，以提供 API 的強型別版本，而幕後則會對應各種基礎索引鍵和值。

因此，例如，如果目標-C API 接受，而且記錄為採用的索引鍵，而此 `NSDictionary` 金鑰的 `XyzVolumeKey` 磁片區 `NSNumber` 值是從0.0 到1.0，而是 `XyzCaptionKey` 採用字串，則您會想要讓使用者擁有類似如下的絕佳 API：
...
- 針對接受新的強型別版本的方法宣告多載 `NSDictionary` 。
...
屬性，以呈現新的 API 進入點，並使用我們的強型別類別來呈現 `XyzOptions` 。  包裝函式方法也允許傳遞 null。

現在，我們並未提到的一件事就是這些 `XyzOptionsKeys` 值來自何處。  您通常會將 API 表面呈現在靜態類別中的金鑰分組 `XyzOptionsKeys` ，如下所示：
...
若要建立強型別字典，請在您的 API 中引進介面，並使用[StrongDictionary](~/cross-platform/macios/binding/binding-types-reference.md#StrongDictionary)屬性裝飾。  這會告訴產生器，它應該建立一個類別，其名稱與將衍生自的介面相同 `DictionaryContainer` ，而且將為其提供強型別存取子。
...
如果您需要在成員中參考 `XyzOption` 不同的欄位（不是具有尾碼的屬性名稱 `Key` ），您可以使用來裝飾屬性[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
具有您想要使用之名稱的屬性。
...
Xamarin iOS 執行時間會自動將 c # 陣列轉換成 `NSArrays` ，並執行轉換，因此例如，會傳回之的虛構目標 C 方法 `NSArray` `UIViews` ：
...
其概念是使用強型別的 c # 陣列，因為這可讓 IDE 以實際的類型提供適當的程式碼完成，而不會強制使用者猜測，或查閱檔以找出陣列中所包含物件的實際類型。
...
選取器會顯示在目標-C API 上做為特殊類型 `SEL` 。 系結選取器時，您會將型別對應至 `ObjCRuntime.Selector` 。  通常會在具有物件、目標物件和要在目標物件中叫用之選取器的 API 中公開選取器。 提供這兩個基本上會對應至 c # 委派：一個專案會封裝要叫用的方法，以及要在其中叫用方法的物件。
...
若要將系結更好到 c # 開發人員，您通常會提供接受參數的方法 `NSAction` ，讓您可以使用 c # 委派和 lambda，而不是 `Target+Selector` 。 若要這麼做，您通常會將 `SetTarget` 方法標記為[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
...
當您要系結採用的方法時， `NSString` 可以在傳回型別和參數上，以 c # 字串型別來取代它。

當您想要直接使用時，唯一的情況 `NSString` 就是使用字串做為標記。 如需有關字串和的詳細資訊 `NSString` ，請參閱[NSString 上的 API 設計](~/ios/internals/api-design/nsstring.md)檔。

在某些罕見的情況下，API 可能會公開類似 C 的字串（ `char *` ），而不是目標-c 字串（ `NSString *` ）。 在這些情況下，您可以使用來標注參數[`[PlainString]`](~/cross-platform/macios/binding/binding-types-reference.md#plainstring)
屬性中。
...
這種模型，雖然 Xamarin 完全支援並呈現，但有時可能會很麻煩。 在這些情況下，Xamarin 會公開 c # 事件模式以及可用於類別的方法回呼系統。 這可讓像這樣的程式碼執行：
...
系結產生器能夠減少將目標 C 模式對應到 c # 模式所需的輸入量。

從 Xamarin. iOS 1.4 開始，您也可以指示產生器針對特定的目標 C 委派產生系結，並將委派公開為主機類型上的 c # 事件和屬性。
...
- 在您的主機類別中，將新增至您的[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute)  
   宣告作為其委派的類型，以及您公開的 c # 名稱。 在上述範例中，它們 `typeof (MyClassDelegate)` 分別是和 `WeakDelegate` 。
...
`EventArgs`是用來指定 `EventArgs` 要產生之類別的名稱。 您應該使用每個簽章一個（在此範例中， `EventArgs` 將包含 `With` nint 類型的屬性）。
...
回呼就像事件叫用一樣，其差異在於，它不會有多個潛在的訂閱者（例如，多個方法可以攔截 `Clicked` 事件或 `DownloadFinished` 事件）回呼只能有一個訂閱者。

此程式完全相同，唯一的差異在於，它 `EventArgs` 實際上是用來為產生的 c # 委派名稱命名，而不是公開將產生的類別名稱。
...
您必須通知 Xamarin，您可以藉由變更組建設定來叫 `mtouch` 用命令，並使用一些額外的組建引數來指定如何使用 "-gcc_flags" 選項來連結新的程式庫，後面加上加上引號的字串，其中包含程式所需的所有額外程式庫，如下所示：
...
或者，您可以利用元件層級 [`[LinkWithAttribute]`](~/cross-platform/macios/binding/binding-types-reference.md#LinkWithAttribute) ，您可以將它內嵌在您的合約檔案中（例如 `AssemblyInfo.cs` ）。
當您使用時，您必須在進行系結 [`[LinkWithAttribute]`](~/cross-platform/macios/binding/binding-types-reference.md#LinkWithAttribute) 時使用原生程式庫，因為這會將原生程式庫內嵌至您的應用程式。 例如：
...
您可能想知道，為什麼需要 `-force_load` 命令，而原因是-ObjC 旗標雖然它會在中編譯器代碼，但它不會保留在執行時間針對 Xamarin 所需的中繼資料，以支援分類（連結器/編譯器停止程式碼排除）。
...
若要利用此功能，您的類別應該在宣告中設定 Events 屬性 [`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute) ，同時將 `KeepUntilRef` 變數設為在物件完成其工作時所叫用的方法名稱，如下所示：
...
從 Xamarin. iOS 3.2 版開始，我們支援從已使用屬性標示的通訊協定繼承 [`[Model]`](~/cross-platform/macios/binding/binding-types-reference.md#ModelAttribute) 。 這在某些 API 模式中很有用，例如，在中， `MapKit` `MKOverlay` 通訊協定會繼承自 `MKAnnotation` 通訊協定，而則是由繼承自的許多類別所採用 `NSObject` 。

在過去，我們需要將通訊協定複製到每個執行，但在這些情況下，我們可以讓 `MKShape` 類別繼承自 `MKOverlay` 通訊協定，而且它會自動產生所有必要的方法。
