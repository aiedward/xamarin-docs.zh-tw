...
`ApiDefinition.cs` 是您將定義 API 合約的位置，這是描述基礎目標-C API 如何投射到C#的檔案。 此檔案的語法和內容是本檔討論的主要主題，而其內容僅限於C#介面和C#委派宣告。 `StructsAndEnums.cs` 檔案是您將在其中輸入介面和委派所需之任何定義的檔案。 這包括您的程式碼可能會使用的列舉值和結構。
...
上述範例會定義衍生自 `NSObject` 基底類型（此類型來自 `Foundation.NSObject`）的 `Cocos2D.Camera` 類別，並定義靜態屬性（`ZEye`）、兩個不採用引數的方法，以及接受三個引數的方法。
...
- API 定義檔（在範本中`ApiDefinition.cs`）。
- 選擇性： API 定義檔（範本中`StructsAndEnums.cs`）所需的任何列舉、類型和結構。
...
`APIDefinition.cs` 檔案會與 `StructsAndEnum` 類別結合，並用於產生程式庫的核心系結。 您可以使用所產生的程式庫，但一般來說，您會想要調整產生的媒體櫃，為您C#的使用者權益新增一些功能。 其中一些範例包括實作為 `ToString()` 方法、 C#提供索引子、在某些原生類型之間新增隱含的轉換，或提供某些方法的強型別版本。 這些改良功能會儲存在C#額外的檔案中。 只要將檔案C#新增至您的專案，這些檔案就會包含在此組建進程中。

這會顯示如何在您的 `Extra.cs` 檔案中執行程式碼。 請注意，您將使用部分類別，因為這些會增強從 `ApiDefinition.cs` 和 `StructsAndEnums.cs` 核心系結的組合產生的部分類別：
...
`libMagicChord.linkwith.cs` 檔案的內容包含如何使用此程式庫的相關資訊，並指示您的 IDE 將此二進位檔封裝到產生的 DLL 檔案中：
...
如何使用[`[LinkWith]`](~/cross-platform/macios/binding/binding-types-reference.md#LinkWithAttribute)的完整詳細資料 
屬性記載于系結[類型參考指南](~/cross-platform/macios/binding/binding-types-reference.md)中。

現在當您建立專案時，將會得到一個同時包含系結和原生程式庫的 `MagicChords.dll` 檔案。 您可以將此專案或產生的 DLL 散發給其他開發人員，以供自己使用。
...
`btouch-native` 命令會將參考參數的檢查引入不是 null。 如果您想要針對特定參數允許 null 值，請使用[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)
參數上的屬性，如下所示：
...
當您使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)關鍵字匯出參考型別時，您也可以指定配置的語義。 這是確保不會遺漏任何資料的必要動作。

<a name="Binding_Properties" />

### <a name="binding-properties"></a>系結屬性

如同方法，會使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)來系結目標-C 屬性。
屬性，並直接對應C#至屬性。 如同方法，屬性可以使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)裝飾
和[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)
特性.

當您在 btouch 的屬性上使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)屬性時，會實際系結兩個方法： getter 和 setter。 您提供給匯出的名稱是**basename** ，而 setter 則是藉由在前面加上 "set" 這個字來計算，將**basename**的第一個字母轉換成大寫，然後讓選取器接受引數。 這表示在屬性上套用的 `[Export ("label")]` 實際上會系結 "label" 和 "setLabel：" 目標 C 方法。

有時，目標-C 屬性不會遵循上述的模式，而是手動覆寫名稱。 在這些情況下，您可以使用[`[Bind]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAttribute)來控制產生系結的方式。 
...
在這裡，getter 和 setter 會明確定義為 `name` 和 `setName` 系結中。

除了支援使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)的靜態屬性之外，您還可以使用[`[IsThreadStatic]`](~/cross-platform/macios/binding/binding-types-reference.md#IsThreadStaticAttribute)裝飾執行緒靜態屬性，例如：
...
就像方法允許某些參數使用[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)來標示，您可以套用[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)
...
您也可以直接在 setter 上指定[`[NullAllowed]`](~/cross-platform/macios/binding/binding-types-reference.md#NullAllowedAttribute)參數：
...
1. 系結**屬性必須為靜態**-定義屬性的系結時，必須使用[`[Static]`](~/cross-platform/macios/binding/binding-types-reference.md#StaticAttribute)屬性。
...
`btouch-native` 工具會在您的類別中自動產生 fours 的函式，針對指定的類別 `Foo`，它會產生：

- `Foo ()`：預設的函式（對應至目標-C 的 "init" 函數）
- `Foo (NSCoder)`：在還原序列化筆尖檔案期間使用的函式（對應至目標-C 的 "initWithCoder：" 程式化）。
- `Foo (IntPtr handle)`：以控制碼為基礎之建立的函式，當執行時間需要從未受管理的物件公開 managed 物件時，會叫用此方法。
- `Foo (NSEmptyFlag)`：衍生類別會使用這個來避免雙重初始化。

針對您所定義的函式，必須在介面定義內使用下列簽章來宣告這些函式：它們必須傳回 `IntPtr` 值，而且方法的名稱應該是「函式」。 例如，若要系結 `initWithFrame:` 的函式，這就是您要使用的：
...
由於這是的 `UIAccelerometerDelegate` 定義上的選擇性方法，因此不需要執行任何其他動作。 但是，如果通訊協定上有必要的方法，您應該加入[`[Abstract]`](~/cross-platform/macios/binding/binding-types-reference.md#AbstractAttribute)
方法的屬性。 這會強制執行程式的使用者實際提供方法的主體。

一般來說，通訊協定是在回應訊息的類別中使用。 這通常是在目標-C 中完成，方法是指派給 "delegate" 屬性物件的實例，以回應通訊協定中的方法。

在 Xamarin 中的慣例是支援目標-C 鬆散結合的樣式，其中 `NSObject` 的任何實例都可以指派給委派，同時也會公開它的強型別版本。 基於這個理由，我們通常會提供強型別的 `Delegate` 屬性以及鬆散型別的 `WeakDelegate`。 我們通常會將鬆散類型的版本與[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)系結，而我們會使用[`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute)屬性來提供強型別版本。
...
針對目標-C 中的每個通訊協定定義 `MyProtocol`，現在會有一個 `IMyProtocol` 介面，其中會列出來自通訊協定的所有必要方法，以及提供所有選擇性方法的延伸模組類別。  上述的與 Xamarin Studio 編輯器中的新支援結合，可讓開發人員不需要使用先前抽象模型類別的個別子類別，就能執行通訊協定方法。

包含[`[Protocol]`](~/cross-platform/macios/binding/binding-types-reference.md#ProtocolAttribute)屬性的任何定義，實際上都會產生三個支援的類別，可大幅改善您取用通訊協定的方式：
...
請注意，介面只會列出必要的方法，並會公開選擇性的方法。  這表示採用通訊協定的類別會針對所需的方法取得完整的類型檢查，但必須使用弱型別（手動使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute) 
選擇性通訊協定方法的屬性和相符的簽章）。
...
例如，在 Xamarin 中，我們會在 `UIKit` 匯入為 `NSStringDrawingExtensions`中的方法時，系結 `NSString` 上定義的擴充方法，如下所示：
...
上述會產生不是衍生自 `NSObject` 的 `LonelyClass`，而且將包含 `NSSomeEventNotification`
`NSString` 公開為 `NSString`的系結。
...
- 32位整數（`System.Int32`）
- 64位整數（`System.Int64`）
- 32位浮點數（`System.Single`）
- 64位浮點數（`System.Double`）
...
如果您要以靜態方式連結，則不會有要系結的程式庫，因此您必須使用 `__Internal` 名稱：
...
您可以直接在您的系結檔案中新增 `enum`，讓它們在 API 定義中更容易使用，而不需要使用不同的原始程式檔（必須在系結和最終專案中編譯）。
...
在上述範例中，您可以決定使用[`[Internal]`](~/cross-platform/macios/binding/binding-types-reference.md#InternalAttribute)屬性來裝飾 `void Perform (NSString mode);`。 這會從您的系結取用者**隱藏**以常數為基礎的 API。

不過，這會限制類型的子類別化，因為更好 API 的替代方法會使用[`[Wrap]`](~/cross-platform/macios/binding/binding-types-reference.md#WrapAttribute)的屬性。 這些產生的方法不 `virtual`，也就是說，您無法覆寫它們，這可能是個不錯的選擇。

另一個替代方式是將原始的以 `NSString`為基礎的定義標示為 `[Protected]`。 這會在必要時允許子類別化工作，而 wrap'ed 版本仍然可以使用並呼叫覆寫的方法。

### <a name="binding-nsvalue-nsnumber-and-nsstring-to-a-better-type"></a>將 `NSValue`、`NSNumber`和 `NSString` 系結至較佳的類型

[`[BindAs]`](~/cross-platform/macios/binding/binding-types-reference.md#BindAsAttribute)屬性允許將 `NSNumber`、`NSValue` 和 `NSString`（列舉）系結至更C#精確的類型。 屬性可以透過原生 API，用來建立更好、更精確的 .NET API。
...
屬性的屬性（attribute）也已標記[`[Field]`](~/cross-platform/macios/binding/binding-types-reference.md#FieldAttribute)
屬性（可以是公用或私用）。
...
上述程式會產生 `MyScreenChangedEventArgs` 類別，其中包含 `ScreenX` 和 `ScreenY` 屬性，會分別使用索引鍵 "ScreenXKey" 和 "ScreenYKey" 來提取[NSNotification](xref:Foundation.NSNotification.UserInfo)中的資料，並套用適當的轉換。 如果在 `UserInfo`中設定索引鍵，則會使用 `[ProbePresence]` 屬性來進行探查，而不是嘗試將值解壓縮。 這適用于有索引鍵為值的情況（通常用於布林值）。
...
類別目錄是一個目標 C 機制，用來擴充類別中可用的方法和屬性集合。   在實務上，當特定架構連結到（例如 `UIKit`）時，它們會用來擴充基類的功能（例如 `NSObject`），使其方法可供使用，但只有在新架構連結時才會使用。   在某些其他情況下，它們是用來依功能來組織類別中的功能。   它們與C#擴充方法的精神很類似。這是類別在目標-C 中的樣子：
...
如果在程式庫上找到，則上述範例會使用 `makeBackgroundRed`方法來擴充 `UIView` 的實例。

若要系結，您可以在介面定義上使用[`[Category]`](~/cross-platform/macios/binding/binding-types-reference.md#CategoryAttribute)屬性。  使用[`[Category]`](~/cross-platform/macios/binding/binding-types-reference.md#CategoryAttribute)
屬性，這是[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute)的意義 
...
區塊是 Apple 引進的新結構，可將功能相當的C#匿名方法帶入目標-C。 例如，`NSSet` 類別現在會公開此方法：

```csharp
- (void) enumerateObjectsUsingBlock:(void (^)(id obj, BOOL *stop) block
```

上述描述會宣告名為 `enumerateObjectsUsingBlock:` 的方法，其採用一個名為 `block`的引數。 這個區塊類似于C#匿名方法，因為它支援捕獲目前的環境（"this" 指標、區域變數和參數的存取權）。 `NSSet` 中的上述方法會叫用具有兩個參數 `NSObject` （`id obj` 部分）的區塊，以及布林值（`BOOL *stop`）部分的指標。
...
系結產生器可以將特定類別的方法變成非同步易懂的方法（傳回工作或工作的方法&lt;T&gt;）。

您可以使用[`[Async]`](~/cross-platform/macios/binding/binding-types-reference.md#AsyncAttribute) 
傳回 void 之方法的屬性，其最後一個引數是回呼。  當您將此套用至方法時，系結產生器將會產生該方法的版本，並 `Async`尾碼。  如果回呼不接受任何參數，則傳回值會是 `Task`，如果回呼接受參數，則結果會是 `Task<T>`。  如果回呼接受多個參數，您應該設定 `ResultType` 或 `ResultTypeName`，以指定將保留所有屬性之產生類型的所需名稱。
...
`Volume` 屬性定義為可為 null 的 float，因為目標-C 中的慣例不需要這些字典具有值，因此在某些情況下可能不會設定此值。

若要這樣做，您需要執行幾項工作：

- 建立強型別類別，子類別會[DictionaryContainer](xref:Foundation.DictionaryContainer)並提供每個屬性的各種 getter 和 setter。
- 宣告方法的多載，讓 `NSDictionary` 取得新的強型別版本。
...
我們沒有提到的一件事，就是 `XyzOptionsKeys` 值來自何處。  您通常會將 API 表面呈現在靜態類別中的索引鍵（例如 `XyzOptionsKeys`）分組，如下所示：
...
若要建立強型別字典，請在您的 API 中引進介面，並使用[StrongDictionary](~/cross-platform/macios/binding/binding-types-reference.md#StrongDictionary)屬性裝飾。  這會告訴產生器應該建立一個類別，其名稱與將衍生自 `DictionaryContainer` 的介面相同，而且將為其提供強型別的存取子。

[`[StrongDictionary]`](~/cross-platform/macios/binding/binding-types-reference.md#StrongDictionary)屬性接受一個參數，也就是包含字典索引鍵的靜態類別名稱。  然後，介面的每個屬性都會變成強型別存取子。  根據預設，此程式碼會在靜態類別中使用具有後置詞 "Key" 的屬性名稱，以建立存取子。
...
如果您需要在 `XyzOption` 成員中參考不同的欄位（不是具有尾碼 `Key`的屬性名稱），您可以使用[`[Export]`](~/cross-platform/macios/binding/binding-types-reference.md#ExportAttribute)來裝飾屬性。 
具有您想要使用之名稱的屬性。
...
|`NSString` （系結[NSString 的詳細資訊](~/ios/internals/api-design/nsstring.md)）|`string`|
...
選取器會顯示在目標-C API 上做為特殊類型 `SEL`。 系結選取器時，您會將此類型對應至 `ObjCRuntime.Selector`。  通常會在具有物件、目標物件和要在目標物件中叫用之選取器的 API 中公開選取器。 提供這兩個基本上會對應至C#委派：一個專案會封裝要叫用的方法，以及要在其中叫用方法的物件。
...
當您要系結接受 `NSString`的方法時，可以在傳回型別和C#參數上，將其取代為字串型別。
...
第一個範例會顯示用來傳回錯誤碼的一般目標-C 程式碼、已傳遞 `NSError` 指標的指標，並在傳回時設定值。   第二個方法會顯示目標 C 方法如何採用物件並修改其內容。   這會是以傳址方式傳遞，而不是單純的輸出值。
...
此套裝程式含兩個類別，也就是目前發出事件的主機類別，並將其傳送至 `Delegate` 或 `WeakDelegate` 以及實際的委派類別。
...
- 在您的主機類別中，將新增至您的[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute)  
   宣告作為其委派的類型，以及您所公開C#的名稱。 在上述範例中，它們分別是 `typeof (MyClassDelegate)` 和 `WeakDelegate`。
...
`EventArgs` 可用來指定要產生之 `EventArgs` 類別的名稱。 您應該使用每個簽章一個（在此範例中，`EventArgs` 將包含 nint 類型的 `With` 屬性）。
...
回呼就像事件叫用一樣，其差異在於，它不會有多個潛在的訂閱者（例如，多個方法可以連結到 `Clicked` 事件或 `DownloadFinished` 事件）回呼只能有一個訂閱者。

此程式完全相同，唯一的差異在於，而不是公開將產生之 `EventArgs` 類別的名稱，而是使用 EventArgs 來命名產生C#的委派名稱。
...
[`[DefaultValue]`](~/cross-platform/macios/binding/binding-types-reference.md#DefaultValueAttribute)會將傳回值硬式編碼，同時[`[DefaultValueFromArgument]`](~/cross-platform/macios/binding/binding-types-reference.md#DefaultValueFromArgumentAttribute)
用來指定將傳回的輸入引數。
...
例如，如果您有一個顯示訊息的類別，然後產生 `Done` 事件，則處理這種情況的傳統方式會是：
...
若要利用此功能，您的類別應該在[`[BaseType]`](~/cross-platform/macios/binding/binding-types-reference.md#BaseTypeAttribute)宣告中設定 Events 屬性，同時將 `KeepUntilRef` 變數設為在物件完成其工作時所叫用之方法的名稱，如下所示：
...
從 iOS 3.2 版開始，我們支援從已使用[`[Model]`](~/cross-platform/macios/binding/binding-types-reference.md#ModelAttribute)屬性標記的通訊協定繼承。 這在某些 API 模式中很有用，例如在 `MapKit` 中，其中 `MKOverlay` 通訊協定會繼承自 `MKAnnotation` 通訊協定，並採用繼承自 `NSObject`的一些類別。

在過去，我們需要將通訊協定複製到每個執行，但在這些情況下，我們現在可以讓 `MKShape` 類別繼承自 `MKOverlay` 通訊協定，而且它會自動產生所有必要的方法。
