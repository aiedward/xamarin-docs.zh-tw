...
#### <a name="a-idwindows-logs-xamarin-vs-namewindows-logs-xamarin-vs--visual-studio-tools-for-xamarin"></a>適用于 Xamarin 的 <a id="windows-logs-xamarin-vs" name="windows-logs-xamarin-vs" /> Visual Studio Tools
...
這些是 `XamarinInstaller.exe` 安裝程式的記錄檔。
...
`~/Library/Logs/VisualStudio/7.0` （此數目可能會根據您使用的版本而變更）
...
`~/Library/Logs/XamarinStudio-6.0` （此數目可能會根據您使用的版本而變更）
...
這些是 `XamarinInstaller.dmg` 安裝程式的記錄檔。
...
2. 針對 iOS 應用程式，也請在 [專案屬性] 下新增 `-v -v -v -v`，以啟用**詳細資訊 mtouch 輸出** **> IOS 組建 > 一般（索引標籤） > 其他選項 > 其他 mtouch 引數**。

3. 清除並重建專案。

4. 將 IDE 的組建輸出複製並貼到文字檔中。
     - Visual Studio （Windows）： **View > 輸出 > 顯示來自： Build 的輸出**
     - Visual Studio for Mac： **View > pad > 錯誤 > 組建輸出（tab）**

## <a name="a-iddebug-logs-for-xamarin-apps-namedebug-logs-for-xamarin-apps-debug-logs-for-xamarinandroid-and-xamarinios-apps"></a><a id="debug-logs-for-xamarin-apps" name="debug-logs-for-xamarin-apps" />Xamarin 和 Xamarin iOS 應用程式的 Debug 記錄檔
...
### <a name="a-idios-simulator-logs-nameios-simulator-logs-ios-simulator-logs-on-mac"></a><a id="ios-simulator-logs" name="ios-simulator-logs" />iOS 模擬器記錄（在 Mac 上）

- 若要存取系統記錄檔，請選取 [Debug] > 開啟 iOS 模擬器應用程式中的 [**系統記錄檔 ...** ]。

- 若要從模擬器查看損毀報告，請開啟 [主控台]，然後流覽至 [`~/Library/Logs > DiagnosticReports`]。
