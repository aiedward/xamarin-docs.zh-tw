...
在此程式碼範例中，當進行驗證的憑證不是 `localhost` 時，就會傳回伺服器憑證驗證結果。 對於此憑證來說，系統會忽略驗證結果並傳回 `true`，指示憑證是否有效。 產生`HttpClientHandler`的物件應該當做引數傳遞給 debug build `HttpClient`的函式：
