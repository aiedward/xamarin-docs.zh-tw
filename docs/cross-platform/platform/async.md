...
## <a name="using-async-amp-await"></a>使用 async &amp; await
 ...
 `async` 和 `await` 是與C#工作平行程式庫搭配運作的新語言功能，可讓您輕鬆地撰寫執行緒程式碼來執行長時間執行的工作，而不會封鎖應用程式的主要執行緒。

## <a name="async"></a>async

### <a name="declaration"></a>宣告

`async` 關鍵字會放在方法宣告（或 lambda 或匿名方法）中，以表示它包含可以非同步執行的程式碼，即。不封鎖呼叫端的執行緒。

標記為 `async` 的方法應該包含至少一個 await 運算式或語句。 如果方法中沒有任何 `await` 語句，則會同步執行（與沒有 `async` 修飾詞相同）。 這也會導致編譯器警告（但不是錯誤）。

### <a name="return-types"></a>傳回型別

非同步方法應該會傳回 `Task`、`Task<TResult>` 或 `void`。

如果方法不會傳回任何其他值，請指定 `Task` 傳回型別。

如果方法需要傳回值，請指定 `Task<TResult>`，其中 `TResult` 是所傳回的類型（例如，`int`）。

`void` 傳回型別主要用於需要它的事件處理常式。 呼叫傳回 void 之非同步方法的程式碼無法在結果上 `await`。
...
在非同步方法內發生的例外狀況會儲存在工作中，並在工作 `await`ed 時擲回。 這些例外狀況可以在 try-catch 區塊內攔截和處理。
...
- 呼叫 `CancellationTokenSource.Cancel` 方法，即可要求取消。
...
下載[範例 Xamarin 解決方案](https://docs.microsoft.com/samples/xamarin/mobile-samples/asyncawait/)（適用于 IOS 和 Android），以查看行動裝置應用程式中 `async` 和 `await` 的實用範例。 本章節將更詳細地討論範例程式碼。

### <a name="writing-an-async-method"></a>撰寫非同步方法

下列方法示範如何使用 `await`ed 工作來撰寫 `async` 方法的程式碼：
...
- 方法宣告包含 `async` 關鍵字。
- 傳回型別為 `Task<int>`，因此呼叫程式碼可以存取在此方法中計算的 `int` 值。
- Return 語句是 `return exampleInt;`，這是一個整數物件，這是方法傳回的事實 `Task<int>` 是語言改善的一部分。
...
- 匿名委派具有 async 關鍵字前置詞。
- 非同步方法 DownloadHomepage 會傳回儲存在 sizeTask 變數中\<int > 的工作。
...
在 iOS 範例應用程式中，範例的撰寫方式稍有不同，以示範替代方法。 這個範例不會使用匿名委派，而是宣告指派為一般事件處理常式的 `async` 事件處理常式：
...
- `DownloadHomepage` 方法上的 `await` 關鍵字會直接指派給變數（`intResult`），與先前的範例不同，我們使用了中繼 `Task<int>` 變數來參考工作。  *這*是在非同步方法于另一個執行緒上完成之前，將控制權傳回給呼叫者的位置。
- 當非同步方法完成並傳回時，會在 `await` 繼續執行，這表示會傳回整數結果，然後在 UI widget 中呈現。
...
- [非同步 in C# 5 （Eric Lippert 的 blog）–關於關鍵字的引進](https://blogs.msdn.microsoft.com/ericlippert/2010/11/11/asynchrony-in-c-5-part-six-whither-async/)
