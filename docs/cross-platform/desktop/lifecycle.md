...
title: WPF 和。Xamarin.Forms App 生命週期
...
ms.locfileid: "61269316"
---
# <a name="wpf-vs-xamarinforms-app-lifecycle"></a>WPF 和。Xamarin.Forms App 生命週期
...
|--- |--- |--- |
|HorizontalAlignment|HorizontalOptions|左/Center/右/Stretch 選項|
|VerticalAlignment|VerticalOptions|頂端/Center/底端/Stretch 選項|
...
WPF 和 Xamarin.Forms 兩者都有資源與資源字典的概念。 您可以將任何物件型別`ResourceDictionary`具有索引鍵然後查詢它與`{StaticResource}`進行的操作不會變更，或`{DynamicResource}`可以變更在執行階段字典中的項目。 使用量和機制都相同，但是有一項差異：Xamarin.Forms 會要求您定義`ResourceDictionary`若要指派給`Resources`屬性而 WPF 預先會建立一個，並將它指派您。
