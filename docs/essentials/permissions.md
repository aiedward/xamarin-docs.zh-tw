...
如果未宣告所需的許可權，則會擲回 `PermissionException`。
...
當使用 `CheckStatusAsync` 或 `RequestAsync` 會傳回用來決定後續步驟的 `PermissionStatus`。
...
| 提醒 | ![不支援 Android](~/media/shared/no.png "不支援 Android") | ![支援的 iOS](~/media/shared/yes.png "支援的 iOS") | ![不支援 UWP](~/media/shared/no.png "不支援 UWP") | ![支援的 watchOS](~/media/shared/yes.png "支援的 watchOS") | ![不支援 tvOS](~/media/shared/no.png "不支援 tvOS") | ![不支援 Tizen](~/media/shared/no.png "不支援 Tizen") |
...
如果許可權標示為 [![不支援](~/media/shared/no.png "不支援")]，則在核取或要求時，一律會傳回 `Granted`。
...
在特定平臺中執行許可權時，可以從繼承 `BasePlatformPermission` 類別。 這會提供額外的平臺協助程式方法，以自動檢查宣告。
...
許可權在 `Info.plist` 檔案中必須有相符的字串。 要求和拒絕許可權後，如果您第二次要求許可權，就不會再出現快顯。 您必須提示使用者手動調整 iOS 中 [應用程式設定] 畫面中的設定。
